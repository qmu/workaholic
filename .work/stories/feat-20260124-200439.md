...
The `.work/changelogs/` directory has been eliminated. Tickets now serve dual purpose: describing change intent (Overview) and recording change metadata (frontmatter). The root CHANGELOG and branch stories are generated directly from archived tickets.
...
- Commands match their targets (`/sync-work` → `.work/`)
...
| Intuitivity    | Strong   | Changes align with developer expectations: `/sync-work` matches `.work/` directory, `created_at` follows `started_at`/`ended_at` conventions, approval flows are clear. |
