...
Add a rule requiring all markdown files under `.work/` (or `.workaholic/`) to have YAML frontmatter with at least `author` (git email) and `modified_at` (ISO timestamp) fields. This ensures traceability for all working artifacts.

## Key Files

- `plugins/core/rules/general.md` - Add the frontmatter requirement rule with path targeting `.work/**/*.md`
...
3. The rule should instruct Claude to:
   - Always include these fields when creating new files in `.work/`
