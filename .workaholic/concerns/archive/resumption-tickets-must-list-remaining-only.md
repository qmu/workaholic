...
resolved_by_pr: 
resolved_by_commit: 
closed_reason: Promoted to ticket 20260716163004-mission-floor-machine-checks.md (2026-07-16 triage-to-zero): lint resumption Implementation Steps as remaining-only now the approval gate can be skipped. Risk now tracked in the queue.
...
Add a body-section lint. Re-grade to urgent only if a resumption ticket is observed re-listing done steps — and reconsider the re-grade's reasoning now that an authorized queue drives unprompted.
