---
type: Feedback
title: Monitor's decision loop has no cross-run deferral memory
kind: concern
source: development
...
The front-loaded batch asks blockers once per run, but nothing makes a deferral sticky across invocations, so a caller-side loop would re-ask the same deferred decisions every cycle. Untouched by this branch; the `/drive` unification (phase 3) supersedes this loop. (See PR #88)

## How to Fix

Record deferred decisions in the run report and re-ask only when the underlying state changed — or resolve via the claim-branch protocol that replaces the loop.
