...
- [x] `/drive` is the unified executor: autonomous PR-unit partitioning, per-unit claim → drive → report, merge-policy routing with automated `/ship` for all-auto units and PR + Slack for the rest, agent-hours absorbed, 5-minute routine documented (#20260728221803-unify-drive-executor.md)
- [x] `/monitor`, `/trip`, `/carry`, the Agent Teams members, and the reflection channel are retired with docs and tests swept (#20260728221804-retire-monitor-trip-carry.md)
