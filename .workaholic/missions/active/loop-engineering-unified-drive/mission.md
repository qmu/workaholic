...
- [ ] The mission lifecycle is one `status` axis with an executable approval flip and per-artifact `merge_policy`; `drive_authorized` is retired via living migration and every reader keys on status (#20260728221801-unify-mission-status-and-merge-policy.md)
