...
英語が主要言語の場合、`.workaholic/` 内のすべてのドキュメントは対応する `_ja.md` 日本語翻訳を持つ必要があります（`plugins/drivin/skills/translate/SKILL.md`、121-127行目：「CRITICAL RULE: `.workaholic/` 内の `.md` ファイルを作成または編集する際は、consumer project の root CLAUDE.md をチェックして主要な記述言語を判定する必要があります」）。この要件はすべてのドキュメント writer subagent にプリロードされる `translate` skill を通じて強制されます。

### 主要言語検出ロジック

翻訳システムは consumer project の主要言語に基づいてどの翻訳を生成するかを決定します（`plugins/drivin/skills/translate/SKILL.md`、123-127行目）：
...
このロジックは主要言語が潜在的な翻訳対象と一致する場合にコンテンツの重複を防ぎます（`plugins/drivin/rules/i18n.md`、58-59行目：「主要言語を翻訳として重複させない」）。

### 翻訳ガイドライン

`translate` skill（`plugins/drivin/skills/translate/SKILL.md`）は日本語翻訳の包括的なポリシーを提供します：
...
各言語の README は同じ言語のドキュメントにリンクする必要があります（`plugins/drivin/skills/translate/SKILL.md`、143-149行目）。`README.md` は英語ドキュメントにリンクし、`README_ja.md` は `_ja.md` 翻訳にリンクし、並列ナビゲーション構造を維持します。これによりユーザーはドキュメントをナビゲートする際に言語コンテキスト内にとどまることができます。
...
日本語翻訳は包括的で、guide、spec、terms、story、policy をカバーしています（`.workaholic/` ディレクトリ内の395個の markdown ファイルのうち43個の `_ja.md` ファイルが観測され、バイリンガルコンテンツの10.9%カバレッジを表しています）。

## Translation Workflow

### ファイル命名規則

翻訳は元のファイルと同じディレクトリに suffix ベースの命名を使用します（`plugins/drivin/rules/i18n.md`、14-21行目、38-39行目）：
...
翻訳は `translate` skill ガイドラインに従って手動で実行されます。自動翻訳ツールやサービスは統合されていません。workflow は次のとおりです（`plugins/drivin/skills/translate/SKILL.md`、152-156行目）：
...
技術用語は日本語翻訳でも英語のまま保持されます（`plugins/drivin/skills/translate/SKILL.md`、79-83行目）。core concept（plugin、command、skill、rule、ticket、workflow）、git 用語（repository、branch、commit）、プログラミング概念（function、class、module、component）は技術的精度を維持し曖昧さを避けるため英語で保持されます。
...
検証 hook（`plugins/drivin/hooks/validate-ticket.sh`）は ticket ファイル標準を強制します：
...
i18n rule（`plugins/drivin/rules/i18n.md`）はパスベースのアクティベーションを使用します：
...
diagrams rule（`plugins/drivin/rules/diagrams.md`）は Mermaid diagram 構文を強制します：
...
Mermaid diagram はクロスプラットフォームレンダリングの改善のため ASCII アートを置き換えます（`plugins/drivin/rules/diagrams.md`、8-21行目）：
...
ボックス描画文字を使用した ASCII アート diagram は禁止されています（`plugins/drivin/rules/diagrams.md`、18-22行目）。

### 自己完結的定義

`.workaholic/terms/` の用語定義は、定義、使用コンテキスト、例、関連概念を組み込んだ包括的な単一段落として記述されます（`plugins/drivin/skills/write-terms/SKILL.md`、62-72行目）。これにより各エントリは相互参照を必要とせずに読みやすくなり、認知負荷が軽減され読者のアクセシビリティが向上します。
...
各用語は定義、使用コンテキスト、他の用語との関係とともに文書化されています（`plugins/drivin/skills/write-terms/SKILL.md`、61-71行目）。

### マルチ Plugin アクセシビリティロール

trippin plugin は3 agent 協調 workflow に明示的なアクセシビリティ責務を導入しています。Architect agent は「Accessibility & Accommodability」を core 責務として割り当てられており（`plugins/trippin/skills/trip-protocol/SKILL.md`、17行目）、trip workflow 中の仕様レビューにアクセシビリティの考慮が構造的に組み込まれることを保証しています。
...
- 主要言語検出ロジックは主要言語が翻訳対象と一致する場合に重複翻訳ファイルの作成を防ぎます
- `plugins/core/` から `plugins/drivin/` へのリネームにより、開発 plugin が独自のアイデンティティの下に統合され、すべての i18n およびアクセシビリティインフラストラクチャは変更なく保持されています
- trippin plugin は Architect agent にアクセシビリティレビューを構造的な責務として割り当て、協調的な仕様 workflow にアクセシビリティを組み込んでいます
...
- 観測されません：trippin plugin の i18n サポート（trippin plugin ドキュメントに `_ja.md` 翻訳が存在しません）
