...
Repository はすべての feature 作業に対して `<prefix>-YYYYMMdd-HHmmss` パターンに従う timestamp 付き topic branch を使用します。各 branch は進行中の作業の復元可能な snapshot を表します。複数の branch が同時に存在し、冗長性と branch 固有の問題からの復元能力を提供します。manage-branch skill はこれらの timestamp 付き branch を自動的に作成します。(`plugins/core/skills/manage-branch/sh/create.sh`)
