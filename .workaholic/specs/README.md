...
Viewpoint-based architecture specifications for the Workaholic plugin system. Each viewpoint analyzes the codebase from a different perspective. These documents are hand-maintained reference docs — update them through the `/ticket` workflow alongside structural changes. They are no longer regenerated automatically; the documentation command and the upstream context tier that drove it have been retired.

- [UX](ux.md) - Who uses the system, their goals, interaction patterns
