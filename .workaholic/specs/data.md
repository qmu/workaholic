...
The single `policies` index skill sets `user-invocable: false`. It is preloaded by commands and agents rather than triggered by users.
...
| Policies | Slug only | `<slug>.md` | `test.md`, `persistence.md`, `observability.md` |
...
The single `standards:policies` index is not a data-producing artifact. It is the policy lens preloaded into work-plugin commands and orchestrators (`/drive`, `/ticket`, `/trip`, `ticket-organizer`, `planner`, `architect`, `constructor`) via the soft cross-plugin reference pattern. Each preload site reads the index at scoping, implementation, and trip-protocol time, applying the relevant pillars to whatever layers the ticket touches (UX → 設計 plus 実装's アクセシビリティ viewpoint, Domain/DB → 実装 妥当性, Infrastructure → 実装 可用性 plus 運用, Config → whichever pillar the config touches). The index itself is a flat markdown document under `plugins/standards/skills/policies/SKILL.md` with `user-invocable: false` frontmatter; it has no producer-consumer relationship with other agents.
...
The standards plugin exposes a single `policies` skill (`plugins/standards/skills/policies/SKILL.md`) with `user-invocable: false` frontmatter. It is a flat markdown index that mirrors qmu.co.jp: per policy, a title, a one-line summary, and a canonical article link, organized into the 設計 / 実装 (with 妥当性 / 可用性 / アクセシビリティ sub-groups) / 運用 pillars. It carries no producer-consumer relationship with other agents and produces no intermediate artifacts.
...
- [Explicit] The `user-invocable: false` field distinguishes internal skills from user-facing commands. The `policies` index skill sets this field to false.
