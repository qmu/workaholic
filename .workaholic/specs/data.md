...
Constraint files live in `.workaholic/constraints/` and are generated by manager agents following the constraint-setting workflow defined in managers-principle. Each manager writes to a domain-specific file (project.md, architecture.md, quality.md). The frontmatter identifies which manager produced the constraints and when they were last updated. Constraint files use a structured markdown template with sections for each constraint, documenting bounds, rationale, affected leaders, compliance criteria, and review triggers.
