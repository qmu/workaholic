...
erDiagram
    ...
    CONSTRAINT {
        string manager "project-manager|architecture-manager|quality-manager"
        datetime last_updated "ISO 8601 with timezone"
    }
    TERMS {
        string title
        string description
...
### Constraint Frontmatter Schema

```yaml
---
manager: project-manager | architecture-manager | quality-manager
last_updated: 2026-02-08T13:17:51+09:00   # ISO 8601 datetime with timezone
---
```

Constraint files は `.workaholic/constraints/` に存在し、managers-principle で定義された constraint-setting workflow に従って manager agents によって生成されます。各 manager は domain-specific file（project.md、architecture.md、quality.md）に書き込みます。Frontmatter はどの manager が constraints を生成したか、いつ最後に更新されたかを識別します。Constraint files は各 constraint のセクションを持つ structured markdown template を使用し、bounds、rationale、affected leaders、compliance criteria、review triggers を文書化します。
...
Manager skills（manage-project、manage-architecture、manage-quality）と lead skills（lead-db、lead-security など）はすべて `user-invocable: false` を設定します。Cross-cutting policy skills（managers-principle、leaders-principle）も `user-invocable: false` を設定します。
...
Marketplace manifest は metadata、owner 情報、plugins のリストを宣言します。Marketplace は現在2つの plugin を含みます：drivin（ticket 駆動開発）と trippin（AI 指向の探索）。Root `version` フィールドは releases 中にすべての plugin manifest versions と同期を保つ必要があります。

### Plugin Manifest Schema

各 plugin は `plugins/<name>/.claude-plugin/plugin.json` に独自の manifest を持ちます。Version フィールドは `marketplace.json` 内の対応するエントリと一致する必要があります。
...
Version 同期は3つのファイルの同時更新を必要とします：`marketplace.json` root version、`plugins/drivin/.claude-plugin/plugin.json` version、`plugins/trippin/.claude-plugin/plugin.json` version。

### Hooks Configuration Schema

`plugins/drivin/hooks/hooks.json` に配置：
...
Trip artifacts は trippin plugin の Implosive Structure workflow によって生成されます。Artifacts は `.workaholic/.trips/<trip-name>/` にバージョン付きファイル名で保存されます。
...
Artifact types と storage locations：
...
各 revision は上書きではなく新しいファイル（例：`direction-v1.md`、`direction-v2.md`）を作成し、完全な review history を保存します。`Status` フィールドは consensus gate の進行に伴い `draft` から `under-review` を経て `approved` に遷移します。

Trip commit messages は `trip(<agent>): <step>` 形式に従い、commit body に phase 番号を含みます。
...
- Branch name patterns: `drive-<YYYYMMDD>-<HHMMSS>` (drivin)、`trip/<trip-name>` (trippin)
...
Version numbers は release 中に3つのファイル間で同期されます：`marketplace.json`（root version）、`plugins/drivin/.claude-plugin/plugin.json`、`plugins/trippin/.claude-plugin/plugin.json`。`/release` command はデフォルトで patch version をインクリメントし、3つのファイルすべてを更新し、commit して git tag を作成します。
...
Trip artifacts は `.workaholic/.trips/<trip-name>/` 内で versioned append-only pattern に従います。各 revision は上書きではなく新しい番号付きファイルを作成し、共同 review history の完全なシーケンスが committed versions として保存されます。Git worktree はすべての trip 作業を専用 branch（`trip/<trip-name>`）に分離し、main working tree をクリーンに保ちます。
...
| `skills` | N/A | N/A | `managers-principle` + domain skill をプリロード | `leaders-principle` + domain skill をプリロード |
...
Managers-principle で導入された constraint-setting workflow は 4 つの artifact types を生成し、それぞれ潜在的な frontmatter 要件を持ちます：
...
- [Explicit] `marketplace.json` と両方の plugin `plugin.json` ファイル（drivin と trippin）間の version synchronization は `CLAUDE.md` の version management section に文書化されているように、releases 中に必要です。
...
- [Explicit] Constraint-setting workflow（Analyze、Ask、Propose、Produce）は `managers-principle/SKILL.md` で定義され、すべての 3 つの manager skill の Execution sections で参照されています。
