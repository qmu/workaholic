...
この diagram は runtime と build-time の依存関係を示しています。Drivin と trippin plugin は共に Claude Code と Git に依存しています。Trippin plugin は追加で experimental な Agent Teams feature を必要とします。GitHub Actions CI は validation と release workflow のために Node.js 20、jq、gh に依存しています。
...
**Working artifact directories**(`.workaholic/`)は、生成された documentation、ticket、開発 narrative を保存します。これらは version 管理された出力であり、将来の開発判断のための context を提供します。Working artifact は以下の意味的区別を持ちます:

- `constraints/` -- Manager が生成する規範的な境界で、決定空間を狭める（例：release cadence、layer boundary rule、test coverage threshold）
- `policies/` -- Leader が生成する現在の practice の観察的 documentation（例：security 措置、testing strategy、deployment 手順）
- `specs/` -- Manager、model-analyst、ux-lead が生成する architecture viewpoint documentation
- `terms/` -- Terms-writer が codebase から抽出する一貫した term 定義
- `tickets/` -- Todo、archive、icebox、abandoned directory 内の work queue と implementation history
- `stories/` -- PR description のための branch ごとの開発 narrative
- `release-notes/` -- GitHub release のための簡潔な release note
- `guides/` -- User 向け documentation

**Source directories**（`plugins/drivin/` と `plugins/trippin/`）は、command、subagent、skill、rule、hook を含む plugin 実装を含みます。Drivin は ticket 駆動開発を提供し、trippin は AI 指向の探索 workflow を提供します。これらは plugin の動作の権威ある source です。
...
- **Skill path scope**: `manage-project`、`manage-architecture`、`manage-quality` の明示的な path(utility skill である `branching` とのマッチングを回避)
...
**Cross-cutting policy skill** は複数の agent に適用される behavioral policy を定義します。`leaders-principle` skill は全ての lead agent に Prior Term Consistency と Vendor Neutrality rule を提供します。`managers-principle` skill は manager agent に Strategic Focus rule を追加します。
...
`.claude/` directory は Claude Code plugin installation target として機能します。Marketplace plugin がインストールされると、Claude Code は `.claude/` から `plugins/drivin/` 内の実際の plugin source への symlink を作成します。これにより、repository は plugin source を Claude Code configuration directory とは別に維持しながら、Claude Code が plugin を正しく load できるようになります。

Symlink 構造は以下の通りです:

```
~/.claude/plugins/marketplaces/workaholic/plugins/drivin/ -> plugins/drivin/
~/.claude/plugins/marketplaces/workaholic/plugins/trippin/ -> plugins/trippin/
```
...
この sequence diagram は、marketplace 登録から installation を経て runtime command 解決までの plugin の流れを示しています。Installed path(`~/.claude/plugins/marketplaces/workaholic/`)が runtime reference として機能し、開発 edit は `plugins/` で行われます。
...
2. `.claude/` から `plugins/drivin/` subdirectory への symlink を確立
...
Plugin は Write または Edit 操作後に ticket file format と location を validation する PostToolUse hook をインストールします。`plugins/drivin/hooks/hooks.json` の hook configuration は、Write と Edit tool の matcher を指定し、10 秒の timeout で `validate-ticket.sh` script を実行します。
...
- `plugins/drivin/.claude-plugin/plugin.json` の drivin plugin `version` field
- `plugins/trippin/.claude-plugin/plugin.json` の trippin plugin `version` field

3つの file はすべて同じ version 番号（現在は `1.0.38`）を指定する必要があります。Version bump は一貫性を維持するためにすべてのファイルを同時に更新します。
...
- `CLAUDE.md`、`README.md`、`plugins/drivin/README.md` - documentation 更新
- `.claude-plugin/marketplace.json`、`plugins/drivin/.claude-plugin/plugin.json`、`plugins/trippin/.claude-plugin/plugin.json` - version 管理

PostToolUse hook は ticket write を validation するために `plugins/drivin/hooks/validate-ticket.sh` への実行 permission を必要とします。`plugins/drivin/skills/*/sh/` directory 内の全ての bundled shell script は実行 permission を必要とします。
...
Trippin plugin の `/trip` command は、Agent Teams 機能を有効にするために `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 環境変数を必要とします。この変数が設定されていない場合、Agent Teams の起動は失敗します。
...
**Lead tier** agent(11 の domain 固有 lead、`infra-lead`、`security-lead`、`quality-lead` など)は manager output を consume して domain 固有の documentation を生成します。これらは domain skill(`lead-<specialty>`)と cross-cutting `leaders-principle` skill の両方を preload します。
...
[Explicit] `.claude-plugin/marketplace.json` file は version 1.0.38 と owner email a@qmu.jp を指定しており、2つの plugin（drivin と trippin）を含む現在の marketplace version と ownership を示しています。
...
[Explicit] `plugins/drivin/hooks/hooks.json` の PostToolUse hook configuration は 10 秒の timeout で Write と Edit 操作を validation します。

[Explicit] `.claude/settings.json` file は Bash command pattern `git -C:*` を明示的に deny します。

[Explicit] `define-manager.md` schema enforcement rule は、utility skill である `branching` とのマッチングを回避するために glob pattern ではなく明示的な skill path を使用します。

[Explicit] Manager agent(`project-manager`、`architecture-manager`、`quality-manager`)は `managers-principle` skill を preload し、lead agent は `leaders-principle` skill を preload します。

[Inferred] `.claude/` から `plugins/drivin/` への symlink architecture は、project 構造ルール「`plugins/` を編集し、`.claude/` は編集しない」と marketplace installation pattern から推測されますが、明示的な symlink 作成 code は観察されませんでした。
...
[Explicit] Trippin plugin は experimental な Agent Teams feature flag `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` を必要とします。これは trip.md に document されています。

[Explicit] Trippin plugin は session 隔離のために git worktree を使用し、`.worktrees/<trip-name>/` directory と `trip/<trip-name>` branch を作成します。

[Explicit] Marketplace は現在 2 つの plugin（drivin と trippin）を synchronized version で含んでおり、marketplace.json に示されています。

[Inferred] Single-plugin から multi-plugin marketplace への拡張は、CI infrastructure が validation に glob pattern(`plugins/*/.claude-plugin/plugin.json`)を使用していることを検証しています。
