...
The nesting hierarchy is strictly enforced: commands at the top, managers and leads in the middle, subagents below that, skills at the bottom. Commands can invoke skills, subagents, managers, and leads. Managers and leads can invoke skills and other subagents. Subagents can invoke skills and other subagents. Skills can only invoke other skills. This hierarchy ensures that orchestration remains thin and knowledge is centralized in reusable skills.
...
The relationships reflect the workflow: commands create tickets and stories, managers produce strategic outputs and some specs, leads produce domain-specific specs and policies, and tickets trigger updates to both specs and policies. Managers execute before leads to ensure strategic context is available.
...
**Manager output consumption**: Every manager output must be consumable by at least one leader. This invariant is enforced through the managers-policy Strategic Focus rule and the define-manager schema which requires each output to name its consuming leaders. Managers without leader consumers violate the strategic tier's purpose.
...
**Subagent naming**: Non-manager, non-lead subagents use role-based naming with `-writer`, `-analyst`, or `-organizer` suffixes. The pattern is `<domain>-<role>`: `ticket-organizer` (organizes ticket creation), `story-writer` (writes stories), `changelog-writer` (writes changelog), `model-analyst` (analyzes model viewpoint), `terms-writer` (writes terms). The suffix indicates the subagent's function: analyzers produce specs/policies, writers produce changelog/terms/stories, organizers coordinate workflows.

**Skill naming**: Skills use verb-noun phrases that describe their purpose: `gather-git-context`, `archive-ticket`, `write-spec`, `validate-writer-output`, `translate`, `managers-policy`, `leaders-policy`. The naming makes skills self-documenting and suggests their appropriate usage contexts. Policy skills use `-policy` suffix to indicate cross-cutting behavioral rules.
...
- [Explicit] The manager tier was introduced in ticket 20260211170401-define-manager-tier-and-skills.md (commit 8dc1ce4), adding 3 managers above the existing 10 leads.
- [Explicit] The `/scan` command was updated to invoke managers before leaders in ticket 20260211170402-wire-leaders-to-manager-outputs.md to ensure manager outputs are available for leader consumption.
- [Explicit] The communication-lead was renamed to ux-lead in ticket 20260211170402-wire-leaders-to-manager-outputs.md (commit a1b2c3d reference inferred from diff output showing `lead-communication` → `lead-ux` rename).
...
- [Explicit] The define-manager schema is enforced via `.claude/rules/define-manager.md` with path patterns for `plugins/core/skills/manage-project/SKILL.md`, `plugins/core/skills/manage-architecture/SKILL.md`, `plugins/core/skills/manage-quality/SKILL.md`, and `plugins/core/agents/*-manager.md`.
...
- [Explicit] The managers-policy skill defines three cross-cutting policies for all managers: Prior Term Consistency, Strategic Focus, and Constraint Setting. Vendor Neutrality was removed per user feedback (ticket 20260211170401 final report line 122).
- [Explicit] The leaders-policy skill defines two cross-cutting policies for all leads: Prior Term Consistency and Vendor Neutrality.
- [Explicit] Manager outputs are defined in the manage-* skill files with explicit naming of consuming leaders: manage-project outputs consumed by all leads (primary: delivery-lead, recovery-lead), manage-architecture outputs consumed by infra-lead, db-lead, security-lead, observability-lead, recovery-lead, manage-quality outputs consumed by quality-lead, test-lead, a11y-lead.
- [Explicit] The architecture-manager absorbed the responsibilities of the former architecture-lead (viewpoint specs: application, component, feature, usecase) as documented in manage-architecture skill Outputs section.
- [Explicit] The scan command now has 8 viewpoint specs (stakeholder, model, ux, usecase, infrastructure, application, component, data, feature) with ux-lead producing ux.md and architecture-manager producing application.md, component.md, feature.md, usecase.md.
...
- [Inferred] The Constraint Setting workflow in managers-policy suggests that managers are primarily responsible for narrowing the decision space through deliberate boundaries (policies, guidelines, roadmaps, decision records), not for executing work directly.
