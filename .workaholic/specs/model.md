...
A skill is the knowledge layer of the system, containing templates, guidelines, rules, and bundled shell scripts. Skills are comprehensive (approximately 50-150 lines), live in `plugins/<plugin-name>/skills/`, and may include an `sh/` directory with POSIX shell scripts. Skills can invoke other skills but cannot invoke subagents or commands. Shell scripts within skills are permission-free and use absolute paths from `~/.claude/plugins/marketplaces/workaholic/plugins/<plugin-name>/skills/<skill-name>/sh/<script>.sh`. Skills establish conventions and provide reusable domain knowledge to commands and subagents. The drivin plugin contains skills including gather-git-context (branch, base, URL extraction), gather-ticket-metadata (datetime, author), archive-ticket (lifecycle state transitions), write-changelog (CHANGELOG.md formatting), write-spec (spec document guidelines), write-terms (terminology management), translate (i18n conventions), validate-writer-output (post-generation verification), managers-principle (cross-cutting manager behavioral principles), leaders-principle (cross-cutting leader behavioral principles), manage-project (project manager skill), manage-architecture (architecture manager skill), manage-quality (quality manager skill), lead-ux (UX lead skill), domain-specific lead skills for each of the 10 leads, drive-approval (user approval flow with ticket context enforcement), and drive-workflow (implementation workflow). The trippin plugin contains trip-protocol (Implosive Structure workflow, artifact conventions, worktree isolation, and commit-per-step discipline).

### Rule

A rule is a system-wide behavioral constraint applied via path patterns. Rules live in `plugins/drivin/rules/` and include general guidelines (architecture nesting policy, thin orchestration principle), diagram policies (Mermaid requirements, label quoting), i18n requirements (mandatory `_ja.md` translations for `.workaholic/`), shell scripting standards (POSIX compliance, bundled scripts in skills), TypeScript conventions, workaholic directory conventions (file structure, frontmatter requirements), define-lead schema enforcement (for `*-lead.md` agents and `lead-*/SKILL.md` skills), and define-manager schema enforcement (for `*-manager.md` agents and `manage-*/SKILL.md` skills). Rules are loaded by Claude Code based on path matching and influence behavior globally without explicit invocation. The trippin plugin currently has no custom rules (only a `.gitkeep` placeholder), relying on inherited rules from the drivin plugin.

### Version

A version is a semantic versioning string (MAJOR.MINOR.PATCH) that synchronizes across three configuration files: `.claude-plugin/marketplace.json` (root `version` field), `plugins/drivin/.claude-plugin/plugin.json` (drivin version), and `plugins/trippin/.claude-plugin/plugin.json` (trippin version). Version management follows CLAUDE.md conventions: read current version from marketplace.json, increment PATCH by default (e.g., 1.0.0 to 1.0.1), update all three files, and commit with message `Bump version to v{new_version}`. The `/report` command automatically performs a patch increment before invoking story-writer, ensuring every PR merge triggers a new release via the GitHub Actions release workflow (`.github/workflows/release.yml`) which compares marketplace.json version against the latest release tag. The branching skill provides a `check-version-bump.sh` script that detects existing "Bump version" commits in the current branch to prevent double-bumping when `/report` runs multiple times.
...
The domain model enforces strict relationships between entities, governed by the component nesting hierarchy and lifecycle state machines. The introduction of managers adds a strategic tier above leads, creating a three-level hierarchy: managers define strategic context, leads consume that context to produce domain-specific documentation, and both consume skills for domain knowledge. The trippin plugin introduces a parallel relationship model where three agents of equal standing collaborate through shared artifacts rather than through a hierarchical invocation chain.
...
**Version synchronization**: The marketplace version (`.claude-plugin/marketplace.json`) and both plugin versions (`plugins/drivin/.claude-plugin/plugin.json`, `plugins/trippin/.claude-plugin/plugin.json`) must remain synchronized. Version bumps update all three files atomically. This invariant ensures that the marketplace catalog and all plugin manifests agree on the distributed version.
...
**Command naming**: Commands use imperative verbs or short nouns that directly describe user intent: `/ticket` (create work item), `/drive` (implement work items), `/scan` (update documentation), `/report` (generate PR), `/trip` (explore collaboratively). The drivin plugin commands reflect a ticket-driven development vocabulary, while the trippin plugin's `/trip` command uses a journey metaphor consistent with the plugin's exploration-oriented identity.

**Plugin naming**: Plugins use colloquial present-participle forms that convey their operational character: `drivin` (driving development forward through tickets) and `trippin` (taking a trip to explore ideas). The informal naming distinguishes plugins from the formal domain vocabulary used for agents and skills.
...
- [Explicit] The marketplace contains two plugins (`drivin` and `trippin`), as seen in `.claude-plugin/marketplace.json`.
- [Explicit] Version synchronization across marketplace.json and both plugin.json files is documented in `CLAUDE.md` Version Management section.
...
- [Explicit] The define-manager schema is enforced via `.claude/rules/define-manager.md` with path patterns for `plugins/drivin/skills/manage-*/SKILL.md` and `plugins/drivin/agents/*-manager.md`.
- [Explicit] The define-lead schema is enforced via `.claude/rules/define-lead.md` with path patterns for `plugins/drivin/skills/lead-*/SKILL.md` and `plugins/drivin/agents/*-lead.md`.
...
- [Inferred] The expansion from a single plugin (`core`) to two plugins (`drivin`, `trippin`) signals a design philosophy of modular, independently-evolving workflow plugins with distinct orchestration models -- one hierarchical and ticket-driven, the other lateral and consensus-driven.
...
- [Inferred] The trippin plugin's use of `model: opus` for all three agents (compared to drivin's `model: sonnet` for scan subagents) suggests that collaborative exploration requires higher-capability reasoning, while documentation generation can be handled by a more efficient model.
...
- [Inferred] The trippin plugin's lack of custom rules (only `.gitkeep`) suggests it either inherits sufficient behavioral constraints from the drivin plugin's rules or that rule development for the trippin domain is planned but not yet implemented.
- [Inferred] The dual objectives framework (Goal: optimization problem; Responsibility: constraint satisfaction) in the trip-protocol skill mirrors the broader Workaholic pattern of balancing creative output with structural constraints, seen also in the manager-leader relationship.
