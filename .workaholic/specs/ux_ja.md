...
開発者は `/report` を呼び出して story を生成し PR を作成します。report command はまず両方の version ファイルで version を bump し、次に story-writer subagent を呼び出します。story-writer は4つの agent を並列実行し(release 分析のための release-readiness、決定品質のための performance-analyst、ナラティブセクションのための overview-writer、outcome/concerns/ideas のための section-reviewer)、story ファイルを `.workaholic/stories/<branch>.md` に作成し、それをコミットしてプッシュし、次に2つの agent をさらに並列実行します(release note のための release-note-writer、GitHub PR 作成のための pr-creator)。

配信ジャーニーは、蓄積された ticket 履歴を凝集したナラティブに変換します。Story のセクション4(Changes)は、すべての変更されたファイルをリストする代わりに ticket ごとの簡潔な要約を提示するようになり、story を網羅的なファイル changelog ではなく開発ナラティブとして読めるようにします。生成された story は PR description となり、レビュアーに motivation、journey、決定品質に関するコンテキストを提供します。
...
最近追加された manager skill は、plugin 作者のための制約設定 workflow を導入しました。Manager が明示的なプロジェクト制約(release cadence、stakeholder 優先順位、scope 境界)を欠いている場合、`managers-policy` からの Constraint Setting workflow に従います:

1. 収集した証拠を分析して、欠落しているまたは暗黙的なプロジェクト制約を特定
2. ビジネス優先順位、stakeholder ランキング、scope 決定に関する対象を絞った質問をユーザーに尋ねる
3. 収集した証拠とユーザーの回答に基づいたプロジェクト制約を提案
4. 指向性のある資料(roadmap、stakeholder 優先度マトリックス、scope 境界ドキュメント)を `.workaholic/` に生成

このジャーニーは、暗黙の仮定を将来の開発を導く明示的な制約に変換します。例えば、project-manager は「このプロジェクトの目標 release cadence は何ですか?」と尋ね、その回答を使用して delivery-lead が release readiness を評価する際に消費する timeline コンテキストを設定できます。
...
```mermaid
flowchart TD
    Start["開発者が /report を入力"] --> Bump["version を bump"]
...
Agent は `plugins/core/skills/` の skill を通じて workflow 固有の知識を受け取ります。例えば、gather-git-context skill は、バンドルされた shell script を介して git コンテキスト収集を提供し、agent markdown のインライン git command を排除します。create-ticket skill は ticket 形式とコンテンツ要件を定義し、全ての ticket-organizer 呼び出しで一貫した ticket 構造を保証します。
...
最近のアーキテクチャ変更は、透明性、構造、downstream 消費に関する進化する UX 優先順位を反映しています。
