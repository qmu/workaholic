...
flowchart LR
  ...
  subgraph Subagents[Subagent Architecture]
    sync-subagent --> story-subagent --> changelog-subagent --> pr-creator-subagent
  end

  subgraph GitSafety[Git Command Safety]
    add-guidelines --> strengthen-guidelines --> embed-guidelines --> deny-rule
  end

  subgraph Skills[Skill Extraction]
    changelog-skill & story-metrics & spec-context & pr-ops & drive-workflow & ticket-format
  end

  subgraph Commands[Command Simplification]
    remove-sync & remove-commit & rename-pr & rename-terms & rename-scripts
  end

  subgraph Infra[Infrastructure]
    posix-sh & rest-api & i18n-skill
  end
```
