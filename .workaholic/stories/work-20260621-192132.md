...
4. Keep the feature branch on merge (`gh pr merge --delete-branch=false`), and add a real hermetic policy-lens injection test
...
Made `gh pr merge` keep the feature branch and stop prompting to delete the remote branch. (The original `--no-delete-branch` flag does not exist in `gh`; the shipped fix uses the explicit negated bool `--delete-branch=false`, discovered when this branch was itself shipped via `/ship`.)
...
- `/ship` keeps the feature branch on merge via `gh pr merge --delete-branch=false`.
