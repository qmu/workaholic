...
- Two complementary layers now enforce the canonical layout: Write/Edit creations (PostToolUse) and shell moves (PreToolUse Bash). The move path that produced every observed violation is now blocked at the source.
...
- **Diagnose-then-enforce from a live failure**: the guard's design was driven by a real consumer repo, so it targets the actual move path that broke, not a hypothetical one.
...
- Once published, a consumer repo (`autoUpdate: true`) pulls the enforcement automatically — confirm a subsequent `bash mv` into a non-canonical tickets path is blocked there.
