...
### Tab-delimited read loops silently drop empty fields

- **Severity:** low
- **Description:** `apply-deferred-concern-verdicts.sh` shifted a commit hash into the `resolved_by_pr` column whenever `resolved_by_pr` was absent, because tab is IFS *whitespace* and `read` collapses consecutive tabs into one delimiter (fixed in [c8f23204](https://github.com/qmu/workaholic/commit/c8f23204) with an absent-field sentinel and four regression tests). The fix is local to that script; the underlying `IFS=<tab> read` idiom appears elsewhere in the skill scripts and would fail the same way with an interior empty field.
- **How to Fix:** When adding or touching a tab-delimited `read` loop, emit a sentinel for absent fields rather than an empty run — or grep the script family for `IFS="$tab" read` and audit each for interior optional columns.
...
- **Running the workflow on itself surfaced a real defect.** This report's own deferred-concern pass exposed the field-shift bug in [c8f23204](https://github.com/qmu/workaholic/commit/c8f23204) — the tooling writing wrong data into an append-only stream, caught only because the record was read back and inspected rather than trusted. Reading back what a mutator just wrote is cheap and catches the class of bug that unit tests with fully-populated fixtures miss.
