...
- **When:** 2026-08-01T02:08:24+09:00
- **By:** a@qmu.jp
- **Target:** release-scan
- **Method:** override
- **Status:** bypassed
- **Observed:** too-large-commit on 5b2b2a7b: 685 non-generated changed lines > 500 ceiling; outputs/** correctly exempted, overage is real source (3 new scripts ~430 lines + docs + tests). Accepted by developer: archive.sh emits one commit per ticket, so a ticket this size cannot satisfy the ceiling by construction; the ceiling's handling belongs to the open mission.

## Deployment Evidence

- **When:** 2026-08-01T02:09:50+09:00
- **By:** a@qmu.jp
- **Target:** Workaholic marketplace plugin
- **Method:** other
- **Status:** pass
- **Observed:** Pre-merge proof per .workaholic/deployments/marketplace.md (deploy-on-merge target): (1) build.mjs run, git status --porcelain outputs/ empty - outputs fresh; (2) verify.mjs 'All built skills are self-contained' and validate-metadata.mjs 'valid and version-aligned'; (3) test-workflow-scripts.mjs 1555 passed / 0 failed; (4) version 1.0.113 consistent across marketplace.json (3 entries), both plugin manifests, and the generated Codex manifest. v1.0.112 already released, so 1.0.113 does not collide.
