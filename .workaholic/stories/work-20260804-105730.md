...
- **Severity:** moderate
- **Description:** This branch's `validate` job failed on
  `mission resolution follows the ticket, not the cwd` with
  `ENOTEMPTY: directory not empty, rmdir '/tmp/workaholic-smoke-…/.git/objects'` — a
  temp-directory **cleanup race** in the test harness, not a failed assertion. The same
  suite passed locally at 2051/0 and the job passed on a plain re-run with no code change.
  The consequence is not cosmetic: `validate` is a required check, so a flake of this class
  blocks a merge and is indistinguishable at a glance from a real regression.
- **How to Fix:** Make the harness's teardown tolerate a busy `.git/objects` — retry the
  `rmSync` with a short backoff, or `rmSync(..., {maxRetries})` — so a background git process
  holding the directory cannot fail a run that has already passed every assertion.

### The per-commit size ceiling counts a catch-up merge as authored work

- **Severity:** moderate
- **Description:** Every one of the four branches shipped after the first hit
  `too-large-commit` on its `Merge remote-tracking branch 'origin/main'` commit — 877, 1266,
  1870 non-generated lines. None of those lines was authored by the branch: they are the
  earlier PRs' work, already reviewed and already on `main`. The rule measures a merge
  commit against its first parent, so routine ship hygiene reads as an oversize commit and
  the gate blocks. Four overrides were recorded in one run.
- **How to Fix:** Exempt a merge commit whose second parent is an ancestor of the base, or
  measure a merge against its merge-base rather than its first parent. This is squarely the
  question the active mission *Make the per-commit changed-lines ceiling a rule that holds*
  exists to settle, and this run is four measured instances for it.
...
- **When:** 2026-08-04T12:45:27+09:00
- **By:** a@qmu.jp
- **Target:** release-scan
- **Method:** override
- **Status:** bypassed
- **Observed:** size/too-large-commit on 182befd6 (1870 non-generated lines > 500). That commit is the catch-up MERGE of origin/main; every line is PR #179/#180/#181/#182 work already reviewed and already on main, authored by no commit of this branch. Overridden on the developer's explicit merge instruction; recorded as a concern for the per-commit-ceiling mission.

## Deployment Evidence

- **When:** 2026-08-04T12:45:55+09:00
- **By:** a@qmu.jp
- **Target:** Workaholic marketplace plugin
- **Method:** other (branch/staging proof per the deployment contract)
- **Status:** pass
- **Observed:** reconciled with main; outputs fresh (0 changes after rebuild); verify.mjs and validate-metadata.mjs pass and report all five version files aligned at 1.0.124; test-workflow-scripts.mjs 2083 passed 0 failed; CI validate+freshness green
