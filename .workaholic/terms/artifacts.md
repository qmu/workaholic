...
A spec is current state documentation that provides an authoritative reference snapshot of the codebase. Unlike tickets (which describe changes), specs describe what exists now and are updated via the spec-writer subagent during `/scan` or `/report` to reflect the current state after changes. Specs live in `.workaholic/specs/` as viewpoint-based architecture documents: `stakeholder.md`, `model.md`, `usecase.md`, `infrastructure.md`, `application.md`, `component.md`, `data.md`, and `feature.md` (each with a `_ja.md` translation). The previous ad-hoc structure (`architecture.md`, `command-flows.md`, `contributing.md`) has been replaced by this systematic 8-viewpoint approach. Related terms: ticket, story, viewpoint. Note: The `/ticket` command description mentions "implementation spec" which conflates ticket and spec terminology.

## policy

A policy is a document describing repository practices and standards for a specific operational domain. Workaholic defines 7 policy domains: test, security, quality, accessibility, observability, delivery, and recovery. Policy documents live in `.workaholic/policies/` as `<slug>.md` and `<slug>_ja.md` files. Each policy documents observable practices with `[Explicit]` and `[Inferred]` annotations, and marks areas with no evidence as "Not observed." Generated by the policy-writer subagent during `/scan`. Related terms: spec, scan, policy-analyst.
...
The changes section is section 4 of a story document, providing a comprehensive list of all changes as subsections in the format `### 4-N. <Ticket title> ([hash](commit-url))`. Each subsection contains a 1-2 sentence description from the ticket's Overview, serving as the detailed "what changed?" companion to Journey's "how did we get here?". Note: H3 heading numbering uses hyphen notation (`4-1.`) rather than the previous dot notation (`4.1.`). Appears in `.workaholic/stories/<branch-name>.md`. Related terms: story, journey, ticket.
