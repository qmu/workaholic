...
directionは`/trip`ワークフローのフェーズ1（Specification）でPlannerエージェントによって生成されるtripアーティファクトです。trip目標に対する創造的方向性、ステークホルダープロファイリング、説明責任を記録します。directionアーティファクトは`.workaholic/.trips/<trip-name>/directions/`に`direction-v1.md`、`direction-v2.md`などとしてバージョン管理され、コンセンサス前にArchitectとConstructorエージェントによるレビューを受けます。関連用語：trip、model、design。

## model

modelは`/trip`ワークフローのフェーズ1（Specification）でArchitectエージェントによって生成されるtripアーティファクトです。trip目標に対する意味的一貫性、静的検証、アクセシビリティの考慮事項を記録します。modelアーティファクトは`.workaholic/.trips/<trip-name>/models/`に`model-v1.md`、`model-v2.md`などとしてバージョン管理され、コンセンサス前にPlannerとConstructorエージェントによるレビューを受けます。関連用語：trip、direction、design。

## design

designは`/trip`ワークフローのフェーズ1（Specification）でConstructorエージェントによって生成されるtripアーティファクトです。trip目標に対する持続可能な実装計画、インフラストラクチャの信頼性の考慮事項、デリバリーの調整を記録します。designアーティファクトは`.workaholic/.trips/<trip-name>/designs/`に`design-v1.md`、`design-v2.md`などとしてバージョン管理され、コンセンサス前にPlannerとArchitectエージェントによるレビューを受けます。関連用語：trip、direction、model。
