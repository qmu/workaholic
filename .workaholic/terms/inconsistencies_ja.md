...
過去のドキュメントは、drivinプラグイン（旧「core」）に統合された別の「TDD plugin」（`plugins/tdd/`）を参照している可能性があります。

### 現在の使用状況

- 現在：すべての開発コマンド、スキル、ルールは`plugins/drivin/`にある
- 過去：一部のドキュメントは`plugins/tdd/`や「TDD plugin」を参照している可能性がある

### 推奨される解決策

TDDプラグインへの残りの参照をdrivinプラグインへの参照に更新します。チケット駆動開発コマンド（`/ticket`、`/drive`）は現在、drivinプラグインの一部です。

## レガシー「core」プラグイン参照

### 問題

`plugins/core/`ディレクトリは新しいtrippinプラグインと並んで独自の名前を付けるため`plugins/drivin/`に改名されました。すべての`subagent_type: "core:*"`参照は`"drivin:*"`に、すべてのインストール済みプラグインパスは`~/.claude/plugins/marketplaces/workaholic/plugins/core/`から`~/.claude/plugins/marketplaces/workaholic/plugins/drivin/`に更新されました。

### 現在の使用状況

- 現在のプラグインディレクトリ：`plugins/drivin/`
- 現在のサブエージェントタイププレフィックス：`"drivin:*"`
- 現在のインストール済みパス：`~/.claude/plugins/marketplaces/workaholic/plugins/drivin/`
- 過去：`plugins/core/`、`"core:*"`、`~/.claude/plugins/marketplaces/workaholic/plugins/core/`
- `.workaholic/specs/`および`.workaholic/policies/`下の生成ドキュメントは`/scan`で再生成されるまで`plugins/core/`参照を含む可能性があります

### 推奨される解決策

残りのアクティブな参照を`plugins/core/`から`plugins/drivin/`に、`"core:*"`から`"drivin:*"`に更新します。過去のドキュメント（アーカイブされたチケット、ストーリー）はその時点の状態を記録しているため、変更しないでください。生成されたスペックとポリシーは次回の`/scan`実行時に自動的に更新されます。
...
- 現在のディレクトリ：`sh/`（例：`plugins/drivin/skills/write-changelog/sh/generate.sh`）
...
過去のポリシードキュメントは、強制されたポリシーと願望的なポリシーを区別するために使用された`[Explicit]`と`[Inferred]`バッジを参照している可能性があります。このバッジシステムは完全に削除されました。現在のポリシードキュメントは、コードベースに実際に実装され実行可能なポリシーのみを文書化します。

### 現在の使用状況

- 現在：バッジなし。各ステートメントは強制メカニズム（CI check、git hook、linter、script、test）を引用します
- 過去：強制されたポリシーには`[Explicit]`プレフィックス、願望的なプラクティスには`[Inferred]`
- コード強制のない文書化のみの規約は現在ポリシーから除外されます

### 推奨される解決策

ポリシーが現在検証可能でコード強制されたプラクティスのみを文書化することを説明するように参照を更新してください。過去のドキュメント（アーカイブされたポリシーファイル）は作成時の規約を反映しているため、変更しないでください。

## レガシー "/story" commandとscannerサブエージェントの参照

### 問題

`/story` commandは削除され、scannerサブエージェントは削除されました。`/story` commandの機能（完全なドキュメントscan + PR作成）は`/scan`（ドキュメント更新）と`/report`（PR作成）に分割されました。scannerサブエージェントのオーケストレーションロジックは`/scan` commandに直接移行されました。

### 現在の使用状況

- 現在のcommand：ドキュメント更新用の`/scan`、PR作成用の`/report`
- 現在のアーキテクチャ：`/scan`は17のエージェントを直接呼び出します（scannerサブエージェントなし）
- 過去のcommand：`/story`はscanとPR作成の両方を実行しました
- 過去のアーキテクチャ：scannerサブエージェントが17のエージェントをオーケストレートしました

### 推奨される解決策

commandの分割とアーキテクチャの平坦化を説明するように参照を更新してください。`/scan` commandは現在、scannerサブエージェントに委譲する代わりにエージェントを直接呼び出すことで、リアルタイムのエージェント毎の進捗可視性を提供します。過去のドキュメントは変更しないでください。
