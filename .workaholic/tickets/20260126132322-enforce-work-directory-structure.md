...
Add a rule to the core plugin that restricts the `.workaholic/` directory to only contain designated subdirectories. This prevents both users and Claude from accidentally creating non-standard directories that would fragment the artifact organization.
...
2. The rule should specify:
   - Only these directories are allowed under `.workaholic/`:
...
4. Update the `paths` frontmatter to target `.workaholic/**/*` so it triggers when working in that directory
