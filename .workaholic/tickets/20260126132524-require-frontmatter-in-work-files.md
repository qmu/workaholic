...
Add a rule requiring all markdown files under `.workaholic/` to have YAML frontmatter with at least `author` (git email) and `modified_at` (ISO timestamp) fields. This ensures traceability for all working artifacts.

## Key Files

- `plugins/core/rules/general.md` - Add the frontmatter requirement rule with path targeting `.workaholic/**/*.md`
