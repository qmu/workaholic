...
1. **Common Operations table updated** — Both paths changed from relative `.claude/skills/` to absolute `~/.claude/plugins/marketplaces/workaholic/plugins/core/skills/`
2. **Skill Script Path Rule section added** — New subsection inserted after Shell Script Principle, before `## Commands`, with CRITICAL callout, explanation, and Wrong/Correct example pair
3. **Shell Script Principle verified** — The existing "Correct" example on line 87 already used the absolute path; no change needed

Single file changed: `CLAUDE.md`
