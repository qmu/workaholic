...
- **Reaches data-platform only after release**: with `autoUpdate: true` now set there, the hardened hooks take effect in `data-platform` only once a new plugin version is published via `/release` (separate, outward-facing step).
