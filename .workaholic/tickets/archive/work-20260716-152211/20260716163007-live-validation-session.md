...
- **(4) Living layout migration — VALIDATED,** in the consumer repos' own histories as the concern demanded: `plgg` commit `0fb13c9b` and `data-platform` commit `2ad863443` record the migration relocating flat `missions/<slug>/` dirs into `missions/active/<slug>/` via history-preserving renames (R100). Every consumer repo on this machine with missions (`InsightBrowser`, `data-platform`, `plgg`, `plggmatic`, `qfs`, `research`) is now on the nested layout; no flat-layout repo remains to migrate.
