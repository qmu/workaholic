...
Development completed as planned. The migration logic lives as `missions_migrate_strategies` in `lib/resolve.sh` (called from the existing `missions_migrate_layout` seam every mission script already runs) with `migrate-strategies.sh` as a thin direct/test wrapper — one implementation, two entries. This repo's own strategy (`agent-orchestrated-development`) was migrated live: its Direction survives verbatim as `feedbacks/20260721033556-strategy-agent-orchestrated-development.md` and `.workaholic/strategies/` is gone from tree, allowlist, and rules table in this same change.

### Discovered Insights

- **Insight**: `read-assignees.sh` moved into the mission skill (git mv, history preserved) rather than being copied — it was already a generic `assignees:` parser, and the mission's tier-1 read plus the migration's fold both use it, so the field shape still lives in exactly one file after the strategy skill's deletion.
  **Context**: `plugins/workaholic/skills/mission/scripts/read-assignees.sh`.
- **Insight**: validate-ticket.sh's ticket-shape rule (`YYYYMMDDHHMMSS-*.md` forbidden outside `tickets/`) collided with feedback filenames the moment a feedback path went through the layout tests — feedbacks/ now carries an explicit exemption there, since `validate-feedback.sh` owns the same-shape floor for that area. Concern filenames had dodged this with a different shape; feedback keeps the chronological shape deliberately.
  **Context**: `plugins/workaholic/hooks/validate-ticket.sh` ticket-shape case.
- **Insight**: The mission-owners oracle dropping its strategy hop needed no transition window in practice because the fold rides the same living-migration seam every mission script touches first — by the time any consumer resolves ownership in a migrated tree, the assignees are already on the mission. The suite pins this with the reappearing-strategies seam test.
  **Context**: `scripts/test-workflow-scripts.mjs` testMigrateStrategies.
