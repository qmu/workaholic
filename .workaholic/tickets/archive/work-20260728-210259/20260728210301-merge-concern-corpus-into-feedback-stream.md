...
Development completed as planned. The live corpus (7 active concerns incl. the PR #98 extraction, plus the archive) migrated in-repo: `list-open-concerns.sh` reports the same 7 open records from `.workaholic/feedbacks/`, the archive survives as `closed:`-stamped records, and `concerns/` is gone from tree, allowlist, and rules table in this change.

### Discovered Insights

- **Insight**: A superseding record is itself a `kind: concern` record, so the open-set rule needed **three** exclusions, not two: closed-stamped, superseded, and *superseding* (`supersedes` non-empty) — a resolution event is not an open concern. The first implementation missed the third and the resolution round-trip test caught the resolved pair reappearing as open.
  **Context**: `plugins/workaholic/skills/feedback/scripts/list-open-concerns.sh`.
- **Insight**: The envelope-compatibility decision (keep `active_count`/`my_lane_count`/`owner_counts` keys) let `/report`'s prose and the expected-count fail-loud contract survive the merger without edits — only the script paths in the SKILL changed. Contract-stable rewires are dramatically cheaper than schema changes.
  **Context**: `feedback/scripts/list-open-concerns.sh` vs the retired `report/scripts/list-active-deferred-concerns.sh`.
- **Insight**: A python closure captured per-loop (`get` over `fm_text`) late-binds to the last iteration — the first `list-open-concerns.sh` returned every record with the final file's fields. Parse frontmatter eagerly into dicts inside file loops; the same pattern exists safely in the retired lister for the same reason.
  **Context**: `list-open-concerns.sh` initial defect, caught by the in-repo demo before commit.
