...
Development completed as planned. Added the dated live-drift addendum to `workaholify/reference/routines.md` (attended-session `RemoteTrigger` presence, both live routine records' empty `cron_expression` and stale prompts), reworded the unqualified absence claims in `workaholify/SKILL.md` (§*What a routine can be triggered by* and the `render-setup-sheet.sh` description), `commands/setup-routines.md`, `render-setup-sheet.sh`'s own rendered output text, and the `/workaholify` row in `CLAUDE.md` to name the session class the finding was measured in and point at the reference doc's scoping, rather than restating the absence as universal. `grep -rn "RemoteTrigger" plugins/workaholic CLAUDE.md` confirms every remaining hit is either already scoped (the `/setup-routines` CLAUDE.md row, `routines/implement.md`'s own-session statement) or a code comment describing the mechanism rather than asserting universal absence. `workaholify` is not part of the `outputs/workflows` bundle, so no rebuild was needed for these files.

### Discovered Insights

- **Insight**: The `/setup-routines` row in `CLAUDE.md` was already correctly scoped ("This finding is scoped to an unattended, routine-fired session... it does not by itself say what tooling a developer's own interactive session carries") before this ticket touched it.
  **Context**: Only the `/workaholify` row and the skill/command/script prose carried the unqualified version — useful to know the drift was partial, not a wholesale restatement across every surface.
