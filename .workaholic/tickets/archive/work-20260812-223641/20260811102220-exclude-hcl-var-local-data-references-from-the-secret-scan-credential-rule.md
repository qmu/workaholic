...
- Extended the shared gate table in `test-workflow-scripts.mjs`: four must-not-flag HCL reference rows (`var.`/`local.`/`data.`/`module.`) and a paired must-flag row for a literal HCL credential (the quoted-literal probe the Quality Gate names).
