...
### To create
- `plugins/standards/skills/planning/SKILL.md` — the index. Structurally a clone of the existing `plugins/standards/skills/{design,implementation,operation}/SKILL.md`: frontmatter `name: planning`, `description:` matching the 企画 pillar's framing on qmu.co.jp, `user-invocable: false`. Body has `# Planning Policies`, a paragraph describing the pillar's scope and the `policies/` mirror convention, a `## Policies` H2 with one bulleted entry per article in the form `- **[Title](policies/<slug>.md)** (japanese-name) — one-line summary`, and a `## Applying this index` H2.
- `plugins/standards/skills/planning/policies/<slug>.md` — one per article currently on qmu.co.jp under `/planning/*`. Each file follows the existing policy-article schema: frontmatter `title` / `slug` / `category: planning` / `source: https://qmu.co.jp/planning/<slug>`, body `# <Title>`, italic one-line summary, intro paragraph, `## Goal (目標)`, `## Responsibility (責務)`, `## Practices (実践)` (with `###` subsections), optionally a `### Related (関連)` subsection.

### Reference (not modified)
- `plugins/standards/skills/design/SKILL.md` — closest structural reference for the new index file.
- `plugins/standards/skills/implementation/SKILL.md` — the most populous existing index, useful as a tone reference (multiple entries, full description block).
- `plugins/standards/skills/implementation/policies/ci-cd.md` (or any other `policies/*.md`) — reference for the policy-article schema.
...
- The previous three-pillar split (commit history of `plugins/standards/skills/{design,implementation,operation}/`) — `planning` is simply the fourth sibling.

## Implementation Steps

1. Create `plugins/standards/skills/planning/SKILL.md` by copying `plugins/standards/skills/design/SKILL.md` and editing: `name`, `description`, H1 (`# Planning Policies`), the intro paragraph, the `## Policies` entries (one per file created in step 2), and the `## Applying this index` paragraph (planning policies typically apply at the **Domain** / **Stakeholder** layer of a ticket; cross-link to design / implementation / operation as appropriate).
2. For each article listed in the Key Files Reference section, create `plugins/standards/skills/planning/policies/<slug>.md` by reading the source at `qmu/qmu-co-jp/docs/planning/<slug>.md` and producing the English hard-copy mirror. Match the tone established by `commit 86a048c` (humble, trade-off-acknowledging). Tip: the existing `plugins/standards/skills/implementation/policies/*.md` files are direct mirrors of `qmu/qmu-co-jp/docs/implementation/*.md` and demonstrate the translation pattern.
...
- **`accessibility-first` collision.** `qmu/qmu-co-jp/docs/planning/accessibility-first.md` shares a slug with `qmu/qmu-co-jp/docs/implementation/accessibility-first.md`. The plugin already has `plugins/standards/skills/implementation/policies/accessibility-first.md` mirroring the implementation variant. If both variants are meant to ship, the planning one needs a distinct slug (e.g. `accessibility-first-planning.md`); if the planning variant supersedes the implementation one (or vice versa), retire the redundant copy in a separate ticket. Do not silently overwrite either side here — call out the decision in this ticket's Final Report.
...
- **Skill-merge in progress** (branch `work-20260617-000311`): the `standards` plugin is being merged into the `workaholic` plugin in a separate effort. If that merge lands before this ticket starts, create the files at the merged location (`plugins/workaholic/skills/planning/...`) instead. The structure is identical either way.
