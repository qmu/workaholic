...
- **`/feedback` has the same latent invisibility** (`plugins/workaholic/commands/feedback.md` step 4 commits without pushing, and never guards on `main`). A feedback recorded on a work branch never reaches `/propose`'s cursor, which reads records *merged to main*. This is deliberately **out of scope** — the request was tickets and missions — but the primitive built here is what a follow-up would use, and the gap should be raised rather than silently inherited.
