...
- **`/feedback` still commits without pushing and without a main guard** (`plugins/workaholic/commands/feedback.md`). A feedback recorded on a work branch never reaches `/propose`'s cursor, which reads records merged to `main`. Out of scope here — the request was tickets and missions — but it is the same defect and the primitive now exists to fix it.
