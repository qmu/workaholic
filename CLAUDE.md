...
- **Sources** (write to `todo/`): `/ticket` (human-directed, with discovery) and `/mission`'s Creation Interrogation, which emits a mission's whole ordered ticket set in one pass. `/propose` feeds them upstream by registering draft missions from the feedback stream.
