...
plugins/                 # Plugin source directory
  workaholic/            # The single plugin (no dependencies; skills exposed cross-agent)
    ...
    commands/            # ticket, request, drive, commit, propose, fb, report, ship, mission, catch, explain, workaholify (Claude-only; ignored by other agents). `fb.md` is the feedback command — abbreviated because Claude Code ships a built-in `/feedback`; only the trigger is short, the artifact/skill/corpus stay "feedback"
    hooks/               # ticket validation (validate-ticket.sh, PostToolUse Write|Edit — frontmatter, location, the mandatory `## Policies` / `## Quality Gate` body sections, a resolvable `mission:` relation, and the remaining-only lint on `resume-*` tickets, all on `todo/<user>/` only) + mission validation (validate-mission.sh, PostToolUse Write|Edit — nothing required at the scaffold moment and never on a `draft`; derived-owner/Experience/Acceptance floor once `status: approved` (a legacy `drive_authorized: true` stamp still triggers it during the transition window); archive/ never retro-blocked) + story/trip OKF-`type` floor (validate-story.sh / validate-trip.sh, PostToolUse Write|Edit — a NEW write must carry `type: Story` / an allowed trip `type`; `trips/` has no writer since 2026-07-28, so validate-trip.sh now only guards a hand-authored edit to that legacy tree; history is grandfathered by git-tracked-ness so predating artifacts are never retro-blocked; the release-note writer already emits its `type`, so it needs no hook) + feedback validation (validate-feedback.sh, PostToolUse Write|Edit — schema floor on NEW `.workaholic/feedbacks/` writes: filename pattern, `type: Feedback`, `kind`/`source` enums; tracked history grandfathered) + structural move guard (guard-ticket-structure.sh, PreToolUse Bash — blocks non-canonical ticket moves like done/ or todo/<user>/archive/) + always-on policy lens (policy-lens.sh) + always-on mission lens (mission-lens.sh, UserPromptSubmit + Stop) + generated policy-index.md
...
- **Claim** — `drive/scripts/claim.sh mission <slug>` / `claim.sh batch <ticket-file>...`: a `Claim <unit-id>` commit on a fresh `work-*` branch cut from `origin/main` by the standard creator, stamping `claim: <branch>` into the claimed artifacts' frontmatter **in the worktree checkout**, pushed immediately. The stamp is branch-only — `main` never shows a claim, and the runner's main checkout stays clean between ticks (which the `/propose` batch depends on). Never prompts.
- **Reader** — `drive/scripts/list-claims.sh`: pure read, emits `{fetched, stale_hours, base, claims: [{unit, branch, artifacts, last_commit_at, stale}]}`. Claims come from git refs, but a survey's **artifacts** come from the working tree — so `/drive` fast-forwards before surveying and `plan-units.sh` reports `current` either way (J3). `claim.sh` no longer tolerates a mission absent from the claiming checkout: it refuses `mission_missing`, because after J1 absence means a wrong slug or a stale checkout, not an artifact hiding on an unmerged branch.
- **Release** — a **merge** releases by definition (the commits reach the base, so the claim leaves the unmerged set with no bookkeeping); discarding an unfinished unit is the explicit `drive/scripts/release-claim.sh <unit-id>`, which tears the worktree down *before* deleting the remote branch so a refused teardown never publishes "this unit is free" over unpushed work.
- **Staleness is reported, never auto-broken** — past `WORKAHOLIC_CLAIM_STALE_HOURS` (default 24) a claim is marked `stale: true` and nothing acts on it; reclaiming is a human/dispatcher decision.
- **Worktree lifecycle (I6)** — worktrees are **claim-born and ship-torn**. `claim.sh` creates `.worktrees/<unit-id>/`; ship or `release-claim.sh` removes it. **`/mission close` keeps only the archive move** — a lingering worktree is an in-flight or stale *claim*, which the reader surfaces and a human decides about, so a bookkeeping action no longer doubles as a destructive one. `cleanup-mission-worktree.sh` is unchanged and still the sanctioned cleaner.
- **The claim is the only creator of a branch or a worktree** (decision J1, 2026-07-30 — the completion of I6). Artifact *creation* publishes to `main` instead of cutting a ref: a source writes into a **publish tree** and pushes the commit, so the artifact is visible to every runner, machine, and fresh clone the moment it exists. `create.sh` and `create-mission-worktree.sh` are claim-side creators only.

**The publish tree** (`plugins/workaholic/skills/branching/SKILL.md`, decision J2) is a checkout of `origin/main` at the fixed, git-ignored `.publish/` on the fixed local branch `publish-main`, opened → written → committed-and-pushed → closed by `open-publish-tree.sh` / `publish-tree-commit.sh` / `close-publish-tree.sh`. It exists because publication must not depend on, or disturb, the caller's checkout: `/propose` can guard on "on `main`, clean tree, else abort" because it is a headless batch, but a developer types `/ticket` and `/mission` mid-work on a dirty branch. The caller's branch, staged set, untracked set, and file contents are left byte-identical; the commit goes through `commit.sh` (so it inherits the subject gate and the trailers, which is why the branch is named rather than detached) and is pushed as `publish-main:main`, so **the branch name never leaves the machine**. It is deliberately not `work-*` — that prefix is the claim vocabulary `claims_scan` keys on. **A publish tree is not a claim worktree**: it holds no unit, is never pushed as a branch, and is disposable at any moment; and neither `open` nor `close` will destroy recoverable state (both refuse a dirty publish tree, and `close` also refuses a clean one whose `publish-main` still carries unpushed commits — the state a `diverged` publish leaves behind). Publishing to `main` is necessary but not sufficient: a runner whose local `main` trails `origin/main` would still survey a stale queue, so `/drive` fast-forwards first through `sync-main.sh` (decision J3).
...
`/request` (`commands/request.md` + `skills/request/`) is the outlet the confinement needs: `resolve-target.sh` resolves the target and reports its **visibility** (public vs private is a different decision, and the developer must see which they are making), and `submit-request.sh` is the only sanctioned writer of a cross-repo artifact. Between them sits the step that actually does the work — **the developer confirms the destination and the exact body, verbatim, in one confirmation that cannot be skipped** (the destination is folded in, but the verbatim body is the surviving gate). `submit-request.sh`'s checks are mechanics (empty body, malformed filename, this repo's own name or path); they are not assurance. The test suite asserts this directly: the four real leaked sentences from the incident pass through the script *without complaint*, because none of them names this repo. That is not a bug to fix — it is the measured reason the human gate exists, and `request/SKILL.md` §1 explains why no matcher can replace it.
