...
- **A repeated delivery incident is retired on live end-to-end evidence only**: `transport/scripts/verify-live-proof.sh` is the one gate — `closure_eligible: true` only for evidence matching the declaration's `declared_digest`, workspace and channel, with the declared sender verified, every declared operation `proved`, and one root/reply/reaction read back. Configuration presence, a `post_reply` alone and an `operations_unsatisfied` reading are not delivery. `transport/scripts/emit-live-proof-evidence.sh` writes the evidence template without credentials and never writes `proved: true` (`available` carries the capability reading; an unmakeable describe is `available: null`); it performs no Slack act. Contract: `skills/transport/SKILL.md`, *Proving the declared route before an incident is retired*.
