...
| `/mission ["<title>" \| "<instruction about an existing mission>" \| close <slug>]` | Create a mission (the **overnight-executable execution plan of a strategy** — a bounded, information-rich batch of tickets; longevity lives above in the strategy — creating one resolves the **strategy** it executes (infer / create-on-the-spot / ask only when genuinely ambiguous), interrogates the developer to a drive-ready state, then spins up a dedicated `.worktrees/<slug>/` worktree holding the mission statement and the **whole** ordered ticket set it emitted), **replan** an existing active one (which also resolves the strategy link for a legacy/thin mission that lacks one) (a free-form instruction referencing it — no subcommand — re-enters the interrogation scoped to what changed, applies the delta, and emits delta tickets; also how a `carried` successor or a thin hand-authored mission gets fleshed out), show the **developer-centric roadmap** (bare `/mission` — full treatment with next step and changelog movement for **your** and unclaimed active missions from `list.sh`'s computed `relation` partition, one-liners for colleagues' and archived ones; the `summary` mode is retired into this view), or close one (achieved / abandoned / **carried** — done as framed, remainder inherited by a successor mission) into `missions/archive/` and tear down its worktree. When a mission's **direction changes** mid-flight (a different class of issue surfaces, the remaining criteria turn contradictory or moot, or the remainder belongs to another active mission), **reorganize-and-carry is the encouraged answer** — replan to drop the now-moot criteria (never grind to check them; the write-time floor is untouched), then `carried` into a fresh or existing (`--successor <slug>` merge) successor — rather than grinding to `achieved` or `abandoned` |
...
- **Ownership is the gate:** a mission's owners are its own plural `assignees` (`mission-owners.sh` — mission `assignees` first, strategy transition fallback, legacy `assignee`; see the mission skill's *Ownership* section). Because each git worktree checks out its own `.workaholic/`, the lens that fires in a worktree reflects the missions owned by whoever is working that tree.
