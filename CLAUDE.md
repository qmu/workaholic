...
- **Plugin boundary**: invoke skills by their loaded `workaholic:` namespace; never read global/marketplace installs on disk, never guess a namespace (`drivin`, `trippin`, `core`, `standards`, `work` are obsolete). If a skill is missing from context, ask which plugins are loaded.
...
| `/setup-routines [repo]` | Detects a `RemoteTrigger`-family tool first. **Exposed** (interactive sessions): list the account's routines, diff each against its template (name/prompt/model/`cron_expression`/`autofix_on_pr_create`/connectors), apply create/update to converge, report per-routine changes; no questions. **Absent** (routine-fired sessions): render copy-paste setup sheets (`render-setup-sheet.sh --all`), report the preconditions (Slack channel probe — `checked: false` is never "does not exist"; web bootstrap), and state what cannot be verified. The account-management surface (digest gate, drift/fleet reports) stays retired. |
