...
| `/monitor`                       | Run your missions in parallel: a developer-confirmed pre-flight (mission set, current position incl. unmerged work, unattended-drive eligibility, cross-mission interference), then one autonomous drive per mission `.worktrees/<slug>/` worktree, looping bounded waves until every mission's completion conditions are met or only explicitly deferred escalations remain. Blockers and escalations are **pushed to the developer as one-at-a-time decisions** (create/replan/claim/worktree), never just reported — an interactive run never emits its terminal `ok` over a decision it did not ask; final output line `ok`/`pending`, so a caller-side loop (e.g. `/goal /monitor ok`) can wait on it. The orchestrating agent is a **non-blocking dispatcher**: it interprets, lightly investigates, and directs — leaves run in the background while it keeps taking instructions and decisions, and each driven mission's declared dev environment is booted at dispatch inside its own worktree on its allocated ports (stopped at run end when the run started it). Only `drive_authorized` missions run unattended. PRs stay with `/report`/`/ship`. Claude-Code-only, like `/trip` |
