...
`/trip` and `/drive` converge on the **ticket** as the unit of work. A trip's Planning phase ends with a **Decomposition gate** (trip-protocol Planning Step 5) where the Constructor decomposes the agreed design into tickets under `.workaholic/tickets/todo/<user>/` (each carrying the mandatory `## Policies` and a **Trip Origin** link back to its `.workaholic/trips/<name>/designs/` rationale); the Coding phase then **drives that ticket queue** one ticket at a time, with the three-agent QA (Constructor implements → Architect reviews → Planner E2E) standing in for `/drive`'s developer approval gate and `drive/archive.sh` archiving each ticket. So `trips/` holds the *why*, `tickets/` the *what*, and `/report`/`/ship` treat a trip exactly like a drive.
