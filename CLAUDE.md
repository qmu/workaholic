...
| `/monitor`                       | Run your missions in parallel: a developer-confirmed pre-flight (mission set, current position incl. unmerged work, unattended-drive eligibility, cross-mission interference), then one autonomous drive per mission `.worktrees/<slug>/` worktree, looping bounded waves until every mission's completion conditions are met or only escalation-blocked items remain — final output line `ok`/`pending`, so a caller-side loop (e.g. `/goal /monitor ok`) can wait on it. Only `drive_authorized` missions run unattended; ineligible ones route to `/mission` replan. PRs stay with `/report`/`/ship`. Claude-Code-only, like `/trip` |
