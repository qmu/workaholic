...
The bare `/mission` view's **full tier** deliberately keeps the **lower** bar — assignee alone (`list.sh`'s `relation`: `mine`/`unassigned`) — so an unfilled `0/0` mission is still visible on demand. An always-on nudge and a view you asked for should not share a threshold; the divergence is intentional, not drift. (The old `/mission summary` mode carried this bar before it was folded into the bare view, 2026-07-22; `summary.sh` remains as the gate's canonical statement.) The **assignee** gate itself is shared: both read "not somebody else's", so an unassigned mission surfaces in each (with an empty `assignee`; the lens and the bare view mark it unclaimed). Unassigned missions were previously invisible in both — an exact email match excluded them for everybody, since an absent field matches no email that can exist.
