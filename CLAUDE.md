...
| `/feedback [<content>]`          | Register one **immutable** feedback record into `.workaholic/feedbacks/` — a design conclusion, an instruction, a development-born concern, or customer material (`kind`: insight/instruction/concern/material/answer; `source`: meeting/slack/discussion) — via the feedback skill's `create.sh` (the only sanctioned writer; resolution is a new record naming the old one via `supersedes`, never an edit) |
...
| `/monitor`                       | Run your missions in parallel, front-loaded then unattended: a developer-confirmed pre-flight over **all** your assigned missions by default — **no "which to drive" prompt** — led by a whole-roadmap progress headline (aggregate checked/total + next step across every assigned mission), current position incl. unmerged work, unattended-drive eligibility, cross-mission interference, and a reevaluation that **auto-applies mechanical replans silently** and surfaces only genuine **design** rulings. Every foreseeable escalation (create/replan-design/claim/worktree/authorization) is resolved in **one up-front blocking batch** — the run's only interaction point, pushed one decision at a time before any leaf spawns. Then it runs **long and unattended**: one leaf per mission `.worktrees/<slug>/` worktree owning the **whole** of that worktree's work — replan *application* (delta tickets, body sections, authorizing commit) included, not just the drive — looping bounded waves until every mission's completion conditions are met or only escalation-blocked items remain; **after dispatch no `AskUserQuestion` fires** — a mid-run item the pre-flight did not foresee is **deferred and recorded** for the morning review (once), never asked. An incomplete mission whose **direction changed** this run (a new class of issue, contradictory/moot criteria, or a remainder belonging to another mission) is flagged in the final report for **reorganize-and-carry** (replan to drop the now-moot criteria, then `carried` into a fresh or `--successor <slug>` mission) rather than presented as a plain `pending` to grind — a **recommendation only**: the unattended run never reorganizes, closes, or replans a mission itself, and the terminal token stays honest. The orchestrating agent is a **non-blocking dispatcher**: it interprets, lightly investigates, and directs — never editing inside a worktree itself (replan bookkeeping included) — leaves run in the background while it actively tunes the wave size down when interference or resource load argues against full parallelism, and each driven mission's declared dev environment is booted at dispatch inside its own worktree on its allocated ports (stopped at run end when the run started it). It **accumulates each mission's agent-hours** (each leaf's dispatch→completion wall-clock, summed and recorded once per mission per run-id via `record-run-hours.sh`) into `actual_hours`, and the final report shows predicted vs actual per mission. The final report also **states, per driven mission, completed-as-planned or the categorized reason** (fixed vocabulary: `complete`/`escalation-blocked`/`deferred`/`gate-failed`/`wave-exhausted`) and writes a dated `## Reflection` entry (`append-reflection.sh` — `blocked`/`leaked questions`/`front-load next:`, records causes not pending decisions) that the next `/mission` Creation Interrogation reads back (`list-reflections.sh`). For each **genuinely complete** mission (per `status.sh`) it **auto-creates the PR** (dispatcher-owned, sequential, reusing `/report`'s story+`create-or-update.sh` seam per worktree; warn-tier scan findings recorded in the PR body, no prompt) — a PR-creation failure is its own report item and never changes a completion count or the terminal token. **Merge stays `/ship`** (deploy-gated; auto-merge rejected), and worktree teardown rides the **claim** — ship, or an explicit `release-claim.sh` — never `/mission close`. The terminal signal is **honest and derived from `status.sh`**: final line `ok` **only** on genuine completion of every driven mission, else `pending`, preceded by an `N/M complete, K escalation-blocked` reconciliation — so a caller-side loop (e.g. `/goal /monitor ok`) waits on a real result, not a self-graded token. Front-loading deliberately supersedes the during-run push model of `edf246a4` for the mission run; the honest terminal resolves the `/monitor` half of `20260719000021` (the `/goal`-gate side stays separate). Only **approved** missions (`status: approved`) run unattended. Claude-Code-only, like `/trip` |
