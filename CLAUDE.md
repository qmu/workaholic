...
Two Claude Code Web routines per repository, from the templates in `skills/workaholify/routines/`: **`[Propose]`** (`fb.md`, hourly `15 * * * *`) and **`[Implement]`** (`implement.md`, hourly `30 * * * *`). The API's minimum interval is one hour; a bare `:00` minute is rewritten to server jitter, so explicit non-zero minutes are used. Both declare `autofix_on_pr_create: true` (stored at `job_config.ccr.session_context.autofix_on_pr_create`). A template is a thin pointer: the prompt carries only the command, the post formats it authorizes, and the environment — every rule stays in the skill that owns it (`workaholic:drive`, `workaholic:notify`, `rules/`). `[Implement]` names one format, its finish line; `[Propose]` names two since 2026-08-14 — the finish line plus the **description root** it replies into when the thread lookup finds none, byte-identical to `notify/reference/notifications.md`'s copy and pinned against drift by `test-workflow-scripts.mjs`. The whole chain is drillable on demand — `scripts/e2e/loop-drill.sh` (seed / status / reset / verify-propose / verify-implement / verify-plan), operator tooling outside the plugin because it assumes the server's full `gh` and `qfs`; the operator procedure, the cron race windows and the failure-reason→file blame tables are `docs/loop-drill-runbook.md`. A routine cannot subscribe to a repository event (the API's trigger surface is `cron_expression` / `run_once_at` / API token only); `[Propose]`'s tick therefore discovers its own asks (the propose skill's *Clock-fired discovery*). Neither trigger narrows to a person: the data decides ownership (`owned_by_other` at `/implement`'s survey, `not_mine` at `/propose`'s input). Do not reintroduce a third routine.

**The deployment-plan refresh rides `[Implement]`'s tick; it is not a third routine** (2026-08-13, resolving the Open Decision ticket `20260813123903-keep-the-deploy-plan-draft-current-periodically` carried). The named surface is **`[Implement]`, hourly at `30 * * * *`**: every `auto` unit it ships reaches `/ship`'s drafting phase, which refreshes that unit's plan inside the unit's own pull request. Nothing commits to `main` outside a merge, and the refresh is idempotent (`draft-deploy-plan.sh` → `changed: false`), so a tick with nothing to change writes nothing; a degraded read is reported and skipped, never half-written. **The stated limit**: a repository whose units are all `review` sees the plan refreshed only when an `auto` unit ships, so "kept up to date by managed agents" holds to exactly that extent and no further — written down rather than quietly dropped. The two alternatives were both refused on standing grounds: a third routine reverses a written decision and costs every consuming repository a setup step, and an hourly agent rewriting a document on `main` is a new class of unattended write for a tree whose conflicts are resolved append-only. The refresh is verifiable on demand with `sh scripts/e2e/loop-drill.sh verify-plan` rather than by waiting for a tick.
