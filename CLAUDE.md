...
The bare `/mission` view's **full tier** deliberately keeps the **lower** bar — ownership alone (`list.sh`'s `relation`: `mine`/`unassigned`) — so an unfilled `0/0` mission is still visible on demand. An always-on nudge and a view you asked for should not share a threshold; the divergence is intentional, not drift. (The old `/mission summary` mode carried this bar before it was folded into the bare view, 2026-07-22; `summary.sh` remains as the gate's canonical statement.) The **ownership** gate itself is shared and derived through `mission-owners.sh`: both read "not somebody else's", so an unowned mission surfaces in each (the lens and the bare view mark it unclaimed). Ownership moved to the strategy layer (2026-07-24): a mission's owner is the `assignees` of the strategy it executes, with a legacy fallback to the mission's own `assignee` so nothing predating the field is orphaned; "mine" means the caller is among the owners, since a strategy can be co-owned.
...
- **Ownership is the gate:** a mission's owner is **derived** from the strategy it executes (`mission-owners.sh` — the strategy's `assignees`, legacy `assignee` fallback; see the mission skill's *Ownership* section). Because each git worktree checks out its own `.workaholic/`, the lens that fires in a worktree reflects the missions owned by whoever is working that tree.
