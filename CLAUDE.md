...
plugins/                 # Plugin source directory
  workaholic/            # The single plugin (no dependencies; skills exposed cross-agent)
    ...
    hooks/               # ticket validation (validate-ticket.sh, PostToolUse Write|Edit — frontmatter, location, the mandatory `## Policies` / `## Quality Gate` body sections, a resolvable `mission:` relation, and the remaining-only lint on `resume-*` tickets, all on `todo/<user>/` only) + mission validation (validate-mission.sh, PostToolUse Write|Edit — nothing required at the scaffold moment; derived-owner/strategy/Experience/Acceptance floor once `drive_authorized: true`; archive/ never retro-blocked) + story/trip OKF-`type` floor (validate-story.sh / validate-trip.sh, PostToolUse Write|Edit — a NEW write must carry `type: Story` / an allowed trip `type`; history is grandfathered by git-tracked-ness so predating artifacts are never retro-blocked; the release-note writer already emits its `type`, so it needs no hook) + feedback validation (validate-feedback.sh, PostToolUse Write|Edit — schema floor on NEW `.workaholic/feedbacks/` writes: filename pattern, `type: Feedback`, `kind`/`source` enums; tracked history grandfathered) + structural move guard (guard-ticket-structure.sh, PreToolUse Bash — blocks non-canonical ticket moves like done/ or todo/<user>/archive/) + always-on policy lens (policy-lens.sh) + always-on mission lens (mission-lens.sh, UserPromptSubmit + Stop) + generated policy-index.md
...
| `/propose`                       | The **headless proposal batch** (also the 15-minute cron entry — `docs/proposal-loop-runbook.md`): on `main`, read feedback records merged since the runner-local `proposal-cursor`, judge them against the propose skill's conservative bar, and either stay **silent** (a valid outcome; cursor advances) or register **draft missions** (`status: draft`, unowned, never authorized, `feedback:`-linked for traceability and dedup) committed and pushed to main, each announced to Slack via the bot notifier (graceful no-op without a token). Never prompts; the cursor advances only after a successful push |
