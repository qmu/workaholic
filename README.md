...
| `/propose` | Headless proposal batch: read feedback newly merged to main (a cursor stored as a pushed ref, so a fresh container reads where the last run left off) and either stay silent or register **draft** missions with `feedback:` traceability, pushed to main and announced to Slack (bot token optional). The 15-minute cron entry of the loop-engineering model |
