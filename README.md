...
The development workflows we use at [qmu](https://github.com/qmu), written down so our coding agents can run them the way we do. **AI agents are the primary users**: the plugin is installed for the agents that consume it — the local loops `/spawn-loops` starts (tmux sessions turning `/propose` → `/specificate` and `/implement` every five minutes, `/moderate` every thirty, each in its own clone), the Claude Code Web routines that are their fallback, and sessions, attended or unattended, running the workflow skills. The **developer is the operator**: approving the pull requests those agents open, configuring the routines, and stepping in when a run defers a decision — and every command also runs by hand, the supported secondary path. The workflows are tuned to how we work, so they may not fit everyone, and they'll keep changing as we do. We keep this public so the people we work with can share the same base.
...
| `/spawn-loops` | **Start the local loops**: one tmux session per loop — `propose` every 5 minutes, `implement` every 5, `moderate` every 30 — each a Claude Code session running `/loop` in its own clone under `~/.workaholic/loops/<repo>/<loop>`, with permission prompts off because an unattended run never waits for a person. Idempotent; a machine without tmux is told the Web routines are its fallback |
