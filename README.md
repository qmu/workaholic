...
Releases publish selected changes, including completed parts of an unfinished mission or
standalone tickets. Remaining work stays open; publishing a version does not itself activate
unfinished production behavior. Deployment confirmation is specific to each target.

The development workflows we use at [qmu](https://github.com/qmu), written down so our coding agents can run them the way we do. **AI agents are the primary users**: the plugin is installed for the agents that consume it — the local loop (a native parent in a capable session, or a same-chat schedule where available), the Claude Code Web routines that are its fallback, and sessions, attended or unattended, running the workflow skills. The **developer is the operator**: approving the pull requests those agents open, configuring routines and schedules, and stepping in when a run defers a decision — and every command also runs by hand, the supported secondary path. The workflows are tuned to how we work, so they may not fit everyone, and they'll keep changing as we do. We keep this public so the people we work with can share the same base.
...
| `/work [interval]` | Start the lightweight development loop (`workaholic:watch`): a script watches the declared Slack channel with no model in the loop (default every two minutes; `1m` means one minute) and prints a line only for a new human message; the session wakes on that line, answers it or tickets the request, and starts `/implement` in the background. A quiet channel costs no tokens. Stop it by stopping the background watcher. The earlier coordinator loop remains as `/infinite-development` for Codex and CLI supervisors. |
| `/infinite-development` | One unattended tick: honor conversation hold/stop first, reconcile native child receipts, observe the declared Slack binding, then dispatch due roles within load and capacity limits. Idle without a terminal result is unknown, not finished. Role cadence advances from durable completion receipts; unreadable claimable work is not zero. The channel is drained to the end of its delta inside the tick and every root and mention it discovers is remembered, so a partly-read channel reports `observation_incomplete` rather than quiet; and a unit waiting on somebody else's merge is a **per-unit** wait that never ends the loop — independent work keeps being dispatched. Report verified results in the initiating chat and keep implementation, merge, deployment and notification outcomes separate. |
...
| `/propose` | **The loop's own ask.** Read your own active strategies, judge the single **evolutionary move** that brings the nearest one closer to its aim before its `target_date`, and open that judgment as a GitHub issue assigned to you — the surface `/specificate`'s hourly tick already reads, so the next tick ingests it and `/implement` drives it. It runs as the `propose` subagent of an `/infinite-development` tick, followed by `/specificate` in the same subagent. **A pure read of the repository**: no file, no commit, no branch, no pull request, no merge, no deployment, and nothing posted to Slack; the only write is the issue. **It is not housekeeping** — a drifted document or a missing test is `/moderate`'s work. The issue names an experience and an ordered ticket set; one ticket is a valid bounded hypothesis, while `/specificate` creates a mission only when two or more tickets share a mid-term outcome. `/propose` plans; `/specificate` writes. Every proposal declares one move (`depth`, `breadth` or `contraction`) and must name what it is chosen against, and a tick that cannot name a move opens nothing and says so. Continuation is governed by strategy ownership, date, lineage, open work and repository WIP; channel silence and `arrived`/`quiescent` are evidence rather than automatic stops, and `観察中` permits observation work. Every refusal is reported by name, and a tick that cannot read its own open proposals proposes nothing at all |
...
| `/moderate` | **The maintenance tick.** `/specificate` turns asks into work and `/implement` drives it; nothing keeps the space *around* them tidy, or says how the repository's development is actually going. One hourly, unattended run walks the ordered registry in `moderate/scripts/steps.json` — the executable registry, rather than any prose count, owns the number of steps and their order, and a count written here goes stale the first time a step is added (it read **33** while the registry held 36). It opens the tick log, sweeps inbound and repository health surfaces, acts where a proof licenses repair, routes findings through feedback/ticket/mission seams, and asks named people up to five questions. A finding it cannot turn into a question stays in the local tick log. It never prompts, never merges a pull request, and never pushes into a branch the claim protocol owns. What it reads about the base is a **reading, not a verdict**: a re-run can turn a red check green, so nothing here re-runs a check, reverts a commit, blocks a merge or gates anything — quality stays gated at the `release/*` QA window. |
...
`watch-slack.sh --until-event` runs as a background command and watches the declared Slack channel
with no model in the loop (default every two minutes; `/work 1m` means one minute), exiting only
when a new human message arrives. The session wakes on that exit, answers it or tickets the request, and starts
...
Native coordinators limit workers across roles to `WORKAHOLIC_MAX_WORKERS` (default 2), within
the runtime's available capacity. Deferred roles remain due. Unreadable claimable work yields
one fallback runner when capacity permits; it never means an empty queue. The Claude command
also carries the unattended decision, evidence-before-diagnosis and gate-before-write rules,
