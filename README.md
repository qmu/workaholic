...
| `/work [interval]` | Start the shared development loop at the requested interval (default five minutes; `1m` means one minute). Select from the session’s capabilities: a native parent reports in this chat while children work, then a same-chat schedule, then the external supervisor. The supervisor’s `--status` reads its state; its output does not return to the initiating chat. The supervisor records recovery from a retired plugin path, or stops explicitly when no complete replacement is available. Never run two clocks against one repository. |
