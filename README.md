...
| `/monitor` | Run your missions in parallel: a confirmed pre-flight (mission set, position, unattended-drive eligibility, interference), then one autonomous drive per mission worktree, looping until every mission completes or only explicitly deferred escalations remain — blockers are pushed to you as one-at-a-time decisions, never just reported (final line `ok`/`pending` for caller-side loops) |
