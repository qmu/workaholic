...
**The planning hierarchy is four layers, each at its own granularity — and no layer restates a lower one's detail:** a **strategy** is long-lived *direction* with no end (why these missions are launched); a **mission** is a strategy's *overnight-executable execution plan* — a bounded batch of tickets an agent fleet drives in a night; a **ticket** is one drive-able change; a **commit** is one normalized change kept to a reviewable size. This is the day/night model we work in: the developer spends the **day planning** (interrogating each mission to question-free, drive-ready readiness), and **coding agents execute at night** in parallel, so the morning starts with reviewable results — open PRs, honest completion reports, and recorded reflections.
...
| `/mission` | Plan the **overnight-executable execution plan of a strategy** — a bounded batch of tickets an agent fleet drives in a night (longevity lives above, in the strategy): create one (resolves the strategy it executes, interrogates you to a drive-ready state, then spins up a dedicated `.worktrees/<slug>/` worktree holding the mission statement and the **whole** ordered ticket set it emitted), list missions with computed progress, show just **your** assigned active missions (`/mission summary`), or close one (achieved / abandoned / **carried** — done as framed, with the unmet criteria carried into a successor mission) into the archive area (tearing down its worktree) |
| `/monitor` | Run your missions in parallel: a confirmed pre-flight (mission set, position, unattended-drive eligibility, interference), then one leaf per mission worktree owning the whole of that worktree's work — a mission needing a replan has it *applied* by its own leaf, not the main agent, then drives — while the main agent stays a thin dispatcher (only the prompts a leaf cannot issue, tuning wave size down for interference/resource load), looping until every mission completes or only explicitly deferred escalations remain — blockers are pushed to you as one-at-a-time decisions, never just reported. It accumulates each mission's agent-hours, writes a per-mission `## Reflection` the next planning reads back, and **auto-creates a PR for each genuinely complete mission** (merge stays `/ship`); final line `ok`/`pending` for caller-side loops |
...
| `strategies/active/<slug>/strategy.md` | `strategy` skill (`create.sh`) | Long-lived **direction** (戦略) with no completion conditions, sitting above missions; a mission records the one strategy it executes as `strategy: <slug>`, and the missions rolling up to a strategy are computed, never stored | committed | n/a — outlives every mission | `retire.sh` flips `status` to `retired` and moves the dir to `strategies/archive/<slug>/` (rare — a changed strategy is rewritten in place) |
| `missions/active/<slug>/mission.md` | `/mission` | Overnight-executable execution plan of a strategy, bundling many tickets: goal, scope, acceptance checklist (progress = checked/total), `predicted_hours`/`actual_hours` (predicted at creation from the archived trend, actual accumulated by `/monitor`), append-only changelog | committed, updated as related work lands | n/a — outlives any branch | `/mission close` flips `status` to `achieved` or `abandoned` and moves the dir to `missions/archive/<slug>/` (file and changelog preserved) |
