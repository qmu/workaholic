...
- **The executor** drains `todo/ → archive/`: **`/drive`**, and there is one of it. It takes work in PR-units, drives each in its own claim worktree, and routes it by merge policy — the same run whether you typed it or a cron tick did. Approval is not asked per ticket; it was already given where the work was decided (a mission's approval, or the ticket's own merge policy at creation), and the qualitative review relocates to the PR. (`/trip` still ships as a second executor in this build; its retirement is its own change.)
