...
| `/propose` | **The routine that supplies the loop's own ask.** Read your own active strategies, judge the single **evolutionary move** that brings the nearest one closer to its aim before its `target_date`, and open that judgment as a GitHub issue assigned to you — the surface `/specificate`'s hourly tick already reads, so the next tick ingests it and `/implement` drives it. Three routines then turn one loop, one turn per hour. **A pure read of the repository**: no file, no commit, no branch, no pull request, no merge, no deployment, and nothing posted to Slack; the only write is the issue. **It is not housekeeping** — a drifted document or a missing test is `/moderate`'s work. Every proposal declares one move (`depth`, `breadth` or `contraction`) and must name what it is chosen against; a tick that cannot name a move opens nothing and says so. It is the first unattended routine here to drop the standing *when unsure, record only* bar, and what replaces the bar is mechanical: one proposal per strategy in flight at a time, one per tick across all of them, nothing for a strategy that is closed, not yours, past its date, or that cites no feedback record for its work to be traced back through — every refusal reported by name, and a tick that cannot read its own open proposals proposing nothing at all |
