...
| `/drive`   | Implement queued tickets one by one (add "night" for an autonomous overnight run with a morning report). Work is taken through the **claim protocol**: a runner claims a PR-unit — one approved mission, or one batch of backlog tickets — on a pushed branch, and every runner reads the claims in flight from the unmerged remote branches, so two runners (or two machines) never pick the same work. There is no lock file and no server; the repository itself is the coordination medium |
