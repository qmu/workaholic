...
| `/propose` | Headless proposal batch: read the feedback merged to main since the cursor — a pushed ref, so a fresh container reads where the last run left off — plus the repository's own state, and either stay silent or propose a **mission with its ticket set** behind a pull request whose merge is the approval, announced to Slack (bot token optional). Runs unattended every 15 minutes as the `[Propose]` routine |
