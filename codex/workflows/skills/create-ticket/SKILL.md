...
Run `bash branching/scripts/check.sh`. If `on_main` is true, create a topic branch via `branching/scripts/create.sh` and record the returned branch name as `branch_created` for the output JSON. Trip branches (`trip/*`) return `on_main: false` and skip branch creation; tickets go to `.workaholic/tickets/todo/` regardless of branch type.
