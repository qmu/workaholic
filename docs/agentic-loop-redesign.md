...
recordのサイズ上限は宣言した値であって、引数の上限ではありません（2026-09-19, ticket `20260919120809`）。recordはイベントごとに全体を書き直すため、`state.sh` は古い値から新しい値を組み立てます。その受け渡しを `argv` で行うと Linux の `MAX_ARG_STRLEN`（`32 × PAGE_SIZE`。ここでは131,072バイト。`ARG_MAX` ではありません）に当たり、recordが育つと書込が通らなくなり、さらに読取すらできなくなります。しかも失敗の出方が三つとも違い、どれも黙っていました。組立が空になって shape 検査が `state_invalid` を返す（原因の名前が違う）、成功結果のレンダリングが **ディスクへ書いた後** に失敗して revision だけ進んだまま stdout が空のまま exit 2 になる（呼び手は失敗と読んで再試行し、自分の書込に対して `revision_conflict` を受け取る）、読取のレンダリングが同様に失敗して `coordinator.sh` が空ファイルを読み、単なる読取を含む **すべての** coordinator イベントが無言で返る、の三つです。実測では、24 workerの散文で `.data` が129,906バイトに達し、`finish` が失敗し、receiptが `running` のまま残りました。

対処は三つです。上限のない値はすべて **ファイル** で渡します（`--slurpfile` / `--rawfile`、`lib/result.sh` に追加した `runtime_json_result_file`。既存の `runtime_json_result` は署名を変えません）。成功結果は **recordを置く前に** レンダリングするので、この seam で起きうる拒否はrecordをバイト等価のまま残します。そして語をそれぞれ一つの意味に割り当てます。**`state_too_large`** は宣言した上限（`RECORD_MAX_BYTES`、1 MiB）超過で、観測サイズと上限を伴い、**書込のみ**を止めます（既存recordの読取は常に通すので、上限を超えた過去のrecordも救出できます）。**`state_write_failed`** は組立・staging・書込のどれかが実行できなかった場合で、その step を名乗ります。**`state_unreadable`** は既存の語で、レンダリングできなかった場合も覆い、`coordinator.sh` が無言で終わる代わりに返す語でもあります。`state_invalid` は shape 検査だけに絞り、広げません。どの新語も `revision_conflict` とは衝突しません。

coordinatorがworkerごとに保存する量も `lib/coordinator.jq` で抑えます。`result.report` を1200文字に切り詰めて印を付け、`executed` / `outcome` / `reason` はそのまま残し（`valid_result`、finish-log の要約、`codex-loop.sh` の worker 読取が使う値です）、`report` は文字列のままにします（`valid_result` の要件）。抑えるのは **保存する側** だけで、finish を記録した tick は `completed[]` でそのworkerの **全文** を渡すので、チャンネルへの投稿は切り詰めたコピーにはなりません。再送された finish は切り詰めた形と比較するため、クラッシュ後の再実行は `duplicate_result` のままです。保存済みのworkerは **書込のたびに** 抑え直すので、変更後の最初の書込が過去の行を再バウンドします。これが移行そのものです（storeは `.git/workaholic/runtime/` 配下でclone固有であり、pull requestが移行を運べないため、prune pass も migration script も作りません）。
