...
| approved missions exist but nothing is claimed | the mission has an empty `## Acceptance`, or its tickets live in an unmerged worktree | check `plan-units.sh`'s `excluded[]` — every drop states its reason |
