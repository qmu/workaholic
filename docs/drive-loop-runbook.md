...
| a claim sits in flight whose work you know already landed | it was recovered by hand onto another branch, re-applied, or reverted and redone — the *commits* never merged even though the *content* did | `list-claims.sh` reports it `resume_reason: superseded`, the survey excludes it `claimed_superseded`, and it does not forbid `ok`. Nothing acts on it: close its pull request (if any) and delete the branch by hand when you are satisfied. It answers for **batch** units; a mission claim keeps today's reading |
...
| a ticket sits in `todo/` excluded `operator_deferred`, and every tick names it again | somebody wrote `deferred: <why>` into its frontmatter — the **operator's own hold** (2026-09-21). The ticket stays queued and counted; it is simply offered to nobody | nothing is wrong. Read the reason off the ticket, and **delete the line** when the hold is over: that is the only re-offer path, and the next survey offers it again with no other act. Nothing in the loop writes or clears the key. To park a ticket *out* of the queue instead, use `status: icebox` — it leaves the queue walk entirely and comes back through `promote-icebox.sh` |
| a ticket is excluded `deferral_unreadable`, and the tick still spawns a runner | its `deferred:` line is malformed — a bare `deferred:` with no reason, or a `[`/`{` collection. That is neither a deferral nor a pass, so the survey holds the ticket out of the offer and `claimable-units.sh` answers `readable: false`, which falls back to **one** runner rather than zero | repair the line (name the reason) or remove it. The write floor refuses both shapes on the next edit, so this only reaches a survey through a hand edit or a file written by an older plugin copy |
