...
| `the_tick_completes_the_log_move` | a repository whose day files are still tracked on the base stays that way after a tick | **the second deliberately broken row.** The off-main design used to reach a repository only when a person ran `/workaholify` — measured as twelve days of silent hourly accumulation on a consuming repository, with `step-open-log.sh` reporting `ok` throughout. The tick now finishes the move itself; when it stops, this goes red |
| `the_move_leaves_the_working_log_on_disk` | the move deletes the tick's own working log | the day files are untracked, never removed: `.gitignore` carries the directory and the files are what this tick is writing |
