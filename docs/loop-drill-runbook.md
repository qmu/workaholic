...
| `direction_health_keys` | the step's **group** keys are not exactly `direction-dormant:<slug>+<slug>` and `direction-overdue:<slug>`, or the per-subject keys in `directions` are not one per direction | `step-direction-health.sh`'s `groups` block (2026-09-03: one question per **reading**, keyed on the sorted subject set). The group key is what the asked-once ledger keys on, so a drifted one is a question asked twice or never; the per-subject keys are the composer's input, and losing them would trade five questions for a message that cannot name what it is about |
| `direction_health_group_names_every_subject` | the dormant group folds two directions into one key without carrying both subjects, or names no assignee | a group is **one reply about several subjects**; a key that silences a direction its message never names is the collapse trading five questions for a lost one |
...
| `arrival_question_keys` | the two arrived directions are not folded into one `direction-arrived:<slug>+<slug>` **group** key beside the existing two readings, or `directions` no longer carries a per-subject key each | `step-direction-health.sh`'s `groups` and `subjects` blocks; the group key is what the asked-once ledger keys on and the subject keys are what the composer writes from |
| `arrival_group_key_carries_the_subject_set` | the arrived group keys on the kind alone, or names fewer subjects than it stands for | a bare `direction-arrived` would be asked **once ever**, so a direction arriving next week would never be asked about at all — the asked-once gate turned into a silence (the stated cost is the opposite one: a fourth direction re-asks the group once) |
| `arrival_body_describes_the_reading` | the body does not state the reading, breaks the 25-word ceiling, or asserts the direction is finished | the same block — the reading is a **candidate**, never a verdict, because "Reached when" is prose no script reads. What landed and the date ride the **heading** (`workaholic:notify` rule 5), which this row still checks; the body carries the reading and the act. Its anchor is the arrived **subject**, not the sentence's opening words: the first version keyed a `sed` on `Everything attributed` and, when that sentence was restated, reported `0 words` — a row that had quietly stopped reading anything |
...
| `expiry_question_keys` | the two directions inside the window are not folded into one `direction-expiring:<slug>+<slug>` **group** key, the live direction draws one, or `directions` no longer carries a per-subject key each | `step-direction-health.sh`'s `groups` block and its subject `select`; the group key is what the asked-once ledger keys on, so a drifted one is a question asked twice or never |
| `expiry_group_names_every_subject` | the expiring group carries fewer subjects than its key stands for | one message names every direction it silences; a group that named one of two would report itself asked while half its subjects went unmentioned |
...
| `base_health_step_reports_the_red_base` | the step does not hand the check-in a `🔴 Blocked` report carrying the attributed merge, or it opens a question key | the `base-red:<commit>` question is **retired** (2026-09-03): a question is held by `quiet_hours` because it addresses a named person, and a red base asks the operator to decide nothing while the loop merges into it all night. When this row is red the announcement is back to waiting for morning, or the repository is announcing one fact twice |
| `base_health_signature_is_the_failing_checks` | two red commits failing the same suite do not carry one byte-identical signature, the signature carries a sha, or the later tick stops reporting | the cool-down's own rule (`workaholic:notify`). A key that changed every commit would suppress nothing on a base this loop merges onto every half hour; a step that went silent on the second tick would be the retired asked-once gate leaking back in |
