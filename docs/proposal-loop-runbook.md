...
| two pull requests implement one defect | **not the in-flight gate.** Measured 2026-09-01 over the five reported pairs (#801/#802/#790 vs #800/#789, #520 vs #519, #466 vs #465): `work_waiting` and `open_proposal` govern whether an *ask* is originated, and none of these asks was machine-originated. Three pairs are a **person implementing the finding by hand while the loop's claim was live** (#789 merged at 19:37 while `work-20260831-184331` was heartbeating), one pair is **two `/specificate` runs 19 seconds apart on one human ask** (the dedup that now reads unmerged branches did not exist yet), and one reported pair is **not a duplicate at all** (#519 and #520 drove different tickets) | nothing in `/propose` to change. Tell the loop before implementing a filed finding by hand: `/mission-close <slug> abandoned`, or drive the mission. A hand-merged fix leaves the mission `active` with its tickets queued, and the loop re-offers it every hour |
