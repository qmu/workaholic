...
8. 見つけた記録を保存して終わります。tick のログ自体はコミットされず、書いたチェックアウトに残ります。tick が書いたフィードバック記録だけを `[Record]` の PR に載せて main へ届けます（`persist-log.sh --record`、PR は release scan が通れば自動マージ。main へ直接コミットすることはありません）。
...
- 各 tick の最後に `persist-log.sh --record` がフィードバック記録を `[Record]` PR に載せて main へ届けます（直接コミットはしません。ログ自体はコミットされません）
