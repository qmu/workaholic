...
description: "Dividing each package's interior by domain and into three responsibilities — model, service, and dependency implementation — so that external libraries, I/O, and entry point wiring cannot corrupt the domain's own vocabulary."
resource: https://qmu.co.jp/implementation/anti-corruption-structure
tags:
  - implementation
...
These three responsibilities are the same across languages. Each language's coding standards handle how to map them to that language's idiomatic file names, package boundaries, and module boundaries. The TypeScript specifics are established by [Coding Standards (TypeScript)](/implementation/coding-standards.md) as `model`, `service`, and `dependency` under `src/<Domain>/`; the Go specifics are established by [Coding Standards (Golang)](/implementation/golang-coding-standards.md) as package and file boundaries. In Rust, the same three responsibilities would be mapped to `mod` and crate boundaries. Directory names themselves may follow language idioms; what is aligned is not the names but the three responsibilities and dependency directions.

### Separate entry points as thin shells

HTTP routers, CLIs, queue workers, and other entry points are not placed in the package expressing the domain. Instead they are placed outside as thin shells that only call the domain's public procedures and format results. Startup and wiring are contained in this shell; decision logic remains in the domain's sections. Being able to start the same domain procedure from a different entry point in the same way is evidence that the separation has held. The rationale for this separation is handled by [Domain Layer Separation](/implementation/domain-layer-separation.md).
...
When uncertain about how to divide sections, draw boundaries along units that can be rebuilt as a whole, not for the convenience of adding features. This arrangement — dependency implementations coexisting inside domain sections — differs from the widely-known approach of placing implementation details away from the domain. Acknowledging that that separation works well in its context, at our scale and team structure we prioritize being able to rebuild at the domain unit level over purity of separation. Containing dependencies within boundaries and managing them within `packages/` in the same repository supports the replaceability envisioned in [Sacrificial Architecture](/design/sacrificial-architecture.md).

### Related

The repository-wide arrangement across packages is handled by [Standard Directory Structure](/implementation/directory-structure.md). The language-specific application of the anti-corruption structure defined in this policy is handled by [Coding Standards (TypeScript)](/implementation/coding-standards.md) and [Coding Standards (Golang)](/implementation/golang-coding-standards.md) respectively. The rationale for separating entry points and external dependencies from the domain is handled by [Domain Layer Separation](/implementation/domain-layer-separation.md), and the rationale for aligning sections to rebuildable units is handled by [Sacrificial Architecture](/design/sacrificial-architecture.md). This policy gathers these principles together, language-agnostically, as a structure that prevents external concerns from corrupting the domain.
