...
The principles for internal package arrangement — dividing by domain and separating the inside into the three responsibilities of model, service, and dependency implementation — and the definition of what each responsibility handles are established language-agnostically by [Anti-Corruption Structure](/implementation/anti-corruption-structure.md). This section shows how to express those three responsibilities using TypeScript directories. A package expressing a domain first divides directly under `src/` by domain; each domain directory is set up one per aggregate root, named with a pronounceable PascalCase word such as `Order` or `Inventory`. Under each domain directory, exactly three are placed: `model/`, `service/`, and `dependency/`.

```
src/
  Order/                 Order domain (one per aggregate root · PascalCase)
    model/               Data types and functions on those types
    service/             A series of procedures exposed to the outside
    dependency/          Implementations containing external dependencies
  Inventory/             Inventory domain
  Billing/               Billing domain
```

The definition of what each directory is responsible for is deferred to [Anti-Corruption Structure](/implementation/anti-corruption-structure.md); here TypeScript-specific details are added. The base library that `model/` may depend on is plgg, our rich-typing reference implementation. `dependency/` contains external libraries such as ORMs and exposes only primitive types, plgg types, and model types at the boundary. HTTP routers, CLIs, and other entry points are not placed in this package; a separate package handles them as thin shells that import and call `service/`'s exported functions (see [Domain Layer Separation](/implementation/domain-layer-separation.md)). What is exposed outside the package is limited to `service` and `model`; `dependency` is not published. Where to place tests, package-specific scripts, and documentation follows the package-level layout in [Standard Directory Structure](/implementation/directory-structure.md).

### Related

The reasoning behind avoiding `any`, `as`, non-null assertions, and `@ts-ignore`, and the approach of rich typing with `unknown` boundaries, `as const`, `satisfies`, and nominal types, is in [Preferring Rich Typing](/implementation/type-driven-design.md). The reasoning behind avoiding `class`, `throw`, and `==` and defaulting to immutable data, return-type failure expression, and declarative comparison is in [Preferring Declarative Code](/implementation/functional-programming.md). The language-agnostic principles for internal package arrangement are in [Anti-Corruption Structure](/implementation/anti-corruption-structure.md), and the repository-wide layout is in [Standard Directory Structure](/implementation/directory-structure.md). This policy sets out these principles as concrete TypeScript feature choices, showing which defaults to reach for.
