...
Entry points such as HTTP, CLI, and queue are kept as thin shells that call functions in the domain layer and return the result, while the domain layer itself is written in the language's standard vocabulary with rich types (see [Preferring Rich Typing](/implementation/type-driven-design.md)). The more this form is maintained, the wider the range in which static checking and unit testing can support the correctness of domain decisions.
