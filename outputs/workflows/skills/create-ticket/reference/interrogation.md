...
Ask, once per `/ticket` run: may this work merge automatically once done and verified, or must a human review the PR? Write the answer into every ticket of the run as `merge_policy: auto | review` (decision G5). One question, two options: *auto — merge on green deploy evidence* / *review — stop at the PR for a human*. This is one of the few genuinely unrecommendable forks — the answer depends on how much the developer trusts this particular change to land unattended, information you do not hold. Do not derive it from the ticket's kind or size.
