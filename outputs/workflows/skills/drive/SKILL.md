...
**Under `/implement` only, post the unit's 🟠 start** into its feedback items' threads — resolve them with `bash drive/scripts/unit-feedback-stems.sh <artifact>...`; when a merged PR started the run, read the thread back first with `bash branching/scripts/read-notify-target.sh <pr-number-or-url>` (`found: false` / `absent` is the documented fallback to the stems). **An attended `/drive` run posts nothing to Slack, at any step** — the operator is present. Post rules: [reference/routing.md](reference/routing.md); `claim.sh`'s own one-line bot notice is a separate token-gated surface, never load-bearing.
