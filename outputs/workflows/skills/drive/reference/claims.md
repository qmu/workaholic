...
| `stranded` | judgement | The unit's tickets reached the base by another route **and** the branch still holds content that is on no other ref (2026-08-31). `superseded`'s sibling, not a narrowing of it: the two states call for different next actions — retire the claim versus tell a person their work is stranded — and one word answering both is what offered branches carrying ~300 lines for deletion. `resumable: false`, because a takeover would re-drive nothing and push a `Resume` commit onto a branch nobody has ruled on. A consumer may only **report** or **ask about** it: nothing merges, closes, deletes, reverts, retires or claims on it, and no automatic recovery path exists. Its one enumerated **reporting** consumer is `/moderate`'s `step-stranded-branches.sh`, which asks the claim holder what should become of the work and names the files. |
