...
parallel workers (preloading `drive`, returning
...
bash ../gather/scripts/migrate-ticket-states.sh
```
...
- **If the ticket carries a `mission:` relation, read the quality gate of EVERY mission it
  names** — `bash ../mission/scripts/gate.sh <mission-slug>` once per
...
- **Load the policy lens first** (when the standards plugin is installed): both entry points
  preload `design` / `implementation` / `operation`. Open every
