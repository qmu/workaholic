...
# Last backstop, and it lives in ONE place. `check-outbound-body.sh` carries the whole
# rule — this repo's own name as an identifier, its `owner/name` remote form, every form
# of its clone URL, and its absolute path — because the same body is now also sent across
# the boundary as a GitHub issue by the /fb crossing flow, and a second copy of a rule
# whose false-positive rate is a usability property is a rule that drifts. Its refusal
# JSON is re-emitted verbatim: the caller reads one message shape whichever route it took.
check="$(sh "${SCRIPT_DIR}/check-outbound-body.sh" "$body_file")"
case "$check" in
    '{"ok": true}'*) ;;
    *) printf '%s\n' "$check"; exit 0 ;;
esac
