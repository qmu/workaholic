...
```bash
bash mission/scripts/close.sh <mission-slug-or-file> <achieved|abandoned> [date]
```

End a mission — the only sanctioned way. Flips `status` to `achieved` or `abandoned`, appends the closing changelog line (`mission achieved` / `mission abandoned`, artifact `mission.md`) through `append-changelog.sh` so the transition itself becomes history (`design` / `history-structures`), moves the mission dir into `archive/`, refreshes the OKF indexes, and git-stages. Idempotent: re-closing with the same status is a no-op (`{closed: false, reason: "already_closed"}`); re-closing with the other status flips it in place and appends its own line. Emits `{closed, slug, status, path}` JSON.
