...
A **mission** is a first-class knowledge artifact: an **optional, epic-equivalent grouping of two or more tickets** — a fairly immediate developer request, interrogated to question-free drive-readiness, that bundles the ordered ticket set an agent fleet drives through together (typically overnight). The lower bound is part of the definition, not a lint on it (*Granularity → The ticket floor*): a grouping of fewer than two things is not a grouping. It carries the information-rich statement of *what this batch of tickets accomplishes* and *how far it has come*, with a machine-readable web of relations to the tickets, stories, and concerns that advance it. The mission is bounded, not long-lived — and **never a required parent**: the ticket stays the first-class standalone unit, and `/ticket` → `/drive` with no mission is a fully sanctioned path. Long-lived *direction* accretes in the feedback stream (`feedback`; `docs/loop-engineering-workflow.md`, decisions B3/B5).
...
- **mission** — an **optional, epic-equivalent grouping**: a bounded batch of **two or more** tickets with acceptance criteria and an append-only changelog. It answers *what does this batch of tickets accomplish together*. A mission finishes; the stream persists. A **single** unit of work is a **ticket** and stays one — wrapping it adds a board, a progress fraction and a close decision to something that already had its tracking.
...
| **mission** | *what does this batch of tickets accomplish together* (optional — a ticket needs no mission) | hours of agent time; **two or more tickets**, a fairly immediate request interrogated to question-free readiness | the Creation Interrogation, floored at two tickets (below) |
...
**A mission is created with two or more tickets, or it is not a mission** (`.workaholic/feedbacks/20260804173526-a-mission-is-created-with-two-or-more-tickets-or-it-is-not-a-mission.md`). This is the granularity table's bottom edge made checkable: without it a ticketless mission is a feedback record on the roadmap and a one-ticket mission is a ticket with a progress bar — the three artifact kinds are only distinguishable if the middle one has a lower bound. **What counts, and when:** tickets carrying this mission in their `mission:` relation, **present in the same publication commit as the `mission.md`** — counted at the **publish seam**, where the mission and its tickets become one artifact, and nowhere else. **Not at the write of `mission.md`**, and the reason is ordering: a `PostToolUse` hook fires *before* the interrogation has emitted anything, so a write-time floor would refuse the normal authoring order. Same argument that put acceptance-link stamping at the emitting seam (`reference/schema.md`, *The link contract*), written after 37 items across six missions were found unlinked — the cost of checking where the data does not yet exist is already measured here. **Do not put the floor in `validate-mission.sh`.** **Exactly two, and one ticket is refused rather than warned** — one ticket has nothing to group, and a warning would preserve the ambiguity the rule removes. **The one archived instance is pre-rule history, not an open violation.** `drop-the-draft-gate-and-make-drive-own-its-worktree-from-refreshed-main` (archived, 1 ticket) predates the floor and shipped fine; the claim was never that it did harm, only that "mission" and "ticket" must not both name it. It is **not** edited and **not** re-opened — history is never rewritten (`design` / `history-structures`) — and an auditor reading `missions/archive/` against this rule should stop at that sentence rather than filing it. **The floor applies to creation, so only the active area is auditable against it**: `check-floor.sh <slug>` over `missions/active/*` is the whole audit, and it was clean when the rule shipped (2026-08-04, four active missions at 4/4/2/4 tickets).

**A refusal names the alternative**, or the author simply retries it: a bare direction is a **feedback record**, a single unit of work is a **plain ticket** (`implementation` / `observability`).

**What a carried close does: `--successor-title` is refused; a carry must name an existing mission** (`--successor <slug>`), because a minted successor arrives with zero tickets and so violates the floor by construction — which is how the live violation of 2026-08-04 reached `main`. The refusal was sequenced behind the inheritance fix (a carry had to keep transferring its remainder before the mint route could go) and **both have landed**: `close.sh` refuses `--successor-title` by name, and the mint branch is deleted rather than gated — there is one carry route, and it appends through `lib/acceptance.sh`. The rejected alternatives and the accepted cost are in [`reference/schema.md`](reference/schema.md), *The ticket floor*. **Where the check lives, stated once:** the publish seam every creation path shares — the Creation Interrogation and `/propose`. The count is `queue-size.sh`'s `meets_floor`; the **verdict and its refusal text** are `check-floor.sh`, which every seam calls rather than re-deriving either. Two seams deliberately do not call it, each recording why in its own header: `create.sh` (the scaffold predates its tickets) and `close.sh` (a categorical refusal, nothing to count).
...
[`reference/command-flows.md`](reference/command-flows.md) carries the orchestration the thin `/mission` and `/mission-close` commands run: how the argument routes (the reference-judgment criteria), the create and replan publish-tree step sequences, the bare-`/mission` planning session, and the close flow — including where each the agent's selection prompt is issued (always by the loading command/main agent, `[<project label>]`-prefixed via `bash gather/scripts/project-label.sh`; a subagent cannot ask).
...
**The floor is checked here, before the publish commit, and stated during the interrogation rather than sprung at the end** — run `check-floor.sh <slug>`; a non-zero exit means the mission is **not published**, and its `alternative` is what to tell the author instead (a feedback record for a bare direction, a plain ticket for a single unit of work). Say the floor out loud while the decomposition is still being decided, so an author who has one unit of work learns it in time to write the right artifact rather than at the publish (*Granularity → The ticket floor*). **The split cap does not apply to a mission — a deliberate, scoped exception.** `create-ticket` §4 caps a split at "2–4 discrete tickets", which is right for one request that turns out to be several. A mission is the opposite case: an execution plan that bundles *many* tickets by definition, and "a complete set to drive through one by one" is the requirement. Capping it at 4 would force either an incomplete plan or a fake ticket boundary. A mission decomposition answers to its own rule: **one ticket per genuinely separable unit of work, however many that is**. The cap still applies to `/ticket` itself; this exception is mission-scoped and stated here so it is not a silent violation.
...
| `queue-size.sh` | How many tickets name the mission (`todo` / `archive` / `total`) — the single counter both drivability floors read, and where the ticket floor's `meets_floor` verdict is computed |
| `check-floor.sh` | The ticket floor as an act-on-able verdict: exit 1 plus the refusal's `alternative` when a mission is under the floor. Every creation seam calls it; none re-derives the count or the refusal text |
...
2. **Carry** the still-valid remainder with `close.sh … carried --successor <existing-slug>`, carrying the unchecked criteria into an existing active mission. For a genuinely new heading, **create that mission first** (the interrogation emits its ticket set) and then carry into it — `--successor-title` is refused by the ticket floor. Merging needs no new operation: `--successor <slug>` **appends** the predecessor's unmet items (markers intact) to the named mission's `## Acceptance`, leaving that mission's own Goal, gates and items untouched, and records lineage both directions — `carried_from` on the successor (a list under a second carry) plus a changelog line on each side. It is idempotent by item text, so a re-run adds nothing. It does **not** import the predecessor's Goal or gate fields — no carry route does, since the minting one was deleted with `--successor-title` (`reference/schema.md`, *The ticket floor*); a mission that needs a different framing is a new mission, not a carry.
