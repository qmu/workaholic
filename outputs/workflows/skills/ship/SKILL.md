...
`/ship` is called two ways: by a developer in a session, and by the unified `/drive` run for a PR-unit whose effective merge policy is `auto` (`drive`, *Unified Run* §6). The second caller **cannot prompt at all** — there is nobody to answer.

**This is a routing table over the existing seams, not a second flow.** Every step below runs exactly as written; only what happens *at each the agent's selection prompt* differs, and in every case the unattended answer is the conservative one — the ship stops or hands the unit back to the PR path, never proceeds on an assumption:
...
- `{caught_up:true, branch_up_to_date:bool, resolved_append_only:[...]}` — merged cleanly (`branch_up_to_date:true` means merging `origin/<base>` into the work branch was a no-op; it is a claim about the **work branch**, not about the local `main` ref, which catch-up does not touch). Proceed. `resolved_append_only` names any `.workaholic/` artifact that *did* conflict and was reconciled by keeping both sides (see below) — report it, but it needs no action.
- `{caught_up:false, conflict:true, conflict_class:"mechanical", conflicted_files:[...], append_only_files:[...]}` — every **remaining** conflict is a version/lockstep manifest (`.claude-plugin/marketplace.json`, either `plugin.json`) or under `outputs/`. **The agent reconciles this itself, as routine ship hygiene — do not ask the developer whether to reconcile.** Merge `origin/main`, resolve the manifest conflicts (take `main`'s side of the version, then re-bump to the next free version), regenerate `outputs/` (`node scripts/build-plugins/build.mjs`), re-run the pre-merge proof (build/verify/tests), and continue. This is the same reconcile-and-re-bump the version-collision guard requires (see below). Any path in `append_only_files` is one the script verified is a pure append on both sides — **keep both sides' lines** when you redo the merge; there is nothing to judge there.
- `{caught_up:false, conflict:true, conflict_class:"content", conflicted_files:[...], append_only_files:[...]}` — a non-manifest path conflicts and needs human judgment. **Halt the Ship Flow and ask the developer to resolve it** before continuing.

**Append-only `.workaholic/` conflicts are reconciled here, not escalated.** The tree is full of artifacts both branches routinely extend — a mission's `## Changelog`, the OKF `index.md` files — and when two units land concurrently both append to the same tail. The resolution is unambiguous (keep both sides), so the script performs it and completes the merge rather than reporting a decision nobody has to make. It cost real merges before: on a measured ship, seven files conflicted, five were manifests already classed mechanical, and the two remaining `.workaholic/` append-only files alone forced the whole catch-up to `content` — one prompt interactively, but a demotion of an `auto` unit to the PR path in the unattended `/drive` run, recurring on **every** concurrent pair.

The **scope is ruled by path and the resolution by shape**, and both halves are load-bearing (the reasoning is recorded in this skill's own `scripts/lib/append-only.sh`): only `.workaholic/` indexes and mission files are eligible at all, and within them a conflict is reconciled only when *both* sides purely inserted lines. A ticked `## Acceptance` box or a reordered regenerated index modifies a line, so it stays `content` and still halts. A merged changelog is ordered by its lines' leading dates when every line carries one, else left in merge order.
...
2. **Catch up with `main`** (mandatory — before any deploy): Run `bash ship/scripts/catchup-main.sh "<base-branch>"` so what you deploy equals what will merge, and apply the §2-5b version-collision guard. On `caught_up:true`, report any `resolved_append_only` paths — the script reconciled those `.workaholic/` appends itself and there is nothing to decide. On `caught_up:false`, branch on `conflict_class`: **`mechanical`** — the agent reconciles it itself as routine ship hygiene (merge `origin/main`, resolve the version/lockstep manifests, re-bump the version past the collision, keep **both sides** of every path named in `append_only_files`, regenerate `outputs/`, re-run the pre-merge proof), no user prompt; **`content`** — halt and ask the user to resolve the conflict before continuing. Never present reconciliation itself as an optional choice.
2b. **Branch-safety scan gate** (PRE-MERGE, blocks the merge exactly like the §1-4 gate — the branch staying open is the rollback). Scan the reconciled branch diff and map it to a decision:
   ...
   - **`overridable: true`** (only `size`/`leak` findings) — report the findings and ask the developer via the agent's selection prompt (command level, §1-3) to either **fix the diff and re-run** (remove the bloat / the leaked term, or scope the `.workaholic/leak-denylist` entry for a false positive), or **accept the risk and override**. On override, record it as accepted-risk evidence so the decision stays auditable — `bash ship/scripts/record-evidence.sh "<branch>" "release-scan" "override" "<findings overridden: rules + files>" "bypassed"` — then continue to Deploy. Re-run the scan after any fix.
...
6. **Merge PR** (LAST — only after a passing confirmation): Run `bash ship/scripts/merge-pr.sh "<pr-number>" [<base-branch>]`. On failure, inform user and stop. Capture the merge `commit_hash`.

   **`commit_hash` is the commit this merge produced ON THE BASE** — not the work branch's head, which is reported separately as `branch_head`. The distinction is load-bearing because step 7 tags `commit_hash`: whenever the base advanced since the branch's last catch-up, the branch head's tree is not what landed, and under a squash or rebase merge it is not even an ancestor of it. `commit_hash_source` says how the value was obtained — `pr_merge_commit` (the PR's own `mergeCommit`, authoritative for every merge strategy), `base_tip` (**derived**: the fetched `origin/<base>` tip, which is the merge commit only while nobody else merged in between), or `unresolved`. Report the source alongside the hash rather than assuming it; on `base_tip`, verify with `git merge-base --is-ancestor <commit_hash> origin/<base>` before tagging. **Ruling (2026-08-04):** this field used to be `git rev-parse --short HEAD`, which inside a claim worktree is the branch head — observed on two consecutive ships, both recovered by hand with `gh pr view --json mergeCommit` before the release tag could be placed correctly.
...
1. **Cut the window.** `bash branching/scripts/cut-release-branch.sh [base]` mints `release/YYYYMMDD-HHMMSS` at the base tip and pushes it. It carries no commits of its own and never checks itself out. On a refusal (`no_origin`, `origin_unreachable`, `base_unresolved`, `branch_collision`, `push_failed`) **stop**: nothing has changed and there is no window.
...
```bash
bash ship/scripts/record-release-cut.sh "<release-branch>" [since-ref] [base]
bash ship/scripts/confirm-release.sh "<release-branch>" "<method>" "<non-secret result>" "pass"|"fail" [tag] [base]
```
...
Promotion issues **no the agent's selection prompt anywhere**, like the rest of the unattended path. Every outcome is a reported JSON refusal or a recorded status; a decision the flow cannot make is a stop with its reason named, never a question.
