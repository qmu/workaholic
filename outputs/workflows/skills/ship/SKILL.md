...
- **A doubtful tick posts, with the count suppressed** — the Open Decision the ticket carried, resolved 2026-08-18. Staying silent is indistinguishable from a quiet repository, which is the same invisible degradation this change exists to remove; publishing a bare count endorses a number the reader has just flagged. So the post gate becomes `actionable` **or** `doubtful`, still AND'd with the digest search, and the line names the degradation instead of a number (`notify`, *the repository tick's one line*).
...
```bash
bash ship/scripts/run-note-cadence.sh [--write] [--target <slug>] [--dry-run] [--force] [base]
```
