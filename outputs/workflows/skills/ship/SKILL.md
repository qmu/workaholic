...
- `{caught_up:true, branch_up_to_date:bool, resolved_append_only:[...]}` — merged cleanly (`branch_up_to_date:true` means merging `origin/<base>` into the work branch was a no-op; it is a claim about the **work branch**, not about the local `main` ref, which catch-up does not touch). Proceed. `resolved_append_only` names any `.workaholic/` file whose conflict the script resolved itself (see below); it is empty when the merge had no conflict at all.
- `{caught_up:false, conflict:true, conflict_class:"mechanical", conflicted_files:[...], append_only_files:[...]}` — every **remaining** conflict is a version/lockstep manifest (`.claude-plugin/marketplace.json`, either `plugin.json`) or under `outputs/`. **The agent reconciles this itself, as routine ship hygiene — do not ask the developer whether to reconcile.** Merge `origin/main`, resolve the manifest conflicts (take `main`'s side of the version, then re-bump to the next free version), regenerate `outputs/` (`node scripts/build-plugins/build.mjs`), re-run the pre-merge proof (build/verify/tests), and continue. This is the same reconcile-and-re-bump the version-collision guard requires (see below). `conflicted_files` lists everything that conflicted; `append_only_files` lists the subset the script had already resolved and the abort discarded — resolve those by **keeping both sides**, no judgement involved.
- `{caught_up:false, conflict:true, conflict_class:"content", conflicted_files:[...], append_only_files:[...]}` — a remaining path is neither a manifest nor an append-only record, so it needs human judgment. **Halt the Ship Flow and ask the developer to resolve it** before continuing.
- `{caught_up:false, conflict:false, reason:"merge_failed"}` — the merge never started (e.g. local changes would be overwritten), so there are no unmerged paths to classify. Not a conflict: fix the working tree and re-run. Do **not** read this as a mechanical conflict over an empty file list.

**Append-only `.workaholic/` conflicts are resolved by the script, not classed.** A mission's `## Changelog` and the OKF `index.md` files are extended by every seam that archives a ticket, reports a story or records a run, so two branches that each land work always conflict at the same tail — with one correct resolution, *keep both*. The script proves the shape (the merge base is an exact line-prefix of both sides — nothing modified, nothing removed) and, when it holds for a path under `.workaholic/`, merges the two tails itself: sorted when every appended line is date-led, in merge order otherwise, with a line both sides appended verbatim kept once. Anything it cannot prove falls through to the classifier above. Classing these as `content` used to cost the **merge** rather than a prompt — `/drive`'s unattended routing demotes an `auto` unit to the PR path — and it recurred on every concurrent pair of units.
...
2. **Catch up with `main`** (mandatory — before any deploy): Run `bash ship/scripts/catchup-main.sh "<base-branch>"` so what you deploy equals what will merge, and apply the §2-5b version-collision guard. On `caught_up:false`, branch on `conflict_class`: **`mechanical`** — the agent reconciles it itself as routine ship hygiene (merge `origin/main`, resolve the version/lockstep manifests plus any `append_only_files` by keeping both sides, re-bump the version past the collision, regenerate `outputs/`, re-run the pre-merge proof), no user prompt; **`content`** — halt and ask the user to resolve the conflict before continuing. A `conflict:false, reason:"merge_failed"` answer is neither: the merge never started, so fix the working tree and re-run. Never present reconciliation itself as an optional choice.
...
6. **Merge PR** (LAST — only after a passing confirmation): Run `bash ship/scripts/merge-pr.sh "<pr-number>" [<base-branch>]`. On failure, inform user and stop. Capture the merge `commit_hash` — **the commit that landed on the base**, resolved from GitHub's own `mergeCommit` (`commit_hash_source: "pr_merge_commit"`) or, failing that, the fetched base tip (`base_tip`). `on_base` reports whether it is an ancestor of `origin/<base>`; `branch_head` carries the work branch's last commit separately. Read `commit_hash_source` before using the value in step 7: a `branch_head` source means neither resolution worked, so the value is **not** a merge commit and must not be tagged.
...
7. **Publish GitHub Release** (post-merge, gated on a successful merge): Run `bash ship/scripts/publish-release.sh "<branch>" "<merge-commit>" "<tag>" "<notes-file>"`. The script first checks for an existing release-publishing GitHub Actions workflow and **defers** to it (`reason:"ci_publishes"`) — do nothing in that case, CI owns releases. Otherwise it creates the release (idempotent) targeting `merge-pr.sh`'s `commit_hash` — the commit that landed on the base, which is what a release-on-tag project must tag. **Refuse to tag when step 6 reported `on_base: false` or `commit_hash_source: "branch_head"`**: the branch head is not on the base's first-parent line, so a tag placed there builds the release from a tree that never existed on the base. Derive `<tag>` from the project version (`.claude-plugin/marketplace.json` or the project's version file) when present, else the next semver after `gh release view`/the latest git tag; for an additional release on the same branch, suffix the tag to stay unique. `<notes-file>` is the note written in step 5. When CI is absent and a release will actually be created interactively, confirm via the agent's selection prompt first. Report `published`/`reason` from the JSON.
