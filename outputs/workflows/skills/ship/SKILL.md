...
5. **Record evidence and prepare merge artifacts** (PRE-MERGE): 
   ...
   - Commit the evidence-updated story and the release note to the branch so both ride into the merge: `bash ship/scripts/commit-release-note.sh "<branch>"` (commit the story update alongside — the script commits only the note). **Read its `pushed` field**: the push is best-effort and never fails the script, so on `pushed: false` the note is committed locally but is NOT on the branch the PR merges, and you must push before step 6 or the merge will not carry it.
