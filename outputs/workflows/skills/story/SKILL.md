...
Generate the story file, then create the PR — the story is the single source of truth for PR content. The `/story` command (main agent) runs this orchestration directly and spawns each leaf worker as a parallel worker Task; fan-out stays one level deep. Phase-by-phase detail: [reference/orchestration.md](reference/orchestration.md). The shape:
...
Run by the overview-writer worker: collect commits with `bash story/scripts/collect-commits.sh [base-branch]` (the structured `body` keys feed Motivation and the highlights; `category` comes from the `Category:` git trailer) and return `{overview, highlights[], motivation, journey}`. Field guidance, line limits, flowchart rules, and both JSON formats: [reference/orchestration.md](reference/orchestration.md).
