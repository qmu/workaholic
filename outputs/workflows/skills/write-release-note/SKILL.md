...
**Who assembles it.** The `ship` skill owns the mechanics: its drafting
...
rendered by `ship`'s `draft-release-note.sh` (this skill stays prose-only and
...
`ship` skill's evidence recorder — so they cannot disagree, and its refusal
