...
6. **Links**: The PR URL is provided as input (by the `core:ship` Ship Flow, from its `pre-check.sh` output). Always include it.

## Output Location

This skill runs at **ship time** (the `core:ship` Ship Flow generates the note before merging), so one branch can produce several notes across multiple ships:
