...
- Leaders (especially reliability-lead, db-lead, security-lead) receive a consistent structural context for their domain-specific work.
...
**Consuming leaders**: reliability-lead (layer taxonomy, system boundaries), db-lead (component inventory, data layer), security-lead (system boundaries, cross-cutting concerns), observability-lead (cross-cutting concerns).
...
- For each viewpoint (application, component, feature, usecase), gather context by running `bash ${CLAUDE_PLUGIN_ROOT}/skills/analyze-viewpoint/scripts/gather.sh <slug> main`.
