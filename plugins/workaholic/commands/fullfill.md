...
**On the name** (renamed from `/release-status`, 2026-08-17, for the one-word command
convention): it reports what is waiting to be *fulfilled* — the commits that have landed
on the base and not yet reached a target. The spelling is the one the ask named. The
`description:` above carries the meaning, because the name alone reads less obviously than
the one it replaced. The `[Release Status]` **routine record** kept its name — only the
command moved — since renaming it would create a duplicate routine rather than rename the
operator's existing one (`skills/workaholify/routines/release-status.md`).
