...
Run the preloaded `workaholic:moderate` skill's **The run** section end to end: one tick — `run.sh` over the steps `STEPS` registers, in order, one log line per step in `.workaholic/moderations/<UTC-day>.md`, then act on every `needs_agent` entry through the seam that step's section names, recording each as `<step>-filed`. The run's **closing act** publishes that log to the log's **own ref** (`persist-log.sh`; `refs/heads/workaholic/moderation-log`, never the base) — a routine's container is discarded, so a log left in the checkout blinds every dedup and leaves the tick with no audit trail. The persist runs **three times** a tick — the opening, the closing act, and yours after you have filed — and all three are kept: each carries a class of line the others cannot, and the objection to the count was three commits a tick on `main`, which the move answers. Finish with one report line per step, the persist's own outcome **by name**, and the counts.
