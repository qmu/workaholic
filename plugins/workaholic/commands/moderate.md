...
Run the preloaded `workaholic:moderate` skill's **The run** section end to end: one tick — `run-planned.sh` selects due steps from `steps.json`, reports every registered step in order, and writes one log line per step in `.workaholic/moderations/<UTC-day>.md`, then act on every `needs_agent` entry through the seam that step's section names, recording each as `<step>-filed`. **That log goes nowhere** — it is git-ignored and stays in this checkout, which is the checkout the next tick reads; the orphan branch that carried it between 2026-09-01 and 2026-09-03 is retired and must not be reintroduced, and neither may a notes ref, a remote store or any other place a commit could land (`persist-log.sh`'s header carries the whole record and the measurement). Carry each feedback record through `persist-log.sh --record <path>` when its filing seam creates it; a record named under `.workaholic/moderations/` is refused `log_destination_is_base` with nothing written. Finish with one report line per step, every record carry outcome **by name**, and the counts.
