...
**Watchdog every long/background handoff (skill §2) — silence is never progress.** Non-blocking must not become fire-and-forget. For any long-running or detached task the dispatcher stops actively driving: set **one bounded fallback wake-up** sized to the job's longest expected step; **on wake verify liveness actively** (read the declared artifact, run log, and process/container state) instead of inferring from a missing message; and treat prolonged silence as a **suspected fault to investigate**, never "still running". **Never disable your own oversight** — you may stop nudging a leaf, never stop watching it. Give detached jobs an explicit self-contained environment (a background process does not inherit the interactive PATH); their early exit is exactly what the watchdog catches.
...
After each wave, run `status.sh` per mission and classify: complete (gate included when declared) / still driveable / escalation-blocked. **Check a blocker earned the label (skill §3):** accept `escalation-blocked` only when the leaf recorded an attempted command with its raw output, or named one of drive §3a's two no-attempt buckets. Work deferred for being long, heavy, exclusive, or abstractly "human-only" is **still driveable** — re-dispatch it next wave with the §3a/§3b rule restated (narrowing the wave if it needs the environment to itself), never launder it into the escalation list. **A silent leaf is a fourth case (skill §3):** when the wave's bound elapses with no report, verify actively — read its declared artifact, run log, worktree `git log`, and any process it launched — then classify from that evidence (`failed` on failure evidence, continue under a fresh bound on progress evidence, failed-and-unreported otherwise). Never let a second bound elapse on the same unverified silence. The run is **unattended after dispatch** — a genuine mid-run escalation the pre-flight did not foresee is **deferred and recorded in the final report, never asked** (recorded once, not re-logged each wave). Re-drive driveable missions — at most 3 waves per invocation, and stop early on a zero-archive wave. Any plan change routes through `/mission` replan, never ad-hoc edits. **Honest terminal signal (skill §3):** emit `ok` **only** when every driven mission genuinely reached `complete` per `status.sh`; if any is incomplete or escalation-blocked, emit `pending` — escalation-blocked is `pending`, not `ok`. This makes `/monitor`'s terminal token honest (the `/monitor` half of `20260719000021`; the `/goal`-gate side stays separate).
