...
**Policy Lens**: The `hooks/policy-lens.sh` UserPromptSubmit hook injects the engineering-policy lens automatically on every `/report` run — load and apply `workaholic:planning`, `workaholic:design`, `workaholic:implementation`, and `workaholic:operation` (see the skill's `### Policy Lens` step) and use them to judge the branch's design, implementation, behavior, and operation when assessing carry-over concerns, reviewing story sections, and reporting release readiness.
