...
4. **Report it to someone who has never seen this repository before.**
   ...
   - On `checked: true`, give one short block per routine: what it does (the `template` id — `fb` turns a reported issue into a record and, when the judgment warrants it, the work it asks for, both in one PR; `merged-pr` announces a merge to `dev-<repo>`; `drive` drains the queue), its `schedule`, its `target_repo`, whether it is `enabled`, and its `status` — those four come from the live account. **`trigger` does not**: report it as *the trigger the template declares* (`trigger_source: template_declared` in every row), and state the standing note **in the report itself**: trigger wiring is not exposed by the RemoteTrigger API and cannot be read, set, or drift-checked here — confirm it in the routines UI at <https://claude.ai/code/routines>. The note is there because a routine whose live wiring does not match its declared trigger is misconfigured and only the UI can show it — measured 2026-08-06, when a `[Propose]` firing on merges read as `github-issue-assigned` in this report and only a manual UI check surfaced the mismatch. Render `template_set_version` as *the version of the template set compared against* — the account records no version on a routine, so nothing here can say which version created one.
