...
4. **Quality Gate interrogation** (skill Step 4b — **mandatory, always run**) — before writing, interrogate the developer about how the outcome's quality will be assured. **Ask decisions; derive the rest**: question only what is genuinely the developer's call (verification depth/method, scope, risk tradeoffs), issued as `AskUserQuestion`(s) at this command level and grilled until objective and checkable; acceptance criteria that follow from discovery and repo conventions are **drafted by you directly into the ticket's mandatory `## Quality Gate` section**, never posed as a select-which-apply menu. **Do not soften or skip the decision questions** on "obvious" requests — the split narrows what qualifies as a question, not whether to ask; the recorded gate is what makes the later `/drive` approval concrete.
