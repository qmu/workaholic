...
**Summary mode**: when `$ARGUMENT` is exactly `summary`, follow the skill's **Summary mode** branch (in `## Trip Command Procedure`, before "Determine execution mode") — a read-only report of every trip under `.workaholic/trips/` with its plan phase/step plus the current user's todo-queue snapshot. It launches no team and creates no worktree, branch, or artifact. Bare `/trip` (no argument) is unchanged: it still runs context-aware queue-execute.
