...
4. **Point at the routine setup sheets** (skill §5). The plugin cannot survey, create or refresh a routine — a routine's GitHub trigger is configurable in the web UI only and the API record carries no event field — so what this step does is hand the developer everything they need to do it themselves:
   ...
   Print the sheets as they come, prompts **verbatim** — what the developer pastes is what runs. Then probe the channel every template posts to:

   ```bash
   bash ${CLAUDE_PLUGIN_ROOT}/skills/workaholify/scripts/check-slack-channel.sh <repo-name>
   ```

   `checked: false` means *could not look*, never *does not exist*. Say plainly that whether the routines were actually created — and whether one has ever fired — is not answerable from here: `last_fired_at` is populated for a cron fire and absent for a GitHub-triggered one, so a routine that ran is recognised by what it produced.
...
Report what was checked, what conforms, and what (if anything) needs fixing. For the routines that includes what the plugin cannot answer: it renders the setup sheets, and the developer creates them in the web UI — nothing here reads or changes the account.
