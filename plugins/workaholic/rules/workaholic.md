...
The `missions/` tree mirrors that working-vs-archived split, keyed off the mission's single `status` axis: an **in-flight** mission (`draft`, awaiting approval, or `approved`) lives at `missions/active/<slug>/mission.md` and an **ended** one (`achieved`, `abandoned` or `carried`) at `missions/archive/<slug>/mission.md`. The mission skill's scripts own both the placement and the status — `approve.sh` flips `draft` → `approved`, `close.sh` flips to an end state and performs the move, and living migrations relocate any legacy flat `missions/<slug>/` dir and normalize a legacy `status: active` (plus its retired `drive_authorized` stamp) on the next mission-script touch. Never `mv` a mission dir or hand-edit its `status:` field.
