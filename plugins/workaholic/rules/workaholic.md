...
| `housekeeping/` | **No frontmatter at all** (the second OKF exception): a day file is `housekeeping/<YYYY-MM-DD>.md`, headed by one paragraph and then `## <tick-id>` sections whose entries read `` - `<step>`: <status> — <summary> ``. Written only through `moderate/scripts/log-append.sh` |
