...
| `moderations/`  | **The `/moderate` tick log — one file per UTC day, one section per tick, one line per step.** Holds: what each step checked, filed, skipped or could not read. Never holds: anything another area owns — a finding becomes a feedback record, work becomes a ticket, and this log only says that it did. Written through `moderate/scripts/log-append.sh` into the checkout, and by **two** producers under their own tick ids — `/moderate`'s steps and `/infinite-development`'s `loop-finish-<name>` per subagent finish (under the **coordinator's** tick id, every five minutes); append-only, and **never pruned by a machine**. **A `loop-finish-<name>` line's summary is one shape — the `{executed, outcome, reason}` object — whichever seam wrote it** (2026-09-20, ticket `20260920014751`): `runtime/scripts/coordinator.sh` always wrote it, `work/scripts/codex-loop.sh` wrote prose into the same day files, and no reader could take `.outcome` as a field (measured on this checkout: 75 such lines, 54 JSON and 21 prose, some Japanese). What a token says about **yield** is declared once in `runtime/scripts/outcome-classify.sh` — `originated` / `nothing` / `unmeasured` / `unclassified`, read per segment of a composite outcome off `.outcome` and never `.reason` — and every reader composes it. Because nothing may rewrite a line, the prose already on disk reads `unclassified` **by name** and ages out of its consumer's two-day window on its own. **A third producer, `/propose`'s `propose-open`/`propose-close`, is documented and does not exist** (2026-09-19, ticket `20260919141500`): no script writes that pair — `log-append.sh`'s only callers are `/moderate`'s own `run.sh`, `ask-question.sh` and `record-answer.sh` — so the `propose` owner reads zero entries and its two consumers are silent by construction. The owner is kept because the derivation is right; whether to build the writer or retire the pair is the operator's call. **The one reader is scoped by that ownership and answers moderation by default** (2026-09-07, ticket `20260907063154`): `log-read.sh --owner <moderate|loop|propose|all>` derives each entry's owner from the step id it already carries, per entry rather than per section, so a caller that names no owner is asking about moderation and the coordinator's own sections stop shadowing it. A reader that wants another owner names it. **It is git-ignored and committed nowhere at all** (2026-09-03): an operational log is not part of the product's history, and it is not part of any other branch's history either — it stays on disk in the checkout that wrote it, which is the checkout the next tick reads. The orphan `workaholic-log` branch that carried it between 2026-09-01 and 2026-09-03 is **retired and must not be reintroduced**; `moderate/scripts/persist-log.sh`'s header carries the whole record and the measurement. An **operational log, not an OKF knowledge artifact**: entries carry no `type:` and the area has no `index.md` |
...
**And a ticket may be HELD IN the queue rather than parked out of it** (2026-09-21, mission `hold-operator-deferred-tickets-out-of-the-offer-and-say-so`). `deferred: <why>` in a queued ticket's frontmatter is the **operator's own** hold — presence is the hold, the value is the reason, **absent means not deferred**, and no run ever writes or clears it, exactly as no run may declare its own `verification_handoff:`. The ticket stays queued: the survey counts it in `backlog_size`, offers it to nobody, and names it `operator_deferred` in `excluded[]`, which `backlog_all_excluded` counts like any other reason. **Removing the line is the only re-offer path** — no promotion script, no flag, no stored cursor. It is read through `drive/scripts/read-deferral.sh` and nowhere else; a declaration that reader could not read is neither a deferral nor a pass and is excluded `deferral_unreadable`, which one layer up is a **degraded** reading that still earns one runner.

**It is a second key rather than a reuse of `status: icebox`, and the two answer different questions.** `icebox` parks a ticket **out** of the queue — `list-todo.sh` drops it from the walk, so it is never counted and never named, which is exactly the invisibility the ask refuses (measured: a queue of one free and one iceboxed ticket answers `backlog_size: 1` with an empty `excluded[]`, byte-identical to a queue that simply holds one ticket). `deferred:` holds a ticket **in** the queue, counted and named. `status: icebox`, `promote-icebox.sh`, `list-icebox.sh` and `list-todo.sh`'s end-state filter are all byte-identical. **The stated cost**: two parking concepts now stand side by side, and the documentation has to say when to reach for which or operators will use them interchangeably.
...
**A cadence is not a `.workaholic/` field, and looking for one here is the mistake worth heading off** (2026-08-31, mission `notice-a-periodic-artifact-that-stopped-being-produced`). A repository may declare **what it expects to keep being produced, and how often**, so that `/moderate` can notice when a producer dies — the one thing no other step can see, because every other step is driven by an object that exists. That declaration is **configuration, not knowledge**: it lives in the repository's own `.claude/settings.json` `env` block, adds no area, no frontmatter key and no file here, and a repository declaring nothing is byte-identical to one before it existed. The home, the three-field shape, the absent-means-nothing rule and the two rejected homes with their costs are stated once in `workaholic:moderate`, *Where a cadence is declared, and what it says*; nothing about it is restated in this table.
...
| `strategies/`   | `type: Strategy`, `title`, `slug`, `status` (`active` / `achieved` / `abandoned`), optional `stage` (the operator's declared phase, from `進行中` / `改良中` / `観察中`; **absent means `進行中`**, resolved in `read.sh` and nowhere else), `target_date` (the Schedule's bound, `YYYY-MM-DD`), non-empty `assignees` (the Assignee), optional `feedback` relation; body sections `## Aim` and `## Schedule` |
| `terms/`        | `type: Term`, `title`, `description`, `category`       |
| `tickets/`      | See `/ticket` command for full schema. **No `type:`** (the OKF exception); optional `status:` — absent means queued, `done` / `abandoned` / `icebox` mean archived with that outcome; optional `deferred: <why>` — the operator's hold on a ticket that stays queued, absent means not deferred |
