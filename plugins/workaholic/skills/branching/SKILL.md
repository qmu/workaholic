...
| `publish-tree-commit.sh` | the base branch directly | **no unattended path** (2026-09-11): the tick's records and the ship's deferred concerns moved onto the pull-request seam; it remains for an attended developer's own publication and the suite's fixtures |
...
**And since 2026-09-19 the sweep has a caller** (issue #1212): `/moderate`'s `worktree-sweep` step runs `reap-worktrees.sh --apply` hourly from the main checkout (`workaholic:moderate`, `reference/workflow.md` §33). Until then nothing invoked it at all, so a mechanism built after a 53 GB incident was reachable only by a person who already knew its name. The step derives **no predicate of its own**, passes **no survey in** (the reaper re-derives the proof from a fresh survey at the moment of the act) and never passes `--force`; running it from a **linked** worktree would silently exclude that one, which is why the caller is the main checkout.

**Reclaimable is two conditions and both are required: `merged` AND `clean`** — either alone is not enough, and every skip is named with its reason. **`merged` is `ahead == 0`, an ancestry test**, so under the squash-merge policy (2026-09-01) a landed branch is **permanently `merged: false`** and stays skipped `unmerged` forever — the same misreading `superseded` was repaired for in issue #788. That is a known, deliberately unloosened limitation (the operator's ruling, 2026-09-19): the predicate is what makes the irreversible act safe, and the cost is that a repository can hold a large, permanently unreclaimable backlog. The sweep's summary names that backlog rather than reporting a clean tick over it. Reap applies exactly the survey's predicate (one rule, one implementation, so the reaper can never disagree with the reader a human just looked at) and is a dry run by default. Prune reclaims build output without removing the worktree; its target must be git-ignored **and** in the named artifact set (`WORKAHOLIC_ARTIFACT_DIRS`) — ignored-ness alone would take `.env` and the leak denylist, exactly what `git clean -Xdf` would destroy.
