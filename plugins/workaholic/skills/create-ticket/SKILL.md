...
```bash
( cd <publish_path> && bash ${CLAUDE_PLUGIN_ROOT}/skills/gather/scripts/migrate-todo-owners.sh )
```
...
- `moderation.status: "needs_decision"` — present the merge/split options via `AskUserQuestion` and act on the choice.
