...
- **[Cautious Consideration of Distributed Systems](policies/modular-monolith-first.md)** (分散システムの慎重検討) — Starting with a well-structured modular monolith and extracting to distributed services only when the boundaries are clear and the evidence for distribution is concrete; avoiding premature operational complexity.
...
- **[Proactive Introduction of History Structures](policies/history-structures.md)** (履歴構造の積極導入) — Designing data structures to preserve history from the start, so that audit, temporal query, and debugging questions can be answered from the accumulated data.
