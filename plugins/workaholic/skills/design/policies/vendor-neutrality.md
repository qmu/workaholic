...
The implementation that secures **modularity** for external dependencies is the **anti-corruption layer**. Place a thin layer that translates the vocabulary, types, and exceptions of the outside world into those of the domain, between the dependency and the domain. The aim is that when a vendor is replaced, **the language of the calling code** does not change (see also [Codifying Domain Terminology](../../planning/policies/terminology.md)).
...
Related: [Active Use of Unit Tests](../../implementation/policies/test.md).
