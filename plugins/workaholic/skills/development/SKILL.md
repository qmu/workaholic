...
- **[Policies Distributed as Plugins](policies/policy-as-plugin.md)** (ポリシーのプラグイン化) — Distributing all six policy series as Claude Code / Codex plugins so that the model's working context already includes our standards when AI writes code.
- **[Maintaining Use of Multiple AIs](policies/multiple-ai-use.md)** (複数のAI利用を維持する) — Keeping multiple coding agents regularly usable so that a failure, rate limit, quality change, or specification change at any single provider does not stop AI-assisted development.
