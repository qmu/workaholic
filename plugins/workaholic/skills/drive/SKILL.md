...
Confirm the install: `bash ${CLAUDE_PLUGIN_ROOT}/skills/check-deps/scripts/check.sh`. `ok: false` → print the `message` and stop. **`loaded_version_behind_registry: true` terminates `pending` before surveying** — a superseded plugin binding changes the survey's answer; treat `registry_unreadable: true`, and the **absence** of the field, identically (see `workaholic:check-deps`). `version_drift` or `missing_guards` → warn, continue, record in the run report.
...
**Under `/implement` only, post the unit's 🟠 start** into its feedback items' threads — resolve them with `bash ${CLAUDE_PLUGIN_ROOT}/skills/drive/scripts/unit-feedback-stems.sh <artifact>...`; when a merged PR started the run, read the thread back first with `bash ${CLAUDE_PLUGIN_ROOT}/skills/branching/scripts/read-notify-target.sh <pr-number-or-url>` (`found: false` / `absent` is the documented fallback to the stems). **An attended `/drive` run posts nothing to Slack, at any step** — the operator is present. Post rules: [reference/routing.md](reference/routing.md); `claim.sh`'s own one-line bot notice is a separate token-gated surface, never load-bearing.
