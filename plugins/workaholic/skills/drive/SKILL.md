...
| **`/implement [<unit>]`** | **Unattended.** Issues no `AskUserQuestion` anywhere, at any step. | The `[Implement]` routine and every caller-side loop (`/goal /implement ok`). |
...
**`loaded_version_behind_registry` is a STOP, and the only one this step has.** When it is `true` — the session bound a *superseded* plugin cache directory while the harness registry records a newer one — **terminate `pending` before surveying**, naming `version` and `registry_version`. Do not claim, do not push, do not report `ok`. It is the one drift that changes the survey's *answer* rather than merely dating the code producing it: measured 2026-08-04T22:58Z, a tick bound to `1.0.112` ran a `claims.sh` predating the rename-following resolution, called five already-driven tickets fresh backlog, and **claimed one of them** — a double-pick reaching a pushed ref, the precise failure the claim protocol exists to prevent. The same standard already applies to `current: false`: a survey computed by scripts of unknown age has established nothing about what remains. The SessionStart bootstrap cannot rescue this (it is decided that it never refreshes a running session), so the repair is a fresh session, not a retry.

Treat `registry_unreadable: true` identically — an unanswerable question must not render as agreement — **and treat the absence of the field itself the same way.** A build too old to emit `loaded_version_behind_registry` is, by construction, exactly the stale build the field exists to catch; reading a missing key as "no drift" would let the defect suppress its own alarm.

If `version_drift` is `true` (the *checkout* axis — a different question, and a warning), **warn and continue**, naming both `version` (installed) and `checkout_version` in the run report. Drift is deliberately not a stop — see `workaholic:check-deps`, *Why drift is a warning, not a stop* — but it is the single most useful line in the report when a tick later dies on `dirty_workspace`, because an installed build running an obsolete always-on migration is what produces that dirty tree. Note the check runs **before** the fast-forward below, so a runner whose clone is itself behind the base can see a falsely-matching pair on this tick and the real drift on the next.

Then **freshen the checkout before reading it** (decision J3):

```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/branching/scripts/sync-main.sh
```
...
**One unit at a time, and the reason is cost rather than impossibility** (relocated from the `[Drive]` routine template, 2026-08-05). Claim a unit, drive it, report it, route it, and only then survey again and take the next; never claim several up front. An unfinished claim *is* recoverable, but only after its heartbeat lapses — at least `WORKAHOLIC_CLAIM_HEARTBEAT_STALE_MINUTES` (default 30) during which the unit sits untouched and invisible to every other runner, plus a resume round trip on a later run. Claiming N units up front puts N-1 of them through that wait; the sequential loop puts at most one through it. **There is no per-run unit limit** in the other direction: keep going until the survey offers nothing claimable or the session ends — a run that stops early with work still queued has wasted its window, and for an hourly runner the tick *is* the throughput. Prefer a mission over backlog tickets whenever both are offered.

**Under `/implement`, the unit announces its start into its feedback item's thread** (2026-08-05; scoped to the unattended entry point 2026-08-07). The threaded posts exist so an **absent** operator can tell a working fleet from a dead one — a developer attending a `/drive` session is already watching the run, so **an attended run posts nothing to Slack, at any step**. On `/implement`, resolve the unit to the records it answers, and post the 🟠 start line into each of their threads:
...
```bash
( cd <worktree_path> && … generate the story per workaholic:report's Write Story flow,
  run the branch-safety scan (warn tier — fold findings into the PR body, never a prompt),
  then: bash ${CLAUDE_PLUGIN_ROOT}/skills/report/scripts/create-or-update.sh <branch> "<title>" )
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/drive/scripts/effective-policy.sh mission <slug>
bash ${CLAUDE_PLUGIN_ROOT}/skills/drive/scripts/effective-policy.sh tickets <ticket-file>...
```
...
**Under `/implement`, whichever route a unit takes, it posts one finish line into the same threads its start landed in** — the stems `unit-feedback-stems.sh` already resolved at §3, or `unit:<unit-id>` when there were none. The shape follows the outcome (🟢 merge requested, 🚀 / 🟣 for a merge, 🟡 for a handoff, 🔴 for a blocked unit), one finish per thread, and a handoff **is** that finish rather than a third post. The rules live in `workaholic:workaholify`; what belongs here is only that the finish is owed by every route, including a demotion — a unit that was going to ship and stopped at a PR reports the shape it actually reached. An attended `/drive` run posts no finish line either — the run report in the session is the developer's record.

**`review` → stop at the PR.** Under `/implement`, post its URL to Slack so the human loop picks it up:

```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/propose/scripts/notify-slack.sh "<message with the PR URL>"
```

The notifier is never load-bearing: without a token it records `{"notified": false, "reason": "no_token"}` and the run continues identically. Under `/drive` the developer is the human loop — report the URL in the session and post nothing. The worktree and the claim **stay** — the unit is unfinished until its PR merges.

**`auto` → ship it** through `workaholic:ship`'s Ship Flow with no prompts (ship's *Unattended routing* section factors each interactive seam), which means the full evidence-gated doctrine and not a shortcut around it: catch up with `main`, prove the deploy contract, confirm in production, record the evidence, **then** merge, then release and extract concerns.
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/branching/scripts/cleanup-mission-worktree.sh <unit-id>
...
A handoff unit **writes the Handoff section** (`workaholic:report`, *Story Content Structure*), **opens or updates its PR** through the ordinary `create-or-update.sh` with the partial work pushed — an unpublished handoff is not a handoff — and, under `/implement`, posts the PR URL through the same notifier the `review` route uses. Its 🟡 line **is the unit's one finish post** in the item's threads (§6), not an extra one: the run ended there, and a unit whose least conclusive outcome produced two terminal posts would read as two events. **Its tickets stay stamped and stay in `todo/`**, so merging that PR carries a `claim:` onto the base: that is expected and is history, not a claim (*Claims*, decision M1). Do **not** strip the stamp to keep the base clean — the stamp at the tip is what keeps the ticket claimed while the PR is open (M1a). The PR section is the **authoritative** record and the run report is the log; they overlap deliberately and neither is redundant. A later run resumes exactly this shape (*Claims → Resume a dropped unit*), so a handoff and a resumption are one story told at two moments, not two mechanisms.

**The run report is the deliverable** — always emitted, terminal or not, because a run nobody can read is a run nobody can trust (`workaholic:implementation` / `observability`). Before the reconciliation line, state:
...
"I stopped" is not "it's done": a blocked unit is `pending`, not `ok`. This is verbatim the contract a caller-side loop such as `/goal /implement ok` waits on (decision I4 — `/goal` is a harness feature, not a command of this plugin; the token is the whole contract). A confident `ok` over an incomplete run is the masked failure `workaholic:implementation` / `observability` forbids, which is why the reconciliation line always precedes it: the outcome must be graspable from outside without a debugger.
...
- **Outside it** → **write a ticket, continue.** Do **not** fix it opportunistically: an unqueued fix rides into a commit whose message describes something else, and that is the "unverified inferences pile up in the code" that `workaholic:development` / `overnight-ai` names as the limit on a blank cheque.
