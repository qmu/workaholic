...
**CRITICAL**: Use `AskUserQuestion` with selectable `options`. NEVER proceed without explicit user approval. (In **night mode** this gate is auto-resolved as "Approve" — the user pre-authorized the selected batch upfront; see **Night Mode**.)
