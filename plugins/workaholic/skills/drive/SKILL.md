...
Confirm the install: `bash ${CLAUDE_PLUGIN_ROOT}/skills/check-deps/scripts/check.sh`. `ok: false` → print the `message` and stop. **`loaded_version_behind_registry: true` terminates `pending` before surveying** — a superseded plugin binding changes the survey's answer; treat `registry_unreadable: true`, and the **absence** of the field, identically (see `workaholic:check-deps`). **`bootstrap_reload_pending: true` terminates `pending` the same way** — this session's own SessionStart hook performed a real install/update, and Claude Code does not re-register a plugin's skills/commands without a human-typed `/reload-plugins`; treat it as a superseded binding (`workaholic:check-deps`'s *Bootstrap reload-pending* section). This gate is best-effort by construction: it protects only the case where this very check remains reachable — a *totally* fresh, never-registered install is exactly as unreachable as this script, and that case is instead caught by `session-start.sh`'s own trailing message, before a `/drive`/`/implement` invocation is even attempted. `version_drift` or `missing_guards` → warn, continue, record in the run report.
...
| The run bound a **superseded plugin** (`loaded_version_behind_registry`, `registry_unreadable`, a `check.sh` too old to report either, or `bootstrap_reload_pending`) — §1 terminated before surveying | `pending` |
