...
- **When the run was started by a merged pull request, that PR names the thread.** Read it back
  ...
  documented fallback to the stems and the thread rules in `workaholic:workaholify`; `no_gh` /
...
- **Per unit, never per run** ("a run started" names no item, so it has no thread); with no stems
  ...
  `workaholic:workaholify` (*One thread per feedback item*). The posts go through the session's
