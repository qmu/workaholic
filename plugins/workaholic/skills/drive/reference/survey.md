...
Two drift axes with different consequences (full field semantics: `workaholic:check-deps`):
...
- **`unbound_in_claude_session: true` is a warning, not a stop** (2026-08-10, ticket
  ...
  `Skill(...)` call failed `Unknown skill: workaholic:drive`. There is no fix inside the plugin:
...
| `dirty_workspace`, **the retry** | **Added 2026-09-08** (mission `clear-the-residue-the-base-already-holds-and-never-stop-silently`). Before terminating on that reason, run `branching/scripts/clear-proved-residue.sh` **once** and, on a `cleared` outcome, re-run `sync-main.sh` **once**. The act clears only residue `classify-residue.sh` proves the base already holds (`on_base` — the index entry *and* the worktree file byte-identical to `<base>:<path>`) or the repository's own generator rewrites (`regenerable`), re-deriving each path's class through that same reader in the moment before touching it. It refuses `divergent_residue`, `unanswerable_residue` and `untracked_present` with the tree **byte-identical**, and **never removes an untracked file** — it is on no ref, so no proof covers it. Report what was cleared and what was refused; a freshen that still refuses terminates `pending` exactly as before. **The retry is once, not a loop** — a freshen still refusing after a successful clear is reporting something the proof does not cover. `sync-main.sh` is **byte-identical** to its pre-change form: the ask refused widening it by name, and the judgement lives in the layer above so that script's refusal contract holds for every other caller. Measured: 21 ticks over ~100 minutes never reached a survey over six staged paths, four of them blob-identical to `origin/main`. |
