...
- **Two mechanical layers sit beneath that judgement, never instead of it**: `scan-outbound-body.sh` applies the release scan's `secret` (hard stop — never overridden, never sent) and `leak` (fix, or override with the reason recorded) rules to the composed body; `check-outbound-body.sh` refuses a body still naming this repo — its name as a standalone identifier, its `owner/name` form, any form of its clone URL, or its absolute path — citing the matched text and line.
