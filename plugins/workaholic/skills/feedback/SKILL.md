...
**The consequence, written down rather than papered over: a fallback record is not discovered by `[Propose]`.** Discovery reads open issues, not files (`propose/scripts/list-inbound-issues.sh`), so the ask is captured but not proposed until someone files it or re-runs. Do **not** add a sweep that re-reads local records for something to propose — that is the retired `[Propose Batch]` design — and resist promoting a fallback record into an issue later, for the same reason.
