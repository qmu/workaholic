...
The definition of what each directory is responsible for is deferred to [Anti-Corruption Structure](anti-corruption-structure.md); here TypeScript-specific details are added. The base library that `model/` may depend on is plgg, our rich-typing reference implementation. `dependency/` contains external libraries such as ORMs and exposes only primitive types, plgg types, and model types at the boundary. HTTP routers, CLIs, and other entry points are not placed in this package; a separate package handles them as thin shells that import and call `service/`'s exported functions (see [Domain Layer Separation](domain-layer-separation.md)). What is exposed outside the package is limited to `service` and `model`; `dependency` is not published. Where to place tests, package-specific scripts, and documentation follows the package-level layout in [Standard Directory Structure](directory-structure.md).

### Related

The reasoning behind avoiding `any`, `as`, non-null assertions, and `@ts-ignore`, and the approach of rich typing with `unknown` boundaries, `as const`, `satisfies`, and nominal types, is in [Preferring Rich Typing](type-driven-design.md). The reasoning behind avoiding `class`, `throw`, and `==` and defaulting to immutable data, return-type failure expression, and declarative comparison is in [Preferring Declarative Code](functional-programming.md). The language-agnostic principles for internal package arrangement are in [Anti-Corruption Structure](anti-corruption-structure.md), and the repository-wide layout is in [Standard Directory Structure](directory-structure.md). This policy sets out these principles as concrete TypeScript feature choices, showing which defaults to reach for.
