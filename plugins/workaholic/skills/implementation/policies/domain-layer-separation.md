...
slug: domain-layer-separation
category: implementation
source: https://qmu.co.jp/implementation/domain-layer-separation
---
...
Entry points such as HTTP, CLI, and queue are kept as thin shells that call functions in the domain layer and return the result, while the domain layer itself is written in the language's standard vocabulary with rich types (see [Preferring Rich Typing](type-driven-design.md)). The more this form is maintained, the wider the range in which static checking and unit testing can support the correctness of domain decisions.

## Responsibility (責務)

The situation this policy aims to prevent is a state in which entry-point-specific types, vendor-specific types, or framework-specific contexts have leaked into the domain layer's signatures.

When ORM record types or HTTP framework Request types appear in domain function signatures, business logic is written directly into HTTP handlers, CLI commands, and queue workers, and the domain layer implicitly depends on framework-specific contexts such as DI container resolution results — what was pure computation verifiable by language standards alone becomes something that can only be confirmed by starting up the framework.
