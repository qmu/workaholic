...
**A mission's owner is not stored on the mission; it is derived from the `assignees` of the strategy it executes.** Ownership moved up to the strategy layer (`workaholic:strategy` — a direction is what a set of people own, and each of its missions is theirs). The mission's own `assignee` field is now a **legacy fallback only**: `create.sh` leaves it empty, and new work is owned through the strategy link.
