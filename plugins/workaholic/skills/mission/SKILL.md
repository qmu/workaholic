...
- **Current meaning** (2026-07-28, bounded below 2026-08-04): an **optional, epic-equivalent grouping of two or more tickets** — bounded at both ends, never a required parent. *Optional* governs whether a ticket needs a mission; it never meant a mission may be shapeless, which is why the ticket floor below is a clarification of this line rather than a fourth meaning. The strategy layer is **retired**: direction accretes in the feedback stream instead of a second direction artifact (two homes would drift), and mission ownership returned to the mission itself. The 2026-07-21 phrase is superseded on both ends — the strategy end and the mandatory-sounding framing (`docs/loop-engineering-workflow.md`, decisions B3/B5; provenance: mission `loop-engineering-foundation`). Retired strategies survive verbatim as feedback records (`migrate-strategies.sh`).
...
**What a carried close does: `--successor-title` is refused; a carry must name an existing mission** (`--successor <slug>`), because a minted successor arrives with zero tickets and so violates the floor by construction — which is how the live violation of 2026-08-04 reached `main`. **The refusal was sequenced behind one fix, and both have landed** (2026-08-04): the unmet-acceptance inheritance used to live entirely inside `close.sh`'s mint branch, so refusing the title first would have left no carry route that transfers the remainder. The extraction moved to `lib/acceptance.sh`, the refusal followed, and the mint branch went with it — **`--successor <slug>` is now the only carry route**. The rejected alternatives, the accepted cost, and the sequencing are in [`reference/schema.md`](reference/schema.md), *The ticket floor*. **Where the check lives, stated once:** the publish seam every creation path shares — the Creation Interrogation, `/propose`, and any future minting path. The count is `queue-size.sh`'s `meets_floor`; enforcement is written against this section, never re-derived per seam.
...
**The floor is checked here, before the publish commit, and stated during the interrogation rather than sprung at the end** — run `queue-size.sh <slug>` and read `meets_floor`; under two the mission is not published, and the refusal names the alternative (*Granularity → The ticket floor*). **The split cap does not apply to a mission — a deliberate, scoped exception.** `create-ticket` §4 caps a split at "2–4 discrete tickets", which is right for one request that turns out to be several. A mission is the opposite case: an execution plan that bundles *many* tickets by definition, and "a complete set to drive through one by one" is the requirement. Capping it at 4 would force either an incomplete plan or a fake ticket boundary. A mission decomposition answers to its own rule: **one ticket per genuinely separable unit of work, however many that is**. The cap still applies to `/ticket` itself; this exception is mission-scoped and stated here so it is not a silent violation.
