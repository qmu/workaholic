...
**What a carried close does: `--successor-title` is refused; a carry must name an existing mission** (`--successor <slug>`), because a minted successor arrives with zero tickets and so violates the floor by construction — which is how the live violation of 2026-08-04 reached `main`. **The refusal was sequenced behind one fix, and that fix has landed**: the unmet-acceptance inheritance used to live entirely inside `close.sh`'s mint branch, so refusing the title first would have left no carry route that transfers the remainder. Both routes now inherit through `lib/acceptance.sh` (2026-08-04), so the refusal is unblocked. The rejected alternatives, the accepted cost, and the sequencing are in [`reference/schema.md`](reference/schema.md), *The ticket floor*. **Where the check lives, stated once:** the publish seam every creation path shares — the Creation Interrogation, `/propose`, and any future minting path. The count is `queue-size.sh`'s `meets_floor`; enforcement is written against this section, never re-derived per seam.
