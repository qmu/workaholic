...
The bar equals creation's: a structured **delta model** — what changes, which tickets, in what order — not a Q&A transcript (`workaholic:planning` / `modeling-centric-design`), grilled until the delta is drive-ready. `gate_*` is never interrogated, exactly as at creation.
