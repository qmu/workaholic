...
**3. Ask the merge-policy ruling, then scaffold.** `merge_policy` is recorded at creation (K2), so the one genuinely human ruling this flow owns is asked before the scaffold: one `AskUserQuestion` — **may this mission's completed units merge automatically, or must a human review each PR?**, options `auto` and `review`. It is **never decided for the developer**: `auto` by default grants unattended merging nobody asked for, and `review` by default silently discards the question. Then run the scaffold with the publish tree as the working directory (a `( cd … )` subshell, so the persistent cwd stays at the repo root):
