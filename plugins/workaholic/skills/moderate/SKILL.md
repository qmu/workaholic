...
**What it can ask about is what its steps read**, which is why the asking steps were added one at a time and each is named where it lives: `stalled-units` (a claim that has not moved — never one whose work already reached the base, which is a fact for the log rather than a question, 2026-08-26), `direction-health` (a direction out of date or unanswered) and, since 2026-08-26, **`unanswered-asks`** — a message on the repository's channel that nobody has answered, **mention or no mention**. Before it, the one surface in this plugin that names a person could not learn that a person was waiting: a question written in the channel reached somebody only if one of the tick's own readers happened to produce a row about it (`reference/workflow.md` §16).
