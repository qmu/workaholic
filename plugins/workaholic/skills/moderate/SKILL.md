...
**Two gates, and an idle hour is silent**: post when there is at least one question **or** at least one changed step. `idle`, `no_previous_tick` and `no_log` all post nothing — and `no_log` is reported separately, because a mechanism that could not read must never announce quiet.
