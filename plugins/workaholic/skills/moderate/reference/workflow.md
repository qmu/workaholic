...
| `stranded-publications` | `left` | Every class `/implement` can attempt is attempted — `mechanical`, `clean` and, since 2026-09-02, `content` — through `settle-stranded-publication.sh`, so filing any of them would ask for work already in flight. What the merge itself cannot settle is the **act's** residue and is reported where the act is, not filed as a finding and not deferred to the publication's author. |
