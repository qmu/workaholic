...
#
#   list-standing-rulings.sh [--root <.workaholic>]
#
...
#
# ═══ IT JUDGES NOTHING ═══════════════════════════════════════════════════════════════
# Every entry reads `decision: "undecided"`. WHICH direction a mission answers and WHICH
# account an address belongs to are readings only a person or a run can make — a script that
# guessed either would be authoring the operator's ruling, which is exactly what
# `carry-attribution.sh`'s header forbids and what the whole ruling path rests on. The
# `repair` string therefore carries a PLACEHOLDER for the half only a judgement supplies
