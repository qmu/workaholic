...
jq -cn --slurpfile registry "$SCRIPT_DIR/steps.json" --slurpfile input "$INPUT" '
  $input[0] as $i | [$registry[0].steps[]
    | . as $s | ($i.last_run[$s.id] // 0) as $last
    | select(([$s.depends_on_snapshot[]?] - ($i.changed_snapshots // [] ) | length) < ([$s.depends_on_snapshot[]?]|length)
             or ($last + ($s.trigger.seconds // 3600)) <= $i.now_epoch)
    | {id,script,reason:(if ($last + (.trigger.seconds // 3600)) <= $i.now_epoch then "due" else "snapshot_changed" end)}]
