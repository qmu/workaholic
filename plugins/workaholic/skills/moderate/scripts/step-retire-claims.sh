...
if [ "$sn" -gt 0 ]; then
    ...
    stranded_needs=$(printf '%s' "$stranded_rows" | jq -c '{action: "offer_stranded_claim_recovery",
        implement: "Run recover-stranded-claim.sh for the named unit/branch and freshly observed head; publish a draft review PR. The holder question is supplemental, not a prerequisite.",
        bound: "one question per unit, addressed to the claim holder, keyed on `key` so it is asked once. The tick offers Implement recovery and may notify the holder: it never deletes the branch, never merges it, never releases the claim and never re-drives a ticket. THIS UNIT IS NOT A RETIREMENT CANDIDATE and must never be offered as one.",
        compose: "lead with what happened in words a reader outside the repository understands -- this unit'"'"'s tickets are archived on the base while its branch has candidate residual files requiring a fresh tree assessment -- and name the unit and the exact branch AFTER that, never before. Name the files from `files` (they are already bounded; when `file_count` is larger, say `and N more`), because a person cannot rule on work they cannot see. Say that deletion is protected until the residual effects are assessed. Ask what should happen to it -- landed on the base, or discarded deliberately -- and never suggest deleting the branch. When `age.first_seen` is set, say how long this has been ASKED ABOUT (`age.ticks` ticks since `age.first_seen`, `at least` that when `age.first_seen_is_floor`); when `age.readable` is false, name it as an age we could not read, by its `age.reason`.",
