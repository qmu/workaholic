...
**What may become a prompt at all — the developer-only test.** Even inside the up-front batch, a prompt is admissible only for a ruling the agent **must not** make on the developer's behalf. There are four such kinds, and the list is exhaustive:

- **Authorization for an irreversible outward action** — deploying, sending to third parties, publishing, merging.
- **Security-boundary values** — who is granted access, which recipients, what is exposed.
- **Secrets and credentials** — values the agent cannot obtain and must never fabricate.
- **A true strategic fork with no evidence-based default** — genuinely diverging directions where the developer holds a preference no evidence can supply. This is `rules/interaction.md`'s Recommended-label test: if the first option could honestly be marked "(Recommended)", it is not this kind.

**Everything else is the agent's to decide.** Which fixable failure to retry, whether to finalize now or push one more step, the ordering of independent work, how to recover a stale environment, which of two mechanical repairs to apply — all follow from the evidence and the developer's stated intent. Decide, **record the decision and its reason in one line** in the pre-flight review's **"decisions taken"** block (item 5 above — the record seam that makes a saved question vetoable rather than silent), and proceed.

**The litmus test, applied before every prompt: name the developer-only input you lack.** State — to yourself, concretely — which of the four kinds above the missing input is, and why you can neither obtain nor derive it. **If you cannot name it, you must not prompt: decide and act.** "I would rather be sure" and "the developer might prefer something else" are not missing inputs; they are the absence of a decision.

**Never re-ask what intent already answered.** When the developer has stated an intent ("do it now", "get these done tonight"), a prompt that asks whether to proceed, which obvious next step to take, or whether they really meant it is not a decision — it is the run declining to act on an instruction it already has. Re-asking a settled question is the failure mode the batch exists to prevent, not a form of diligence.

**And after dispatch, nothing is asked at all** (§3) — regardless of kind. A genuine developer-only ruling that surfaces mid-run is **deferred and recorded** for the morning, never prompted. The four kinds above govern what may enter the *up-front batch*; they never reopen the interaction point once the run is unattended.

**Planning-time requirements elicitation is a separate discipline — and it is mandatory, never a "don't ask".** Everything above governs the *execution* run: what an unattended drive may decide vs. defer. It does **not** apply to the **planning** that produced these missions. A mission's plan must have been built by **eliciting the developer's requirements** — what a user must be able to do, what a good output looks like, the real workflow — because the developer holds the *what* and the agent cannot derive it (`workaholic:mission`'s *Elicit the requirements first* gate). A mission planned from a title, or one whose requirements the developer *invited questions about* and the planner skipped, is a **planning defect** an unattended run will faithfully amplify into hours of unusable output; no downstream verification rescues it. So: **an unattended run must not begin on a plan that was never elicited.** If the pre-flight finds a mission whose plan reads as title-level guesswork for user-facing work (no user-experience-level `## Acceptance`, no elicited `## Experience`), that is a **replan design ruling** for the up-front batch (elicit the requirements now, in the main agent), not an eligible drive. Decide the *how* autonomously at run time; never assume the *what* at plan time.
...
- Inherit drive's **failure contract** verbatim: attempt every authorized ticket (size/complexity never skip), `failed` = implemented but red (stash, record, continue), `blocked` = a named hard external blocker **with the attempted command and its raw output recorded** (a verdict reached without running anything is an unattempted ticket, not a blocker), `deferred` = an unqueued observed problem becomes a minted ticket (inheriting the `mission:` relation) and the run continues. Long, heavy, or exclusive live verification is **preferred** overnight work, never a deferral reason. Only the contract's two buckets defer without an attempt: the safety floor, and a concretely-stated external blocker. Safety floor unchanged: never auto-icebox, never destructive git.
...
**Watchdog every long or backgrounded handoff — silence is never progress.** Non-blocking must not decay into fire-and-forget. Whenever the dispatcher hands off a long-running or detached task and stops actively driving it, it owes that task three things:

- **A bounded fallback wake-up**, sized to the job's **longest expected step** — one fallback that fires if the completion signal has not arrived by then, not a tight poll. A healthy run pays almost nothing for it, because the completion signal normally arrives first.
- **Active liveness verification on wake.** Do not infer state from the absence of a message: read the job's **declared output artifact**, its run log, and the relevant process/container state. A result file recording a failure, an exited process, or a torn-down environment is a **terminal failure to surface now**, not a reason to keep waiting.
- **An honest classification of prolonged silence.** Past the bound, with neither a completion signal nor evidence of progress, the task is a **suspected fault to investigate**, never "still running." Investigate by reading the artifacts, then classify.

**Never disable your own oversight.** Telling a leaf "finish at your own pace, I will not check again" removes the only remaining way the run would have looked — it converts a monitorable task into an unowned one. The dispatcher may stop *nudging*; it may not stop *watching*. This is the mirror of drive's failure contract: there a leaf gave up without evidence, here a dispatcher waited without evidence — both trust a liveness signal nobody verified (`workaholic:operation` / `recovery-and-runtime`: liveness is observed, not assumed).

**Give a detached job an explicit, self-contained environment.** A background or detached process does **not** inherit an interactive shell's PATH or environment, so a job that runs by hand can exit immediately when detached — unable to find a CLI the interactive shell resolves fine. Pass an explicit environment and absolute paths when spawning one, and treat its early exit as exactly the failure the watchdog exists to catch.

**Check provisioning before trusting a "missing credentials" deferral.** `create-mission-worktree.sh` carries every env file the project reads (declared in a repo-root `.worktree-env`, else the root `.env` plus discovered subdir `.env` files) and reports `env_files_carried` in its JSON. A worktree whose carry is **empty** — or whose leaf reports absent credentials — is a **provisioning** finding to inspect, not a fact to record: an env loader fails silently on a missing file, so "credentials absent" from inside a worktree is consistent with a real shortfall *and* with a file the creator never carried. Surface an empty carry in the pre-flight, and treat a leaf's credential-shortfall deferral as unverified until the provisioning is confirmed (drive's failure contract). Days of unattended capacity were once burned on exactly this false finding — the credentials were on the machine the whole time; the worktree just never carried the file that held them.
...
- **Escalation-blocked** — every remaining item waits on a judgment call. **Verify the blocker earned the label before accepting it:** a leaf's `blocked` entry counts only when it carries an attempted command and its raw output, or names one of drive's two no-attempt buckets (safety floor; a concretely-stated external blocker naming what is missing and who must provide it). A leaf that deferred authorized work for being long, heavy, exclusive, or abstractly "human-only" has **not** produced an escalation — that mission is **still driveable**, and the next wave re-dispatches it with the failure contract's rule restated in its payload (narrow the wave if the work needs the environment to itself). Do not launder an unattempted ticket into the escalation list; it inflates the morning's decision queue with decisions nobody has to make. The run is **unattended after dispatch**, so a genuine escalation does **not** stop it to prompt: because every *foreseeable* decision was resolved in the up-front batch (§1), a decision surfacing now is by definition one the pre-flight could not foresee, and it is **deferred and recorded in the final report** (§5), never asked. The mission is left short of that step, its blocker named for the morning review. A deferral is recorded **once** — it is **not re-asked or re-logged** on later waves. This is the `overnight-ai` guardrail: an item AI cannot judge is collected for the morning, never guessed and never silently acted on (a novel destructive or irreversible step is exactly such an item — deferred, the mission left short of it).

- **Silent** — the wave's bound elapsed and this mission's leaf has reported nothing. This is a **suspected fault, not a running job**: verify it actively before classifying (read the leaf's declared artifact, its run log, the worktree's `git log`/`status.sh` for evidence of progress, and any process or container it launched). Evidence of failure → surface it as `failed` with what the artifact says; evidence of real progress → let it continue under a fresh bound; no evidence either way → treat it as failed-and-unreported rather than waiting again. A run must never let a second bound elapse on the same unverified silence.

**Bounded run**: at most **3 waves per invocation**, and a wave that archives nothing terminates the loop (no spinning on the same blockers). A wave is classified when its reports are in **or its bound has elapsed and the silent leaves have been actively verified** — never by waiting indefinitely on a leaf that may already be dead. The caller's loop (`/goal`) provides the long horizon; one invocation provides bounded progress plus an honest report.
