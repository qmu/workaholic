...
- Start is always `🟠 Implementing for [...]` (issue #300's literal template, [reference/notifications.md](reference/notifications.md)); exactly one start and one finish per thread, and the finish's shape follows the outcome: `🟢 Implemented` for the ordinary review-stop, `🚀` merge, `🟡` handoff, `🔴` blocked. A handoff is the finish, never a third post. A human merge of a `review` unit's PR posts no finish line of its own — `/implement` never announces it (that was `[Consent]`'s retired job); the thread stays at `🟢 Implemented` and the merge is readable on GitHub.
