...
The exact shapes of the runner's posts (🔵 proposed / 🟢 implemented / 🔴 blocked / 🟡 handoff / 🚀 auto merge) are in [reference/notifications.md](reference/notifications.md); a template names its postable events and defers the shapes there. **The start shapes (`📐 Proposing`, `🟠 Implementing`) are retired** (2026-08-11, developer's order — a routine posts its finish only); `/propose`'s and `/implement`'s finish lines carry issue #300's literal wording as the **sole** sanctioned format (P10, 2026-08-07, narrowed to the finish events), and the outcome-specific finish shapes (`🚀` merge, `🟡` handoff, `🔴` blocked) are unchanged, since each carries information the generic finish line would lose. The fourth outcome shape this line once listed, the purple-circle human-merge line, is erased (qmu/workaholic#317) — see *Which thread an `/implement` unit's posts land in* above and [reference/notifications.md](reference/notifications.md) for what stands in its place. Standing rules:
