...
`🔵 Proposed` retires the earlier `🟢 Proposed to <@U…> - ...` shape and carries the root role: when it is the first post to land in a thread the stateless lookup did not find (SKILL, *One thread per feedback item*, case 5) it carries `` `fb:<stem>` `` on its own line, never dropped — one shape covers both a fresh root and a reply into a found thread. The retired `📐 Proposing` start once preceded it; nothing replaces it.
...
**Ticket `20260810163359` (2026-08-10) narrowed statelessness again, without reopening Q1.** A live measurement (issue #360, feedback `20260810163347`; a closely related earlier report, feedback `20260808030816`/issue #306) found the search itself unreliable in practice: on a miss for both `fb:<stem>` and the issue/PR number, a routine posted a brand-new top-level root instead of finding an existing thread it simply couldn't locate. Q1's reasoning for statelessness was specifically about **P4's mechanism** — a target embedded in a public pull-request/Issue body, carried *between* routines and readable by anyone who could read the repository — and that reasoning is untouched: this ticket carries nothing in a public body, and the thread URL still never appears in one. What it adds is orthogonal — the item's own feedback record, already the vehicle for `fb:<stem>`, gains one more field (`thread_ref`) naming where its root landed, written once by the routine that posted it and read by every later event for that item before any search runs. The failure mode this reopens is the one P4's carried-target already surfaced and Q1's withdrawal (above) already accepted as a cost: a stale or unwritten `thread_ref` (a record predating the field, or a failed write) degrades to exactly the same search-based fallback that was the *whole* mechanism before this ticket — never to a guess, and never to a second propagation channel.
