...
- **[AI-Assisted Production Investigation](policies/ai-production-investigation.md)** (AIによる本番環境調査) — Arranging read-only paths for AI to reach the same production observation outputs as humans, with PII masked before entering AI context, so investigation speed improves without expanding the blast radius.
