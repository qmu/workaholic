...
#
# THE ONE `unresolved` REASON IS `stems_unresolvable`: the item qualified as an ask but no
# feedback record on the base names its issue, so there is no `fb:<stem>` to search for and no
# thread to announce into. It is NAMED there rather than dropped from the output — an item with
# no thread and an item that does not exist are different facts — and it is never given an
# invented key.
