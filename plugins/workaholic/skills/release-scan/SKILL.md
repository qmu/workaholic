...
**One consumer got this wrong until 2026-08-21.** `/drive`'s `review` route read `verdict == "pass"`, so a `size`-only branch — the tier this skill calls *a granularity nudge, not a hard block* two sections down — held its merge open indefinitely, and the unattended run reporting it asked a human what to do. The counting no longer happens in each consumer: `gate-decision.sh` computes `override_only` alongside `overridable`, because three copies of a tier rule is how one rule becomes three. The measurement that forced it is in `workaholic:drive` §6 and in that script's header.

**Who reads the script, precisely** (corrected 2026-09-18, ticket `20260918150931` — this sentence said *three consumers* and counted `/story` among them). `gate-decision.sh`'s consumers are **`/ship` §2b**, **`/drive`'s `review` route**, **`drive/scripts/catch-up-claim.sh`** and **`branching/scripts/prepare-publication.sh`** (behind `settle-stranded-publication.sh` and the operator catch-up) and — since 2026-09-18, ticket `20260918164552` — **`drive/scripts/land-unit.sh`**, which until then carried its own inline copy of the tier reading. **`/story` is not one of them**: it reads finding severities straight off the scan output, because it warns rather than blocks — do not add a gate call to it. `publish-tree-pr.sh` is now the only script that reads the scan JSON without this gate, and it is **safe**: its `case` ends in a `*)` arm answering `merge_reason="scan_unreadable"`.

**An unread scan REFUSES; it never passes** (2026-09-18, ticket `20260918150931`). There is a **third `decision`** beside `pass` and `block`:
...
The closed reason set, six words: **`no_scan_input`** (stdin empty or whitespace only), **`unparseable_input`** (not JSON), **`not_a_scan_verdict`** (JSON, but not one object whose `.findings` is an array), **`finding_unclassified`** (a `findings[]` element that is not an object, or whose `severity` is absent or outside `hard | confirm | override`), **`bad_argument`** (a positional argument — the script's one input route is stdin, and `< scan.json` is the recovery), **`jq_unavailable`** (`jq` absent, or ran and failed). **Exit status is 0 in every case**, the repository's refusal convention: every script consumer wraps the call in `|| printf ''`, so a non-zero exit would erase the reason and land them on their generic empty-output path.

The counts are **`null`, never `0`** — `0` reads as *counted, found none*, which is the conflation being removed — and that is load-bearing: a consumer reading a boolean licence cannot read a refusal as one. All three script consumers carry an **ordered `case` arm above their existing arms** mapping the word onto the existing `scan_unreadable`; no new vocabulary was minted. In `land-unit.sh` that ordering also carries the **`--override-scan` bound** — the flag is a ruling about findings, and a refusal means there are none to rule on, so the refusal arm sits above the override rather than a second condition being added beside it. The agent-level contracts (`/ship` §2b, `/drive` §6, `commands/implement.md`, `commands/infinite-development.md`) state the remedy: **re-run the scan correctly**. A refusal is neither a secret nor an override — never hunt a credential on it, and never record it as an accepted risk.

**Why a third word rather than resolving to `block` or `pass`.** `block` means *findings were found*, and a `block` carrying `hard: 0, confirm: 0` satisfies `override_only`, which is a merge licence — the permissive answer wearing the blocking word. Forcing `overridable: false` would make `/ship` report a credential that does not exist. **Measured** at `2a85aff9e`: empty stdin, a positionally-passed file, and a finding carrying `severity` but no `category` each answered `decision: "pass"`, the last one beside `"hard": 1` in the same object; and in the other direction a `"category":` anywhere outside `findings[]` inflated `total` into a false `block`. The tier is now counted **structurally** off `severity` with `jq`, and `decision` is derived from the severity counts rather than from `total`, so a `pass` beside a non-zero `hard` is unreachable by construction. `category` is still emitted and still read by `/story` and `publish-tree-pr.sh`; it is only no longer the axis this script counts. (`land-unit.sh` counted it too until 2026-09-18 and now takes the gate's structural `total` instead — the grep was the second half of the same fragility.)
