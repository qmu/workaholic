...
**Who reads the script, precisely** (corrected 2026-09-18, ticket `20260918150931` — this sentence said *three consumers* and counted `/story` among them). `gate-decision.sh`'s consumers are **`/ship` §2b**, **`/drive`'s `review` route**, **`drive/scripts/catch-up-claim.sh`** and **`branching/scripts/prepare-publication.sh`** (behind `settle-stranded-publication.sh` and the operator catch-up). **`/story` is not one of them**: it reads finding severities straight off the scan output, because it warns rather than blocks — do not add a gate call to it. `publish-tree-pr.sh` and `land-unit.sh` also read the scan JSON without this gate.
