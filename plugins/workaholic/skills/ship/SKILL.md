...
**CI also fixes the input, not only the permission.** The renderer's boundary is the latest release tag, and a container's clone carries whatever refs it happened to get. Measured 2026-08-18: one routine container had **no tags at all** and an `origin/main` five days stale, which collapses the boundary to `full_history` — the same repository state reported **9**, then **191**, then the true **8** unreleased commits as refs were fetched, and the `deploy:<digest>` moved with each. The workflow checks out with `fetch-depth: 0` and tags, so the rendered draft's input is defined rather than inherited. **What this does not fix is the reading half**: `report-deploy-status.sh` still reports against the container's refs, so the hourly line's count and digest remain only as good as the clone. That is left untouched here rather than half-solved.
