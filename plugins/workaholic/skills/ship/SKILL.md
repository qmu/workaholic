...
2. **Catch up with `main`**: Run `bash ${CLAUDE_PLUGIN_ROOT}/skills/ship/scripts/catchup-main.sh "<base-branch>"` so what you deploy equals what will merge. If `caught_up` is `false` (`conflict:true`): halt and ask the user to resolve the conflict before continuing.
