...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/ship/scripts/read-deployments.sh [--slugs | --slug <slug>]
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/ship/scripts/read-release-notes.sh [--latest-for <slug> [--exclude <path>]]
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/ship/scripts/read-deploy-state.sh [--rows | --base-rev] [--exclude-note <path>] [<base>]
```
...
Run after `workaholic:write-release-note` and before `merge-pr.sh`. **A failed push is a
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/ship/scripts/record-evidence.sh "<branch>" "<target>" "<method>" "<result>" "<status>" ["<note-path>"]
```
