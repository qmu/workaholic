...
**assigned to the session's own identity**, excludes any already related from a planned
artifact — **on the base or on an unmerged remote branch** — and hands back the rest
oldest-first. The exclusion is reported, never silently dropped, and under two words
rather than one: `already_planned` when a base record is carried by a mission or ticket,
`captured_on_branch` when it is on an open proposal's branch and the ask is
waiting on that pull request — and a third word that **is** a gate, `self_originated` (2026-09-02, issue #864): an issue whose body header reads `source: moderate` is the tick's own finding about the loop's own artifact, and it is never taken. Measured: five consecutive self-filed issues in one day, each proposed, ticketed, implemented and merged by the next ticks, every link refining the one before it, while the operator's development stopped entirely. Only a human's ask, or a strategy a human authored, originates a mission; the finding stays open as knowledge for a person to read. The branch half is the same git-native walk the dedup set
uses (`lib/unmerged-branches.sh`, extracted so the two cannot drift) and it degrades the
same way — every ambiguity resolves toward counting the branch, so toward excluding the
issue, with the reason on stderr; a walk that cannot run at all leaves the base grep
...
Process the complete oldest-first page, extending a compatible active mission before minting
another. This closes on an observable condition — no unsettled assigned ask and no capture-only
proposal branch — rather than an arbitrary delay or count. Unrelated and urgent asks still form
their own unit; they do not inherit another mission merely to empty the page.
...
| ------------- | ---------------------------- |
| Decomposes into **two or more** units of work **and there is a mid-term plan to hold** | The record **plus a mission with its whole ordered ticket set** |
...
- **Row 1 has TWO terms, and both must hold** (2026-09-03, mission `emit-a-mission-only-when-there-is-a-mid-term-plan-to-hold`). The ask must decompose into two or more units **and** there must be a **mid-term plan to hold** — several tickets wanting ordering and allocation across a period (`rules/workaholic.md`, *What a Mission Must Be Able to Hold*, cited rather than restated). Until then row 1 read on decomposition alone, which is *one mission per inbound ask*: measured in a single hour, eight asks arrived from the channel, seven became missions and forty-eight tickets inside twenty-one minutes, and 52% of the corpus sat at exactly seven or eight tickets. **An ask that decomposes but carries no such plan takes the next row it fits** — a loose ticket when it is atomic enough, otherwise the tickets it names with no mission wrapper, otherwise the record alone. **The judgement is reported by name** beside `precedence:<form>`, in the run report and the pull-request body: what the ask was judged to be and what it became instead, so a reader can disagree with it. It is a **judgement, not a gate** — nothing mechanical decides it, and rule 1's two-ticket floor stays exactly where it is, because a floor is checkable and this is not.
- **Row 1's one named case is a person's single review pass** — a batch whose items share **one review surface, one feedback thread and one coherent acceptance walk** is a mid-term container and takes row 1, all three terms required, with concrete **release, dependency, ownership or risk** evidence separating it and one mission absent such evidence (`rules/workaholic.md`, *What a Mission Must Be Able to Hold*, rule 2, cited rather than restated). Report `mission_held_by:review_batch` beside `precedence:<form>` and `mid_term_plan:<yes|no>` when that case decided, `mission_held_by:mid_term_plan` when the ordinary criterion did, and `separated_by:<release|dependency|ownership|risk>` when the evidence split the batch. It adds a case and removes none: the mid-term-plan term still refuses an ask with no plan, and every floor and the ceiling are untouched.
- **No cap is placed on the ingest path, deliberately** (the same mission). The obvious next request is a bound on how many missions `/specificate` may open in a window, and it is refused by name: `WORKAHOLIC_WIP_LIMIT` holds **origination** only and says so, and a limit that swallowed the operator's own instructions on a busy day would be a worse defect than the one it cures. The repair is the judgement above, applied per ask.
...
- **A strategy is not a mission factory** (2026-08-24, the developer's ruling, measured on a
  ...
  read them). The intended scale of a strategy's plan is **one mission holding that strategy's
  mid-term plan, extended by at most one follow-up repair mission** — sized by what the container
  must be able to hold rather than by a ticket count (`rules/workaholic.md`, *What a Mission Must
  Be Able to Hold*) — and the way row 1 honours
...
`make-a-direction-s-lifecycle-a-declared-stage`). The declared stage narrows nothing on this
route: an ask that arrives from outside — a swept channel message, an issue somebody filed, an
error reported — is judged, attributed and emitted here exactly as before. **And it narrows
nothing on the origination route either** (2026-09-18, ticket `20260908190000`): the
`observing` refusal this paragraph used to cite was retired the week it was introduced, so
`/propose` may originate a bounded observation hypothesis against such a direction and the
stage only leads its sort (`workaholic:propose`, *観察中 permits observation work*). What the
stage means is which kind of learning the direction needs, never that it needs none.
...
- **Only a human's ask originates a mission — `self_authored`** (2026-09-02, mission `refuse-an-ask-the-loop-wrote-to-itself`). The rule itself is stated once, at `rules/workaholic.md`, *What May Originate a Mission*, and cited here rather than restated. Read the ask's origin through `bash ${CLAUDE_PLUGIN_ROOT}/skills/feedback/scripts/ask-origin.sh` (the one reader; `workaholic:feedback`, *Whether a person wanted this*). On **`machine`**, apply the operator's second condition as **the run's own judgement**, stated in words in the pull-request body where a person can argue with it: *is its subject the loop's own apparatus?* When both hold, the outcome is **record-only, reported `self_authored`** — not silence and not an error. **On `unreadable`, do not refuse**: a grandfathered record with no `subject:` is an ordinary ask, and the reading is reported by name. **The sweep is never caught**: a person's channel message filed as an issue by a routine carries `subject: person:<author>` by the sweep's own contract, so the reader answers `human` — that is a human ask arriving through a machine, and it is pinned in the suite because it is the loop's main inbound path. **Measured**: five consecutive `[FB]` roots in one day that no human wrote, each proposed, ticketed, implemented and merged by the next ticks, each link refining the one before it, until the operator abandoned the direction mid-drive — every existing gate held, and not one of them asked *who wanted this*.

  **The refusal sits at the judgment, not at discovery, and that is a decision with a reason.** At discovery the ask would never be taken in hand at all; at the judgment *the record is written whatever the judgment concludes*, which is a standing contract of this seam — so the capture stays honest and the triggering issue still closes. Discovery's own `self_originated` exclusion (issue #864, keyed on a `source: moderate` body header) is **untouched and is a different test**: it keeps the tick's own findings out of the inbox, while this one reads the **subject axis** of whatever ask did arrive.
