...
3. **A strategy needs an owner and an aim present in the ask**, and a **date** it either states or takes from the operator's default — an **owner** (below) and an **aim with no decomposable plan**, both from the ask itself and neither ever inferred; and a **date** resolvable to a single `YYYY-MM-DD`, taken from the ask when it states one and otherwise **one week from the ask** (`strategy/scripts/default-target-date.sh`, the one derivation). The owner or the aim missing → record-only, naming which. **The date is the one part that is defaulted, and only when the ask states none** (2026-08-30, the operator's ruling; mission `draft-a-dateless-direction-with-the-operator-s-one-week-default`): an ask that *states* a date this run cannot resolve is still record-only, `no_target_date`, because defaulting over the operator's own words is the failure a default must never introduce. Measured 2026-08-30: three announced directions died at `no_target_date` with the refusal traced only by a parenthetical addressed to nobody, and the loop went on planning from the directions that already existed.
...
**The owner question the artifact forces.** `create.sh` refuses empty `assignees` — the one artifact where empty is a refusal rather than team-owned — and `/specificate` may never substitute the running identity (*Act only on an ask that is yours*). The two rules meet exactly at an **unassigned issue**: it cannot produce a strategy. The outcome there is **record-only with `no_assignee` reported as the reason**, never a strategy owned by whichever container executed the tick. **The `target_date` is not read the same way, and the difference is the operator's ruling rather than an inconsistency** (2026-08-30, mission `draft-a-dateless-direction-with-the-operator-s-one-week-default`). An owner cannot be defaulted because there is no answer to default *to* — substituting the running identity is precisely what *Act only on an ask that is yours* forbids, and a machine cannot know whose direction this is. A date has one, and the operator has stated it: **one week from the ask**. So an ask with an aim and an owner but no date **drafts the strategy on that default**, on the never-auto-merge path that already exists, where their merge is the authorship and editing the date before merging is the veto. The date is derived in exactly one place — `strategy/scripts/default-target-date.sh`, which counts from the triggering issue's own date so a tick ingesting an old issue does not date a direction from its own clock — and `create.sh` is byte-identical and never learns where the date came from.
