...
- **No mention token of any kind.** The line names the repository's state, not a person's work — the same reason `📦 Prepare release` carries none. A strategy's `assignees` is *in* the digest as a fact; nobody is paged by it.
