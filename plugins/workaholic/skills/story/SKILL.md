...
1. **Workspace Guard**: `bash ${CLAUDE_PLUGIN_ROOT}/skills/branching/scripts/check-workspace.sh`. If `clean` is false, display `summary` and ask via AskUserQuestion: "Ignore and proceed" or "Stop" (end the command).
2. **Detect Context**: `bash ${CLAUDE_PLUGIN_ROOT}/skills/branching/scripts/detect-context.sh`, then **Route by Context** on `context`:
   ...
   - `worktree` — `bash ${CLAUDE_PLUGIN_ROOT}/skills/branching/scripts/list-worktrees.sh`, filter to `has_pr: false`; none → say so and stop; one → confirm via AskUserQuestion; several → ask which. Run all subsequent git operations from inside the chosen claim worktree and re-detect there.
   - `unknown` — tell the user which branch was detected and that `/story` needs a work branch or a claim worktree, then stop. NEVER guess a branch to report on — a story written against the wrong diff is worse than no story.

## Write Story

Generate the story file, then create the PR — the story is the single source of truth for PR content. The `/story` command (main agent) runs this orchestration directly and spawns each leaf worker as a `subagent_type: "general-purpose"` Task; fan-out stays one level deep. Phase-by-phase detail: [reference/orchestration.md](reference/orchestration.md). The shape:

- **Phase 0** — first run `scripts/release-boundary.sh`. Bump the version only when it reports
  `eligible: true`: the branch has a readable, nonempty committed change set. Mission membership,
  completion and remaining tickets do not govern version eligibility. `eligible: false` is a
  completed reading, not a warning to override: report `version_not_allocated:<reason>` and
  continue the story/PR without a release note or delivery claim. This is what separates
  empty or unreadable ranges from releases; eligibility does not certify production readiness. At an eligible boundary, bump unless
  `check-version-bump.sh` says `already_bumped` **and** `version_ahead: true` (2026-09-02: a branch that bumped is not the same as a branch whose number is still free — several branches cut from one base each bump `N → N+1` correctly, the first to merge takes `N+1`, and every later squash then carries a bump commit whose *content* the base already holds, so it produces no version diff and nothing reports it; measured, five consecutive merges on this repository carried two versions between them). **`version_ahead: null` means bump** for an eligible release — it is the unreadable-manifest answer, and the direction is the one the degraded base read already takes; gather context via `bash ${CLAUDE_PLUGIN_ROOT}/skills/gather/scripts/git-context.sh`. Report the `reason` in the run's output; never re-derive the base by hand.
...
- **Phase 2** — right-sized to the branch: **≤2 archived tickets** (the common case since PRs merge per single ticket or small batch) spawns **one** combined opus worker covering release-readiness + a short overview + section-review, with no mermaid Journey; **>2** spawns the 3 workers in parallel as before — release-readiness (opus, `## Assess Release Readiness`), overview-writer (haiku, `### Overview Generation`), section-reviewer (haiku, preloads `workaholic:review-sections`). Both paths preserve the result record and cross-document relations identically; detail: [reference/orchestration.md](reference/orchestration.md).
- **Phase 3** — write the story per Story Content Structure. Take each ticket's commit hash from `bash ${CLAUDE_PLUGIN_ROOT}/skills/story/scripts/ticket-commits.sh <branch-name>`, NEVER from ticket `commit_hash` frontmatter (those are dead pre-amend hashes; every link built from one 404s). Update the stories index.
- **Phase 4** — roll every mission in the story's `mission:` list through the shared idempotent mutators (`append-changelog.sh`, `tick-acceptance.sh`), refresh the OKF indexes (`okf/scripts/refresh-index.sh`), commit, push. **Name the story file in `commit.sh`'s `files...`** — `.workaholic/stories/<branch>.md` is a file this phase just created, and default staging is `git add -u`, which cannot pick up a new file (`workaholic:commit`, *A caller that has just written a NEW file passes it in `files...`*). This is the seam that lost a story body while its index entry merged; since 2026-08-23 the split-rename refusal makes a forgotten name fail loudly rather than silently, but the name is still the caller's to pass.
- **Phase 5** — spawn the PR worker (opus, `## Create PR`); then display the full story content inline and the PR URL (mandatory). Release notes are written at ship time by `workaholic:ship`, not here.

### Judge Deferred Concerns

Run by the Phase 1 judge subagent. List the open set with `bash ${CLAUDE_PLUGIN_ROOT}/skills/feedback/scripts/list-open-concerns.sh` (empty → return `{"verdicts": []}` and stop); judge each concern against what landed since its `origin_commit`; return one `{"verdicts": [...]}` object. When in doubt prefer `still_active` — a false `resolved` loses institutional memory, a false `still_active` merely re-surfaces next story. Each `resolved` verdict becomes a superseding feedback record (`kind: concern`, `supersedes:`); the resolved record itself is immutable, never edited or moved. Evidence, heuristics, large-corpus efficiency, and the verdict schema: [reference/judge-deferred-concerns.md](reference/judge-deferred-concerns.md).

### Overview Generation

Run by the overview-writer worker: collect commits with `bash ${CLAUDE_PLUGIN_ROOT}/skills/story/scripts/collect-commits.sh [base-branch]` (the structured `body` keys feed Motivation and the highlights; `category` comes from the `Category:` git trailer) and return `{overview, highlights[], motivation, journey}`. Field guidance, line limits, flowchart rules, and both JSON formats: [reference/orchestration.md](reference/orchestration.md).
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/story/scripts/create-or-update.sh <branch-name> "<title>"
```
...
- **Branch-safety scan** (warn tier — `/story` cannot merge, so it warns loudly where `/ship` blocks): `bash ${CLAUDE_PLUGIN_ROOT}/skills/release-scan/scripts/scan-branch-safety.sh`. Key releasability off finding **severity**, not the binary verdict: `hard` (secret) or `confirm` (leak) forces `releasable: false`; `override`-only (size) findings keep `releasable: true`, each recorded as a concern plus a `pre_release` note that `/ship` will ask for the conscious override.
- **Hand-maintained area freshness**: `bash ${CLAUDE_PLUGIN_ROOT}/skills/story/scripts/area-freshness.sh` — the upkeep seam for `deployments/` and `terms/`, the two areas with no writer in the loop that survived the 2026-08-13 reshape. Facts, never verdicts, and it never edits a record: `retired_terms` (the record still names a de-listed area or a retired namespace — wrong, not merely old) and `stale_days` (reported, thresholded by nobody). Raise it only for a record this branch touched or affected; the known backlog is not every branch's concern. Full judgment rules: [`reference/release-readiness.md`](reference/release-readiness.md) §4a.
- **Doc-drift backstop**: `bash ${CLAUDE_PLUGIN_ROOT}/skills/story/scripts/doc-drift.sh "<base_branch>"` returns drift facts, not verdicts. With **no argument** the base is resolved by `gather/scripts/base-ref.sh` — `origin/<default>`, what the pull request is diffed against — never a local `main`, which in a claim worktree is whatever the clone happened to fetch and would narrate another merge's changes as this branch's drift; an unresolvable base comes back `not_applicable` by name (`base_never_fetched` / `no_base_ref` / `base_unresolved`) rather than silently falling back — judge each candidate against the diff and the doc's content, exactly like the deferred-concern judge. Confirmed drift becomes a readiness concern plus a `pre_release` instruction, and a durable Concerns entry via `workaholic:review-sections`. `outputs/` staleness and version/manifest drift are excluded — other guards own those domains.
