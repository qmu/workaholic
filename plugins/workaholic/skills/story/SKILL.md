...
- **Phase 0** — bump the version unless `check-version-bump.sh` says `already_bumped`; gather context via `bash ${CLAUDE_PLUGIN_ROOT}/skills/gather/scripts/git-context.sh`. **A degraded read (`ok: false`, with `reason`) means bump** — it already answers `already_bumped: false` for exactly that reason, and the two directions are not symmetric: a redundant bump is a no-op commit a human drops, while a skipped one ships plugin changes on a stale version. Report the `reason` in the run's output; never re-derive the base by hand.
