...
- **Phase 0** — bump the version unless `check-version-bump.sh` says `already_bumped` **and** `version_ahead: true` (2026-09-02: a branch that bumped is not the same as a branch whose number is still free — several branches cut from one base each bump `N → N+1` correctly, the first to merge takes `N+1`, and every later squash then carries a bump commit whose *content* the base already holds, so it produces no version diff and nothing reports it; measured, five consecutive merges on this repository carried two versions between them). **`version_ahead: null` means bump** — it is the unreadable-manifest answer, and the direction is the one the degraded base read already takes; gather context via `bash ${CLAUDE_PLUGIN_ROOT}/skills/gather/scripts/git-context.sh`. **A degraded read (`ok: false`, with `reason`) means bump** — it already answers `already_bumped: false` for exactly that reason, and the two directions are not symmetric: a redundant bump is a no-op commit a human drops, while a skipped one ships plugin changes on a stale version. **The same asymmetry governs `version_ahead`**, which is why its unknown bumps too. Report the `reason` in the run's output; never re-derive the base by hand.
