...
- **Phase 0** — first run `scripts/release-boundary.sh`. Bump the version only when it reports
  `eligible: true`: the branch has archived the ticket set of exactly one mission, and the
  mission is archived `achieved`, fully accepted and queue-empty. `eligible: false` is a
  ...
  proposal/ticket merges from product releases. At an eligible boundary, bump unless
