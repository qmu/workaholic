...
- **Doc-drift backstop**: `bash ${CLAUDE_PLUGIN_ROOT}/skills/story/scripts/doc-drift.sh "<base_branch>"` returns drift facts, not verdicts — judge each candidate against the diff and the doc's content, exactly like the deferred-concern judge. Confirmed drift becomes a readiness concern plus a `pre_release` instruction, and a durable Concerns entry via `workaholic:review-sections`. `outputs/` staleness and version/manifest drift are excluded — other guards own those domains.
