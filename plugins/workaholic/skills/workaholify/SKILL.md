...
**The designed wiring, per template** (the developer's instruction, 2026-08-06): `[Propose]` fires when **a GitHub issue assigned to the developer is opened**; `[Implement]` fires when **a proposal's pull request merges** (Pull request → closed, filtered `is merged: true`, title contains `[Proposal]`, **and author = the developer**). One event, one owner — and with `[Consent]` retired, the merged-pull-request event has exactly one owner rather than two templates to keep apart.
